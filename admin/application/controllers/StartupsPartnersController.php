...
class StartupsPartnersController extends CI_Controller {
	...
	public function index()
	{
					  ...
					  if (!empty($_FILES['file']['name']))
					   {
                        ...
                        if($width > "100" || $height > "100") { 
                            ...
                            $this->session->set_flashdata('error', 'Image size must be 100 x 100 pixels!!');
							redirect ('startups-partners');
                        }
			            $config['upload_path'] = './../assets/cdma/startups-partners';
			            ...
			            if ($this->upload->do_upload('file'))
			              {
			                ...
			                $Array = array('section_id' => '7', 'category' => $category, 'file' => $file, 'status' => 'Enable', 'created_at' =>$todat_date_time);

			                $run = $this->CreatePostModel->insert('tbl_startups_partners', $Array);
			                 if ($run) 
							   {
	  ...
	  $submenudata=array();
	   ...
	   // print_r($forms);
	    ...
	    $data['all_startups_partners']= $this->CreatePostModel->all_records('tbl_startups_partners', $params);
	    //print_r($data['all_startups_partners']);exit;
	    $this->load->view('header',$data);
		$this->load->view('startups-partners/startups-partners',$data);
	...
	public function delete_about_content()
	{
	    ...
	    $del = $this->CreatePostModel->delete('tbl_startups_partners', array('id' => $delid));
	    if ($del) 
	     {
