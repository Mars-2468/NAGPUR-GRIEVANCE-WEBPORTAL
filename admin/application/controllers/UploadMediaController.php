	...
	public function imageUploadPost()
{
    ...
    //$base_path = FCPATH . "assets/$ulbid/$curyear/$curmonth/mediafiles/";
    $base_path = "../assets/$ulbid/$curyear/$curmonth/mediafiles/";
    $relative_path = "assets/$ulbid/$curyear/$curmonth/mediafiles/";
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        // ✅ PDF malware validation
        if ($mime === 'application/pdf') {
            if ($this->isMaliciousPdf($file_tmp)) {
                $rejectedFiles[] = $file_name;
                continue;
            }
        }
        ...
        $newFileName = uniqid() . '_' . preg_replace('/[^A-Za-z0-9_\-\.]/', '', $file_name);
        ...
        if (in_array($mime, ['image/png', 'image/jpeg'])) {
            ...
            if ($image_width > 300) {
                ...
                $thumbnail_path = $relative_path . $newFileName;
                //$thumbnail_path = $relative_path . '/adhikari/assets/img/pdf.png';
            }
        }
        ...
        $params = [
            'updatedBy'         => $userid,
            'ulbid'             => $ulbid,
            'folder_path'       => $relative_path . $newFileName,
            'image_path'        => $relative_path . $newFileName,
            'status'            => 0,
            'file_type'         => $mime,
            'file_path'         => $base_path,
    ...
    if (!empty($rejectedFiles)) {
        $this->session->set_flashdata(
            'error_message',
            'Some files were rejected due to security validation: ' . implode(', ', $rejectedFiles)
        );
    } else {
        $this->session->set_flashdata(
            'message',
	...
	public function isMaliciousPdf($filePath): bool
{
