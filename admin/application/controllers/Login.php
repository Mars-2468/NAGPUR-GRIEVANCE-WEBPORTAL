...
class Login extends CI_Controller {
	...
	{	
		...
		//echo "<pre>";print_r($user_session_id);echo "</pre>";die();
	    ...
	    if($this->input->post('submit'))
	    {
		 ...
		 //echo "<pre>";print_r($inputCaptcha==$sesscaptcha);echo "</pre>";die();	
		...
		//echo "<pre>";print_r($this->is_strong_password($inputpassword));echo "</pre>";die();		   
			
			   
			   if($this->is_strong_password($inputpassword)){			   
					$password=md5($inputpassword);
			   }else{
				    $this->session->set_flashdata('error_message',"<div class='alert alert-danger'>Invalid username or password,<br>Password must be at least 8 characters and include uppercase, lowercase, number, and special character.</div>");
					redirect('/');
			   }
	 	...
	 	//echo "<pre>";print_r($data['result']);echo "</pre>";die();	
				...
				$user_agent = $this->getBrowserName($this->input->user_agent());	           
			   
			    $existing_session = $this->db->where('user_id', $data['object']['user_id'])->get('user_sessions')->row();
  
				// Generate a new session ID
				//$this->session->sess_regenerate(TRUE);

				// Store new session ID in the database
                ...
                if ($existing_session->session_id!='') {					
					...
					if(empty($existing_session)){
						...
						$this->LoginModel->update_session_id($data['object']['user_id'], $session_id);
					}
	                ...
	                $this->session->set_userdata(array( 
	                    'userid'=>$data['object']['user_id'],
						'has_access'=>$data['object']['has_access'],
