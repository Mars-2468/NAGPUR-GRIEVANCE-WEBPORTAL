...
class AboutSliderController extends CI_Controller {
	...
	public function index()
	{
					  ...
					  if (!empty($_FILES['file']['name']))
					   {
                        ...
                        $run = $this->CreatePostModel->insert('about_slider', $Array);
                         if ($run) 
        				   {
        				     $this->session->set_flashdata('success', 'Apply Now Menu Added Successfully !!');
        				     redirect ('about_slider');
        				   }
					      }
					   }
					}
	  ...
	  $submenudata=array();
	   ...
	   // print_r($forms);
	    ...
	    $data['all']= $this->CreatePostModel->all('about_slider', $params);
	    //print_r($data['all_startups_partners']);exit;
	    $this->load->view('header',$data);
		$this->load->view('testimonials/about_slider',$data);
	...
	public function delete()
	{
	    ...
	    $del = $this->CreatePostModel->delete('about_slider', array('id' => $delid));
	    if ($del) 
	     {
	...
	public function edit_about_slider()
	{
	    
	   if($this->session->userdata('session_id')==session_id())
	    {
	        $editid = $this->uri->segment('2');
	        $data['dtl'] = $this->CreatePostModel->get_row('about_slider', array('id' => $editid));
			            ...
			            if ($this->upload->do_upload('file'))
			              {
			                ...
			                $run = $this->CreatePostModel->update('about_slider', $Array, array('id' => $editid));
					      }
					   }
					}
	  ...
	  $submenudata=array();
	   ...
	   // print_r($forms);
	    ...
	    $this->load->view('testimonials/edit_about_slider',$data);
