...
class CustomePageController extends MY_Controller
{
	...
	public function updatePageContent()
	{
		...
		//echo "<pre>";print_r($this->input->post('content_new'));echo "</pre>";die();
		...
		if ($this->session->userdata('session_id') == session_id()) {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if ($this->input->post('save')) {
					...
					$content = $this->input->post('content_new',FALSE);
					...
					$pageid = intval($this->security->xss_clean(trim($this->input->post('pageid'))));
					...
					$imageurl=$this->CreatePostModel->updateSliderThumbnailimage($pageid,$photo_file_url);
						//echo $imageurl; exit;
					$this->session->set_userdata('source_image', $imageurl);
					
					$content_new = $content;//$_POST['content_new'];
/* 
	...
	// disabled validations
		...
		/* 		foreach($params4 as $key=>$param){
					if (preg_match('/<[^<]+>/', $param)) {
					
					$this->session->set_flashdata('error_message', "Error: HTML or script tag detected in ".$key." field!");
			
						//echo "Error: HTML or script tag detected in ".$key." field!" ;
						redirect($_SERVER['HTTP_REFERER']);
						exit;
					}
				}  */
					
				//$params4['content']=$content_new;
					
					
	//echo "<pre>";print_r($content_new);echo "</pre>";die();			
		...
		//echo "<pre>";print_r($table);echo "</pre>";die();					/*** updating slider thumbnail image ****/
						...
						$imgurl=$this->fileUpload($_FILES);
						
				//echo "<pre>";print_r($imgurl);echo "</pre>";die();
									
								
						$this->session->set_userdata('source_image', $imgurl);
						
						if (in_array('slider_mst2', $alltables)) {
							if (!$code == 1) {
								$redirect_url = "edit-crop-image/" . $pageid;							
								
								//echo "<pre>";print_r($this->session->userdata('source_image'));echo "</pre>";die();
		...
		// loading 
				...
				//echo "<pre>";print_r($data);echo "</pre>";die('test223333221...');	
				
				foreach ($selected_values as $key => $val) {
					$data['categories_selected'][$val['category_id']]['checked'] = 'checked'; // adding checked values to the mapped with this page or post
					if ($val['table_name'] != '') {
	...
	//echo "<pre>";print_r($data['content']);echo "</pre>";die();			
				
			$data['slider_data'] = $this->CustomModel->getSliderData($data['content'][0]['page_id']);
	//echo "<pre>";print_r($data['slider_data']);echo "</pre>";die();	
			$ulb = $this->session->userdata('ulbid');
			$ulb_baseurl = $this->ViewPagesModel->getulb_baseurl($ulb);
			...
			// getting innerpage layouts
			$data['innerpagelayouts'] = $this->CustomModel->getInnerpageLayouts($data['theme_id']['theme_id']);
//echo "<pre>";print_r($data['theme_id']);echo "</pre>";die();
			$params = array('ulbid' => $this->session->userdata('ulbid'), 'is_custumlink' => 3, 'langId' => $this->session->userdata('langId'));
			// will get all categories created by users and super admin
			...
			$params['page_id']=$data['content'][0]['page_id'];
			
			$categories = $this->CreatePostModel->getPostCategoriescustom($params, $assignDeptArray);
	...
	public function fileUpload($FILESS){
		...
		//echo "<pre>";print_r($FILESS['image_path']);echo "</pre>";die('ssss');		
			   if (isset($FILESS['image_path']) && $FILESS['image_path']['error'] == 0) {

					// Get file information
					$file_name = $FILESS['image_path']['name'];
					$file_tmp = $FILESS['image_path']['tmp_name'];
					$file_size = $FILESS['image_path']['size'];
					$file_error = $FILESS['image_path']['error'];
				//echo "<pre>";print_r($FILESS['image_path']['tmp_name']);echo "</pre>";die('ssss');		
					// Allowed file types
					$allowed_extensions = ['jpg', 'jpeg', 'png', 'gif'];
					$file_ext = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));

 //echo "<pre>";print_r(!in_array($file_ext, $allowed_extensions));echo "</pre>";die('ssss');				
