            ...
            $p = array(
             ...
             "gO5a" => '?',
                ...
                $pass = implode('',$p_dummy1);
                 ...
                 // echo"<pre>";print_r($_SESSION['userid']);echo"</pre>";die();
			 ...
			 $str1 = str_replace(' ', '', htmlspecialchars(strip_tags($this->input->post('old_password'))));
			
            $len = $str1[0];
            $hash1 = substr($str1,1,$len);
            $hash2 = substr($str1, -$len);
            $old_password1 = substr($str1, $len+1);
            $old_password = substr($old_password1, 0 , -$len);
				...
				$userpwd=hash('SHA256',$this->input->post('old_password'));
				//$userpwd=sha1(md5($this->input->post('old_password')));
				
                $user_id = $this->session->userdata('userid');
                $params = array('user_id'=>$user_id,'user_pwd'=>$userpwd);
                        // print_r($params);die;
                    ...
                    if($chkoldpassword) {
                                ...
                                elseif (preg_match("/\s/", $pass)) {
									...
									$secretKey=$_SESSION['userid'];
									
									//echo"<pre>";print_r($_POST);echo"</pre>";die();
									
									$input_pass=$this->encrypt_text($pass, $secretKey);
