...
class EventsController extends CI_Controller {
	...
	public function index()
	{
	   if($this->session->userdata('session_id')==session_id())
	    {
	        if($this->input->post('submit'))
	        {
              ...
              {
                $this->session->set_flashdata('namedateerror', 'Title & Date Required !!');
                redirect ('addevents');
              }  
			  ...
			  if (!empty($_FILES['file']['name']))
			   {
	            $config['upload_path'] = './../assets/cdma/css/aurangabad/images';
	            ...
	            if ($this->upload->do_upload('file'))
	              {
                    ...
                    $run = $this->CreatePostModel->update('tbl_map', $array, array('id' => '1'));
	              } 
			   }
	  ...
	  $submenudata=array();
	   ...
	   // print_r($forms);
	    ...
	    $data['all']= $this->CreatePostModel->all_complaint_details('tbl_complaint_details');
	...
	public function delete()
	{
	    ...
	    $del = $this->CreatePostModel->delete('table_events', array('id' => $delid));
	    if ($del) 
	     {
	...
	public function edit_events()
	{
	   if($this->session->userdata('session_id')==session_id())
	    {
	        ...
	        $data['dtl'] = $this->CreatePostModel->get_row('table_events', array('id' => $editid));
	        
	        if($this->input->post('update'))
	        {
	          $this->form_validation->set_rules('title','Title','trim|required');
              $this->form_validation->set_rules('dt','Date','trim|required');
              ...
              if($this->form_validation->run() == false)
              {
                ...
                $run = $this->CreatePostModel->update('table_events', $Array, array('id' => $editid));
                 if ($run) 
				   {
	  ...
	  $submenudata=array();
	   ...
	   // print_r($forms);
	    ...
	    $data['all']= $this->CreatePostModel->all_records('table_events', $params);
	    //print_r($data['all_startups_partners']);exit;
	    $this->load->view('header',$data);
	    $this->load->view('events/edit_events',$data);
		$this->load->view('divdata',$data);
		$this->load->view('footer');
	...
	public function contactusDetails()
	{
	   if($this->session->userdata('session_id')==session_id())
	    {
	        $data['dtl'] = $this->CreatePostModel->get_row('contact_dtl', array('id' => '1'));
	        ...
	        if($this->input->post('update'))
	        {
                ...
                $run = $this->CreatePostModel->update('contact_dtl', $Array, array('id' => '1'));
                 if ($run) 
				   {
				     $this->session->set_flashdata('success', 'Update Successfully !!');
				     redirect ('contactus_details');
				   }
	        }
	  ...
	  $submenudata=array();
	   ...
	   // print_r($forms);
	    ...
	    $params = array('status' => '1');
	    $data['all']= $this->CreatePostModel->all_records('contact_dtl', $params);
	    //print_r($data['all_startups_partners']);exit;
	    $this->load->view('header',$data);
	    $this->load->view('events/edit_contact_dtl',$data);
		$this->load->view('divdata',$data);
		$this->load->view('footer');
