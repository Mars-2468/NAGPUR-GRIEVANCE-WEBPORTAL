...
class ApplynowmenuController extends MY_Controller {
	...
	public function index()
	{
	   if($this->session->userdata('session_id')==session_id())
	    {
	        ...
	        if($this->input->post('applynow'))
	        {
                ...
                $run = $this->CreatePostModel->insert('apply_now_menu', $Array);
                 if ($run) 
				   {
				     $this->session->set_flashdata('success', 'New Added Successfully !!');
				     redirect ('recent-announcements-text');
				   }
	        }
	  ...
	  $submenudata=array();
	   ...
	   // print_r($forms);
	    ...
	    $data['all']= $this->CreatePostModel->all('apply_now_menu', $params);
	    //print_r($data['all_startups_partners']);exit;
	    $this->load->view('header',$data);
		$this->load->view('testimonials/applynowmenu',$data);
	...
	public function delete()
	{
	    ...
	    $del = $this->CreatePostModel->delete('apply_now_menu', array('id' => $delid));
	    if ($del) 
	     {
	...
	public function edit_apply_now_menus()
	{
	    
	   if($this->session->userdata('session_id')==session_id())
	    {
	        $editid = $this->uri->segment('2');
	        $data['dtl'] = $this->CreatePostModel->get_row('apply_now_menu', array('id' => $editid));
	        ...
	        if($this->input->post('update_01'))
	        {
                ...
                $run = $this->CreatePostModel->update('apply_now_menu', $Array, array('id' => $editid));
                 if ($run) 
				   {
	  ...
	  $submenudata=array();
	   ...
	   // print_r($forms);
	    ...
	    $this->load->view('testimonials/edit_apply_now_menus',$data);
