...
class MapPageController extends CI_Controller {
	 ...
	 */
	 ...
	 public function saveNewMenu()
	 {
	      ...
	      if($this->session->userdata('session_id')==session_id())
	    {
	     ...
	     if($this->security->xss_clean(trim(htmlspecialchars(strip_tags($this->input->post('savemenu'))))))
	     {
	         ...
	         else
	         {
	             ...
	             $params=array(
	                 ...
	                 'menu_type_desc'=>htmlspecialchars(strip_tags($this->input->post('menuname')))
	                 );
	      ...
	      if($this->session->userdata('session_id')==session_id())
	    {
                	        if(!empty($this->security->xss_clean(trim($this->input->post('page_id')))) && !empty($this->security->xss_clean(trim($this->input->post('page_name')))) &&  !empty($this->security->xss_clean(trim($this->input->post('page_title')))))
                	        {
                    	     ...
                    	     $params=array(
                    	         ...
                    	         'page_title'=>$this->security->xss_clean(trim(htmlspecialchars(strip_tags($this->input->post('page_title'))))),
                    	         'is_targetblank'=>$this->security->xss_clean(trim(htmlspecialchars(strip_tags($this->input->post('is_targetblank'))))),
                    	         'is_alert'=>$this->security->xss_clean(trim(htmlspecialchars(strip_tags($this->input->post('is_alert'))))),
                    	         'hover_title'=>$this->security->xss_clean(trim(htmlspecialchars(strip_tags($this->input->post('hover_title'))))),
                    	         'page_url'=>$this->security->xss_clean(trim(htmlspecialchars(strip_tags($this->input->post('page_url')))))
                    	         );
	 ...
	 {
	     //echo json_encode($this->input->get('page_id'));exit;
    	 if($this->session->userdata('session_id')==session_id())
	    {
                	        if($this->security->xss_clean(trim($this->input->get('page_id'))))
                	        {
                    	     $page_id=$this->security->xss_clean(trim(htmlspecialchars(strip_tags($this->input->get('page_id')))));
	 ...
	 public function createCustomLink()
	 {
	    ...
	    {  
	     ...
	     $linkText=$this->security->xss_clean(trim(htmlspecialchars(strip_tags($this->input->post('linktext')))));
	     $menu_type_id=$this->security->xss_clean(trim(htmlspecialchars(strip_tags($this->input->post('menu_type')))));
	     $params=array(
	         'page_title'=>$linkText,
	         'page_name'=>$linkText,
	         'controller'=>$url,
	         ...
	         'is_alert'=>$this->security->xss_clean(trim(htmlspecialchars(strip_tags($this->input->post('is_alert'))))),
	         'is_target_blank'=>$this->security->xss_clean(trim(htmlspecialchars(strip_tags($this->input->post('is_targetblank')))))
	         );
	...
	public function index()
	{
	   ...
	   if($this->session->userdata('session_id')==session_id())
	    ...
	    if($this->security->xss_clean(trim(htmlspecialchars(strip_tags($this->input->post('select'))))))
	        {
	         $this->security->xss_clean(trim(htmlspecialchars(strip_tags($this->input->post('menuType')))));
	         $selectedMenu=$this->security->xss_clean(trim(htmlspecialchars(strip_tags($this->input->post('menuType')))));
	       }
	...
	public function ajax_pages_search(){
	     ...
	     if($this->session->userdata('session_id')==session_id())
	    {
	        ...
	        $id=$this->security->xss_clean(trim(htmlspecialchars(strip_tags($_POST['page_val']))));
	        $id1=$this->security->xss_clean(trim(htmlspecialchars(strip_tags($_POST['post_val']))));
	        $id2=$this->security->xss_clean(trim(htmlspecialchars(strip_tags($_POST['customlink_val']))));
	        $id3=$this->security->xss_clean(trim(htmlspecialchars(strip_tags($_POST['categori_val']))));
	       
	        $ulbid=$this->session->userdata('ulbid');
	        $is_draft='0';
	        $is_custumlink='0';
	        $langId=$this->session->userdata('langId');
	        $keyword4 =$this->security->xss_clean(trim(htmlspecialchars(strip_tags($_POST['keyword']))));
	      ...
	      $searching_pages = $this->AddMenuModel->ajax_page_search($ulbid,$is_draft,$is_custumlink,$langId,$keyword4,$id);
	        ...
	        $is_custumlink='2';
	        $langId=$this->session->userdata('langId');
	        $keyword5 =$this->security->xss_clean(trim(htmlspecialchars(strip_tags($_POST['keywordpost']))));
	     ...
	     $searching_posts = $this->AddMenuModel->ajax_page_search($ulbid,$is_draft,$is_custumlink,$langId,$keyword5,$id1);
	        ...
	        $is_custumlink='1';
	        $langId=$this->session->userdata('langId');
	        $keyword6 =$this->security->xss_clean(trim(htmlspecialchars(strip_tags($_POST['keywordcustomlink']))));
	      ...
	      $searching_customlinks = $this->AddMenuModel->ajax_page_search($ulbid,$is_draft,$is_custumlink,$langId,$keyword6,$id2);  
	        ...
	        $is_custumlink='3';
	        $langId=$this->session->userdata('langId');
	        $keyword7 =$this->security->xss_clean(trim(htmlspecialchars(strip_tags($_POST['keywordcategories']))));
