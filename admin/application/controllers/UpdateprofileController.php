...
class UpdateprofileController extends CI_Controller {
	...
	public function index()
	{
	    ...
	    $data['getuser_categories']=$this->CreaUlbUserModel->getuser_categories($user_category);
	   ...
	   /* $params=array('user_id'=>$this->session->userdata('username'),'langId'=>$this->session->userdata('langId'));
        ...
        if($this->input->post('save'))
        {
            ...
            if($this->session->userdata('user_level') == 'A'){
                $ulbid = $this->input->post('municipality');
            }else{
                $ulbid = $this->session->userdata('ulbid');
            }
            $userdetails=array(
                ...
                'user_type'=>$this->input->post('user_level_id'),
                'ulbid'=> $ulbid,
                'user_category'=>$this->input->post('user_category'),
                'is_custom_user'=>$this->input->post('is_custom_user'),
                ...
                if($this->input->post('is_custom_user')=='Yes'){
                    ...
                    for($j=1;$j<$widget_count-1;$j++)
                    {
                        ...
                        if($this->input->post($widget) > 0)
                        {
                            ...
                            $params2['user_id']=$this->input->post('user_id');
                    ...
                    $result1=$this->CreaUlbUserModel->addCommonpages($this->input->post('user_id'));
                    if($result1){
	    ...
	    $data['edituser']=$this->CreaUlbUserModel->edituserprofile($user_id);
		...
		public function check_userid_avalibility(){
	     ...
	     if($this->CreaUlbUserModel->check_userid_avalibility_profile($_POST["user"]))  
               {  
	      ...
	      if($this->input->post('save'))
	      {
	       $user_id=$this->input->post('user_id1'); 
	        ...
	        $userdetails=array(
                ...
                'user_id'=>$this->input->post('user_id')
               // 'user_pwd'=>$this->input->post('password'),
               // 'show_pwd'=>$this->input->post('password')
            
            );
	      $data=$this->CreaUlbUserModel->update_userprofile($user_id,$userdetails);
	       if($data){
