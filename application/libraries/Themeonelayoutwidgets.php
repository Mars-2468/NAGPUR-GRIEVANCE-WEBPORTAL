		...
		if ($parameters['widget_type'] == 1) {
			...
			if ($result[0]['widget_type_style'] == 1) {
				...
				$content .= '<div class="col">
						<div class="card card22 card-color shadow pointer">
						<div class="card-body" style="padding-left:0px !important;padding-right:0px !important;padding-bottom:0px !important;">
							<p class="card-text text-center small" style="font-size:12px;padding:0px;">' . $result[0]['title'] . '</p>
							<p class="card-text text-center small" style="display:none;font-size:12px;">' . $result[0]['description'] . '</p>
							<a class="pointer" onclick="toggleContent(this)" style="text-decoration:none;font-size:10px;color:#0000FF;float:right"><em>more...</em></a>
						 </div>
						  <div class="card-footer d-flex justify-content-between align-items-center">
							<div class="text-center pl-2 pr-2 ">';
								
							if(!empty($result[0]['source_path']) && !file_exists($result[0]['source_path'])){
								$content .= '<img src="' . $result[0]['source_path'] . '" class="card-img-top imgbr" alt="' . $result[0]['title'] . '" style="width:30px;height:30px;">';
							}else{
								$content .= '<img src="assets/nmc/ngmc/images/image-icon.jpg" class="card-img-top imgbr" alt="' . $result[0]['title'] . '" style="width:30px;height:30px;">';
							}
						  ...
						  <div>
							<small class="text-body-secondary">
								<div class="text-center">	
									<a href="' . $result[0]['url_link'] . '" target="_blank" class="btn btn-secondary btn-sm  custom-btn" style="border-radius:10px;font-size:10px;" ><em>Click here</em></a>
								</div>
							</small>
						  </div>
						  </div>
			...
			// <a href='".$result[0]['url_link']."' ".$target.">".$result[0]['title']."</a>
				...
				$content .= '<div class="col">
								<a class="lnk" href="' . $result[0]['url_link'] . '" target="_blank" style="text-decoration:none;color:blue;">
									<div class="card card22 card-color shadow pointer mb-1" style="max-width: 540px;">
										<div class="col-md-12">
											<div class="d-flex justify-content-left">
												<div class="col-md-4 p-2">
												  <img src="' . $result[0]['source_path'] . '" class="img-fluid rounded-start1 imgbr" style="width:60px;height:60px;"  alt="' . $result[0]['file_name'] . '">
												</div>
												<div class="col-md-8 small p-1 d-flex justify-content-left text-primary align-items-center">
													' . $result[0]['title'] . '				  
												</div>
											</div>
										</div>
			...
			$select_array = array('s.page_id', 's.menu_id', 'c.tabs_status', 'c.page_name', 'c.hover_title', 'c.controller', 'c.is_custumlink', 'c.is_target_blank', 'c.site_controller', 'c.is_alert');
			$this->CI->db->select($select_array);
			$this->CI->db->from('custom_menus c');
			...
			foreach ($result['main_menus'] as $key => $mainmenuarray) {
				...
				$data1[$mainmenuarray['menu_id']]['tabs_status'] = $mainmenuarray['tabs_status'];
				$data1[$mainmenuarray['menu_id']]['child'] = array();
			}
			...
			$uri = $parameters['uri'];
			$content .= '
				<div class="d-flex justify-content-center nav-bg" id="menu-top">

				<nav class="wsmenu clearfix">
					<label for="wsmainlinks" class="wsdownopener"><i class="fas fa-bars"></i> Menu</label>
					<input type="checkbox" id="wsmainlinks" class="wsdexpander" />
					<ul class="wsmenu-list">
						<li aria-haspopup="true"><a href="' . base_url() . '" class="wshomeico active"><i class="fas fa-home"></i><span
			...
			foreach ($mainmenus as $key => $mainmenu_det) {
				...
				if (in_array($mainmenu_det['page_id'], $megamenupages)) {
					...
					if ($mainmenu_det['tabs_status'] == '1') {
						$content .= '
							
							<li aria-haspopup="true" class="nav-item dropdown">
							<label for="wsmainlinks-3" class="wsdownopener-sub"></label>
							<a class="nav-link dropdown-toggle dropbtn" href="#" id="navbarDarkDropdownMenuLink" role="button" data-bs-toggle="dropdown" aria-expanded="false">' . $mainmenu_det['page_name'] . ' </a>
						...
						foreach ($submenus[$key] as $key2 => $submenu_det) {
							...
							$content .= "<ul class='col-lg-3 col-md-12 col-xs-12 link-list'>
									<li class='title'><a href='" . $submenu_det['site_controller'] . "'>" . $submenu_det['page_name'] . " </a></li>";
							foreach ($subsubmenus[$key][$key2] as $key3 => $subsubdet) {
								...
								$content .= "<li class='inner-menu'><a href='" . $string . $subsubdet['site_controller'] . "'><i class='fas fa-arrow-circle-right'></i>" . $subsubdet['page_name'] . "</a></li>";
							}
							$content .= " </ul>";
						}
						...
						foreach ($submenus[$key] as $key2 => $submenu_det) {
							if (count($subsubmenus[$key][$key2]) <= 0) {
								...
								foreach ($subsubmenus[$key][$key2] as $key3 => $subsubdet) {
									...
									$content .= "
										<li aria-haspopup='true'><a href='" . $string . $subsubdet['site_controller'] . "' class='dropdown-item" . $alertClass . "' " . $target . " title='" . $subsubdet['hover_title'] . "'>" . $subsubdet['page_name'] . "</a></li>
						...
						$content .= '
							<li aria-haspopup="true">
								...
								<a  href="' . $base_url . $mainmenu_det['site_controller'] . '" title="' . $mainmenu_det['hover_title'] . '" ' . $target . ' >' . $mainmenu_det['page_name'] . ' </a>
							</li>
            				  ';
					}
				}
			...
			foreach ($result as $key => $val) {
				...
				$post_list[$val['page_id']]['site_controller'] = $val['site_controller'];
				$post_list[$val['page_id']]['sliderLinkText'] = "https://nmcnagpur.gov.in/public-notices";
			}
			...
			foreach ($result as $key => $val) {
				...
				$post_list[$val['page_id']]['last_updated'] = $val['last_updated'];
				$post_list[$val['page_id']]['site_controller'] = $val['site_controller'];
				$post_list[$val['page_id']]['sliderLinkText'] = "https://nmcnagpur.gov.in/work-orders";
			}
            ...
            $post_list[$val['page_id']]['last_updated'] = $val['last_updated'];
            $post_list[$val['page_id']]['site_controller'] = $val['site_controller'];
            $post_list[$val['page_id']]['sliderLinkText'] = "https://nmcnagpur.gov.in/quotations";
        }*/
			...
			$content .= '<div class="col-md-4 news-update">
				...
				<div class="card">
				  ...
				  <marquee direction="up" scrollamount=3 onmouseover= stop() onmouseout= start() style="height:96%">  ';
			//print_R(); exit;
			...
			$content2 .= '
             ...
             <a class="twitter-timeline" href="https://twitter.com/majhi_smart_bus?ref_src=twsrc%5Etfw">Tweets by majhi_smart_bus</a> 
             </div>
