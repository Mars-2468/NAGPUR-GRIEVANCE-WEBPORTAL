...
<div id="div_print" class="cap_font" style="border:#999999 0px solid;">
	...
	<div class="tab-content">
		...
		<div class="tab-pane active table-responsive">
			<div style="display:flex;justify-content: space-between;align-items:center;padding:20px;margin:10px;">
				<div>
					<caption style="caption-side:top;padding-top: 10px;padding-bottom: 10px;font-size:16px;">
						<b>
							{if $origin_id eq '0' }
			...
			<table width="97%" border="1" cellspacing="0" cellpadding="0" class="display table-bordered table-striped table-condensed cf" id="data-table">
				...
				<tr>
					...
					<td align="center" class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}" valign="middle"><strong>{if $data2.pendingforapproval eq ''} 0 {else}<a href="corp_Totalorigin.php?ulbid={$ulbid}&originid={$origin_id}&status=100" target="_blank">{$data2.pendingforapproval}</a>{/if}</strong></td>
					<td align="center" class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}" valign="middle"><strong>{if $data2.pending eq ''} 0 {else}<a href="corp_Totalorigin.php?ulbid={$ulbid}&originid={$origin_id}&status=200" target="_blank">{$data2.pending} </a>{/if}</strong></td>
					<td align="center" class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}" valign="middle"><strong>{if $data2.completed eq ''} 0 {else}<a href="corp_Totalorigin.php?ulbid={$ulbid}&originid={$origin_id}&status=300" target="_blank">{$data2.completed} </a>{/if}</strong></td>
					<td align="center" class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}" valign="middle"><strong>{if $data2.total eq ''} 0 {else}<a href="corp_Totalorigin.php?ulbid={$ulbid}&originid={$origin_id}&status=400" target="_blank">{$data2.total} </a>{/if}</strong></td>
				</tr>
		...
		<th align="center" valign="middle">Receipt</th>
		...
		{assign var="i" value="1"}
		{foreach from=$data3 key=grievance_id item=row}
		<tr>
			<td>{$i}</td>
			<td>{$grievance_id}</td>
			<td>{$row.date_regd}</td>
		    ...
		    <td><a href="corp_receive_print.php?gid={$grievance_id}&aptid={$row.app_type_id}" target="_blank">Receipt</a></td>
		    <td>
		        ...
		        <form action="corp_view_pending_approval.php" method="POST">
		            <input type="hidden" name="grievance_id" value="{$grievance_id}">
		            <input type="submit" value="Assign To Employee" class="btn btn-success">
