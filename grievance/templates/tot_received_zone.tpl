	...
	<form method="POST" class="form-horizontal">
		...
		<div class="boxed">
			...
			<div class="inner no-radius">
				...
				<div style="text-align:right;"><a href="ajax_dashboard_zone.php" class="btn btn-warning"><i class="fa fa-backward"></i> Back</a></div> -->
				...
				<table border="1" cellspacing="0" cellpadding="0" class="table table-bordered" id="data-table" width="100%">
					<caption style="caption-side:top; text-align:center;font-size:16px;">
						<b>
							...
							<CENTER><strong>VIEW {if $app_type_id eq '1'} ZONE RECEIVED COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

							{elseif $status eq '111' && $sla eq '0' }
							<CENTER><strong>VIEW {if $app_type_id eq '1' } ZONE TODAY'S RECEIVED COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

							{elseif $status eq '802' && $sla eq '2' }
							<CENTER><strong>VIEW {if $app_type_id eq '1'} ZONE TOTAL RESOLVED COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

							{elseif $status eq '803' && $sla eq '2' }
							<CENTER><strong>VIEW {if $app_type_id eq '1' } ZONE TOTAL UNDERPROGRESS COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

							{elseif $status eq '804' }
							<CENTER><strong>VIEW {if $app_type_id eq '1'} ZONE FINANCIAL IMPLICATIONS COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

							{elseif $status eq '805' && $sla eq '1' }
							<CENTER><strong>VIEW {if $app_type_id eq '1'} ZONE COMPLETED WITHIN SLA COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

							{elseif $status eq '806' && $sla eq '2' }
							<CENTER><strong>VIEW {if $app_type_id eq '1'} ZONE COMPLETED BEYOND SLA COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

							{elseif $status eq '807' && $sla eq '1' }
							<CENTER><strong>VIEW {if $app_type_id eq '1'} ZONE UNDER PROGRESS WITHIN SLA COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

							{elseif $status eq '808' && $sla eq '2' }
							<CENTER><strong>VIEW {if $app_type_id eq '1'} ZONE UNDER PROGRESS BEYOND SLA COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

							{elseif $status eq '809' }
							<CENTER><strong>VIEW {if $app_type_id eq '1'} ZONE TOTAL REOPENED COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

							{elseif $status eq '810' }
							<CENTER><strong>VIEW {if $app_type_id eq '1'} ZONE REOPENED UNDER PROGRESS COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

							{elseif $status eq '811' }
							<CENTER><strong>VIEW {if $app_type_id eq '1'} ZONE REOPENED COMPLETED COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>
					...
					<thead>
						<!--01-06-24 <tr style="background-color:#2c3e50; color:#FFF;text-align:center;">
						...
						<!--Not Opened <th align="center" valign="middle">{if $app_type_id eq '1'} Complaint {else} SERVICE{/if} Details</th>	-->
						<!--01-06-24 <th align="center" valign="middle">Status</th>

							<th align="center" valign="middle">Updated Image</th>



							<th align="center" valign="middle" style="width: 100px">Received Date</th>
							<th align="center" valign="middle" style="width: 200px">Cut off Date</th>
						...
						<!--Not Opened <th align="center" valign="middle">Completed Date</th>
					...
					<tbody style="background-color:#FFF;">
						...
						<tr>
							...
							<!--11-06-2024 <td align="center" class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}"><a href="view_comp_det_admin.php?grievance_id={$grievance_id}">{$grievance_id}</a></td> -->
							...
							<td align="center">{$department_list[$row.dept_id]}</td>
...
<div style="width:100%; text-align:center">
    ...
    <button class="btn btn-primary" id="download-pdf" onclick="exportTableToPDF('example', 'GrievanceReport')" value="Dashboard"></i> Export to PDF</button> -->
		...
		<form action="exporttoexcel.php" method="post">
			<input type="hidden" name="status" value="{$status}">
			<input type="hidden" name="sla" value="{$sla}">
			<input type="hidden" name="user_type3" value="{$user_type3}">
