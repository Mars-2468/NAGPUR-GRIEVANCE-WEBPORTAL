				...
				<div id="area">
					<table id='example' cellpadding="3" cellspacing="0" border="1" class="display table-bordered table-striped table-condensed cf" width="100%">
							...
							<tr>
								...
								<td align='center' class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}">
									{if $data[$emp_id].count neq ''}
										<a href="all_showcause_notice_list.php?app_type_id={$app_type_id}&emp_id={$emp_id}&status=show&dept_id={$emp_dept_list[$emp_id]}&f_date={$fdate}&t_date={$tdate}" target="" style="text-decoration: none;">
											{$data[$emp_id].count}
										</a>
									{else}
										<b>-</b>
						...
						<tfoot>
							...
							{if $user_type eq 'U'}
								<td align='center' class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}">{if $tot.received neq ''}<a href="showcauseTotalNotices.php?ulbid={$ulbid}&app_type_id={$app_type_id}&status=show_all&f_date={$fdate}&t_date={$tdate}" style="text-decoration: none;"><b>{$tot.received}</b></a>{else}<b>0</b>{/if}</td>
							{else}
