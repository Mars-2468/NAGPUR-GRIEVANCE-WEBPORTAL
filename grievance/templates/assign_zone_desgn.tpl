...
<script>
	function fill(emp_id, ward_id) {
		//alert('hi');
		$('#emp_id').val(emp_id);
		$('#ward_id').val(ward_id);

		document.assign_zone_desgn.emp_id.value = emp_id;
		document.assign_zone_desgn.ward_id.value = ward_id;
	...
	function delete_hod(i, emp_id, ward_id) {
		if (confirm('Do Your Really Want To Delete This Record?')) {
			$.post('ajax_delete_map_zone.php', {
				emp_id: emp_id,
				ward_id: ward_id
			}, function(data) {
				if (data == 1) {
...
<script type="text/javascript" language="javascript">
	$(document).ready(function() { /// Wait till page is loaded
		$('#buss').click(function() {
			//alert();
			$('#ref').load('http://municipalservices.in/assign_zone_desgn.php #ref', function() {
	...
	<div>
		<div class="boxed">
			...
			<div class="title-bar success">
				<h4>MAPPING FOR ZONE DASHBOARD </h4>
			</div>
			<!-- Title Bart End -->
			<div class="inner no-radius">

				<form method="post" action="assign_zone_desgn.php" name="manage_emp" class="form-horizontal" onSubmit="return validateForm()">
					...
					<div class="form-body">
						...
						<div class="form-group">
							...
							<div class="col-md-4">
								<select name='ward_id' id='ward_id' class="form-control" autocomplete="off" required="required">
	...
	<div>
		<div class="boxed">
			...
			<div class="title-bar white">
				<h4>EXISTING MAPPING FOR ZONE DASHBOARD DETAILS</h4>
			</div>
			<!-- Title Bart End -->
			<div class="inner no-radius table-responsive">
				<form action="assign_zone_desgn.php" method="post" onSubmit="return validateForm2()">
					...
					<table class="table table-striped table-bordered table-hover table-full-width" id="data-table" width="100%">
						<caption style="caption-side:top;text-align:center;padding-top: 10px;padding-bottom: 10px;font-size:16px;">
							<b>
								<CENTER><strong>VIEW MAPPING FOR ZONE DASHBOARD DETAILS</strong></CENTER>
							</b>
						</caption>
						<thead>
						...
						<tbody>
							...
							<tr id="{'trid'|cat:$i}">
								...
								<td style="text-align: center;">{$row.ward_desc}</td>
								<!-- 20-06-2024 <td class="noExport" style="text-align: center;"> -->
								<!--<input type='radio' name='update' onchange="fill('{$desg_id}','{$row.ward_id}','{$row.desg_desc}')"> -->
								<!-- 20-06-2024 <input type="button" class="btn btn-primary" id="{'id11'|cat:$i}" onclick="fill('{$row.emp_id}','{$row.ward_id}')" value="Edit"> -->
								<!-- <button class="btn btn-success" name='update' onclick="fill('{$id}{$emp_id}','{$ward_id}')">
										<span class="fa fa-pencil"></span> Edit
									</button> -->
								<!-- 20-06-2024 </td>-->
								<td class="noExport" style="text-align: center;">
									<!-- <input type='radio' name='delete_hod' onchange="delete_hod('{$i}','{$row.emp_id}','{$row.ward_id}')"> Delete -->
									<span class="btn btn-danger" onclick="delete_hod('{$i}','{$row.emp_id}','{$row.ward_id}')" title="Delete">Delete</span>
								</td>
							</tr>
