...
<div class="" style="margin-top: -45px;">
	...
	<div class="boxed">
		<div class="title-bar blue">
			{if $status eq '109'}
			<h4>COMPLAINTS < 7 DAYS REPORT</h4>
					{else}
					<h4>COMPLAINTS > 30 DAYS REPORT</h4>
					{/if}
		</div>
		<div class="inner no-radius">
			...
			<form method="POST" class="form-horizontal">
				...
				<div class="button-container">
					...
					<a href="corp_grievance_month_report.php" class="btn btn-warning" onclick="history.go(-1);">
						<i class="fa fa-backward"></i> Back
					</a>
				</div>
			</form>
			<!--18-06-2024 <div style="text-align:right;"><a href="grievance_month_report.php" class="btn btn-warning" onclick="history.go(-1);"><i class="fa fa-backward"></i> Back</a></div> -->
						...
						<table width="100%" border="1" cellspacing="0" cellpadding="0" class="display table-bordered table-striped table-condensed cf" id="data-table">
							<caption style="caption-side:top; text-align:center;font-size:16px;">
								<b>
									...
									{if $status eq '109' }
									<CENTER><strong>VIEW COMPLAINTS < 7 DAYS REPORT DETAILS</strong>
									</CENTER>
									{else}
									<CENTER><strong>VIEW COMPLAINTS > 30 DAYS REPORT DETAILS</strong></CENTER>
									{/if}
								</b>
							</caption>
							<thead>
...
<nav>
    <ul class="pagination">
        ...
        {if $pagination.current_page > 1}
            <li><a href="?status={$status}&page={$pagination.current_page - 1}">&laquo; Prev</a></li>
        {/if}

        <!-- First Page Button -->
        {if $start_page > 1}
            <li><a href="?status={$status}&page=1">1</a></li>
            {if $start_page > 2}
        ...
        {section name=pages start=$start_page loop=$end_page}
            <li class="{if $pagination.current_page == $smarty.section.pages.index+1}active{/if}">
                <a href="?status={$status}&page={$smarty.section.pages.index+1}">{$smarty.section.pages.index+1}</a>
            </li>
        {/section}
        <!-- Last Page Button -->
        {if $end_page < $pagination.total_pages}
            {if $end_page < $pagination.total_pages - 1}
                <li class="disabled"><a href="#">...</a></li>
            {/if}
            <li><a href="?status={$status}&page={$pagination.total_pages}">{$pagination.total_pages}</a></li>
        {/if}

        <!-- Next Button -->
        {if $pagination.current_page < $pagination.total_pages}
            <li><a href="?status={$status}&page={$pagination.current_page + 1}">Next &raquo;</a></li>
        {/if}
    </ul>
</nav>
<!-- option2 pagination end -->
	...
	<div style="width:100%; text-align:center">
		<form action="corp_exporttoexcel.php" method="post">
			...
			<button type='button' class="btn btn-primary" id="download-pdf" onclick="exportTableToPDF('data-table', 'GRIEVANCEMONTHINNERREPORT')" value="Export To PDF"></i> Export To PDF</button>
