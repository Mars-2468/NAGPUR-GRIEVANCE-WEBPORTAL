...
<table   id='example' width="100%" height="100%" border="0" cellspacing="0" cellpadding="0" >
 <tr>
    ...
    <td align="left" valign="top">
		...
		<th align="center" valign="middle">Total No</th>
		
	    </tr>
	    </thead>
		<tbody>
		{foreach from=$data key=dept_id item=row}
		<tr>
			<td>{counter}</td>
            ...
            {else}
            <td ><label title="{$row.comp_desc}">{$dept_list[$dept_id]}</label></td>
            {/if}
			
			<td align="center"><a href="emp_in_services.php?aptid=2&status={$status}&user_type={$user_type}&dept_id={$dept_id}&sla={$sla}">{$row.count}</a></td>
