...
<div class="" style="margin-top: -45px;">
	<div class="boxed">
		...
		<div class="inner no-radius">
			<form method="POST" class="form-horizontal">
				...
				<div class="button-container">
					...
					<a href="dept_empwise.php?app_type_id=1&dept_id={$dept_id_sel}&status=0" class="btn btn-warning" onclick="history.go(-1);">
			...
			<!--15-06-2024 <div class="col-md-1" align="right"><input name="search" type="submit" class="btn btn-success" value="SEARCH"> </div>

			</form>
				{if $user_type eq 'E'}
					/*07-03-24 <div style="text-align:right;"><a href="ajax_dashboard_dept.php" class="btn btn-warning"><i class="fa fa-backward"></i> Back</a></div>*/
		...
		<div id="div_print" style="border:#999999 0px solid; margin-top:2%">
				...
				<div id="area" class="table-responsive">
					<table width="100%" border="1" cellspacing="0" cellpadding="0" class="display table-bordered table-striped table-condensed cf" id="data-table">
						<caption style="caption-side:top; text-align:center;font-size:16px;"> 
							<b><CENTER><strong> Mr.<span class="text-primary">{strtoupper($emp_list[$emp_id])}</span> (<span class="text-warning">{strtoupper($dept_list[$dept_id])} EMPLOYEE</span>)
								{if $status eq '0' && $user_type eq 'U' }
								{if $app_type_id eq '1'}  TOTAL RECEIVED COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>
								{elseif $status eq '0' && $user_type eq 'E' }
								 VIEW {if $app_type_id eq '1'} EMPLOYEE RECEIVED COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

								{elseif $status eq '2' && $user_type eq 'U' }
								 VIEW {if $app_type_id eq '1'} ADMIN UNDER PROGRESS WITHIN SLA COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>
								{elseif $status eq '2' && $user_type eq 'E' }
								 VIEW {if $app_type_id eq '1'} EMPLOYEE UNDER PROGRESS WITHIN SLA COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

								{elseif $status eq '8' && $user_type eq 'U' }
								 VIEW {if $app_type_id eq '1'} ADMIN UNDER PROGRESS BEYOND SLA COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>
								{elseif $status eq '8' && $user_type eq 'E' }
								 VIEW {if $app_type_id eq '1'} EMPLOYEE UNDER PROGRESS BEYOND SLA COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

								{elseif $status eq '3' && $user_type eq 'U' }
								 VIEW {if $app_type_id eq '1'} ADMIN COMPLETED WITHIN SLA COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>
								{elseif $status eq '3' && $user_type eq 'E' }
								 VIEW {if $app_type_id eq '1'} EMPLOYEE COMPLETED WITHIN SLA COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

								{elseif $status eq '9' && $user_type eq 'U' }
								 VIEW {if $app_type_id eq '1'} ADMIN COMPLETED BEYOND SLA COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>
								{elseif $status eq '9' && $user_type eq 'E' }
								 VIEW {if $app_type_id eq '1'} EMPLOYEE COMPLETED BEYOND SLA COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

								{elseif $status eq '5' && $user_type eq 'U' }
								 VIEW {if $app_type_id eq '1'} ADMIN REOPENED COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>
								{elseif $status eq '5' && $user_type eq 'E' }
								 VIEW {if $app_type_id eq '1'} EMPLOYEE REOPENED COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

								{elseif $status eq '105' && $user_type eq 'U' }
								 VIEW {if $app_type_id eq '1'} ADMIN REOPENED UNDER PROGRESS COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>
								{elseif $status eq '105' && $user_type eq 'E' }
								 VIEW {if $app_type_id eq '1'} EMPLOYEE REOPENED UNDER PROGRESS COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

								{elseif $status eq '7' && $user_type eq 'U' }
								 VIEW {if $app_type_id eq '1'} ADMIN REOPENED COMPLETED COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>
								{elseif $status eq '7' && $user_type eq 'E' }
								 VIEW {if $app_type_id eq '1'} EMPLOYEE REOPENED COMPLETED COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

								{elseif $status eq '10' && $user_type eq 'U' }
								 VIEW {if $app_type_id eq '1'} ADMIN FINANCIAL IMPLICATIONS COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>
								{elseif $status eq '10' && $user_type eq 'E' }
								 VIEW {if $app_type_id eq '1'} EMPLOYEE FINANCIAL IMPLICATIONS COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

								{else}
								VIEW {if $app_type_id eq '1'} COMPLAINT {else} SERVICE{/if} DETAILS</strong></CENTER>
								{/if}
							</b>
						</caption>
						<thead>
						...
						<tbody>
							...
							<tr>
								...
								<!--29-05-2024 <td align='center' class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}"><a href="view_comp_det_admin.php?grievance_id={$grievance_id}">{$grievance_id}</a></td> -->
								<td align='center' class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}">
									<a href="view_comp_det_admin.php?grievance_id={$grievance_id}&app_type_id=1&emp_id={$emp_id}&status={$status}&dept_id={$dept_id}&f_date={$f_date}&t_date={$t_date}" target="_blank">
										...
										<b style="margin-right: 1px;">QR - </b>{$grievance_id}
										{else}
										{$grievance_id}
										{/if}
									</a>
