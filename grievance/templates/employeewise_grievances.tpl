					...
					<table width="100%" border="1" cellspacing="0" cellpadding="0" class="display table-bordered table-striped table-condensed cf" id="example">
						<caption style="caption-side:top; text-align:center;font-size:16px;">
							<b>
								...
								<CENTER><strong>{if $app_type_id eq '1'} TOTAL RECEIVED COMPLAINT {else} SERVICE {/if} DETAILS  {if $user_type eq 'U'} OF <span class="text-primary"> {$emp_list[$emp_id]}</span> EMPLOYEE {/if}</strong></CENTER>
								{elseif $status eq '100' && $user_type eq 'E' }
								<CENTER><strong>{if $app_type_id eq '1'} TOTAL RECEIVED COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

								{elseif ($status eq '200' || $status eq '201') && $user_type eq 'U' }
								<CENTER><strong> {if $app_type_id eq '1'} TOTAL UNDER PROGRESS WITHIN SLA COMPLAINT {else} SERVICE {/if} DETAILS {if $user_type eq 'U'} OF <span class="text-primary"> {$emp_list[$emp_id]}</span> EMPLOYEE {/if}</strong></CENTER>
								{elseif $status eq '200' && $user_type eq 'E' }
								<CENTER><strong> {if $app_type_id eq '1'} TOTAL UNDER PROGRESS BEYOND SLA COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

								{elseif ($status eq '300' || $status eq '301') && $user_type eq 'U' }
								<CENTER><strong> {if $app_type_id eq '1'} TOTAL COMPLETED WITHIN SLA COMPLAINT {else} SERVICE {/if} DETAILS {if $user_type eq 'U'} OF <span class="text-primary"> {$emp_list[$emp_id]}</span> EMPLOYEE {/if}</strong></CENTER>
								{elseif $status eq '300' && $user_type eq 'E' }
								<CENTER><strong> {if $app_type_id eq '1'} TOTAL COMPLETED BEYOND SLA COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

								{elseif ($status eq '400' || $status eq '401' || $status eq '402') && $user_type eq 'U' }
								<CENTER><strong> {if $app_type_id eq '1'} ADMIN REOPENED COMPLAINT {else} SERVICE {/if} DETAILS {if $user_type eq 'U'} OF <span class="text-primary"> {$emp_list[$emp_id]}</span> EMPLOYEE {/if}</strong></CENTER>
								
								{elseif ($status eq '400' || $status eq '401' || $status eq '402') && $user_type eq 'E' }
								<CENTER><strong> {if $app_type_id eq '1'} TOTAL REOPENED COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>
																
								{elseif $status eq '500' && $user_type eq 'U' }
								<CENTER><strong> {if $app_type_id eq '1'} TOTAL TRANSFERED COMPLAINT {else} SERVICE {/if} DETAILS {if $user_type eq 'U'} OF <span class="text-primary"> {$emp_list[$emp_id]}</span> EMPLOYEE {/if}</strong></CENTER>
								{elseif $status eq '500' && $user_type eq 'E' }
								<CENTER><strong> {if $app_type_id eq '1'} TOTAL TRANSFERED COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

								{elseif $status eq '600' && $user_type eq 'U' }
								<CENTER><strong> {if $app_type_id eq '1'} TOTAL FINANCIAL IMPLICATIONS COMPLAINT {else} SERVICE {/if} DETAILS {if $user_type eq 'U'} OF <span class="text-primary"> {$emp_list[$emp_id]}</span> EMPLOYEE {/if}</strong></CENTER>
								{elseif $status eq '600' && $user_type eq 'E' }
								<CENTER><strong> {if $app_type_id eq '1'} TOTAL FINANCIAL IMPLICATIONS COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>
								...
								{else}
								<CENTER><strong> {if $app_type_id eq '1'} COMPLAINT {else} SERVICE{/if} DETAILS</strong></CENTER>
								{/if}
...
<nav>
    <ul class="pagination">
        ...
        {if $pagination.current_page > 1}
            <li><a href="?ulbid={$ulbid}&emp_id={$emp_id}&dept_id={$dept_id}&app_type_id={$app_type_id}&status={$status}&page={$pagination.current_page - 1}">&laquo; Prev</a></li>
        {/if}

        <!-- First Page Button -->
        {if $start_page > 1}
            <li><a href="?ulbid={$ulbid}&emp_id={$emp_id}&dept_id={$dept_id}&app_type_id={$app_type_id}&status={$status}&page=1">1</a></li>
            {if $start_page > 2}
        ...
        {section name=pages start=$start_page loop=$end_page}
            <li class="{if $pagination.current_page == $smarty.section.pages.index+1}active{/if}">
                <a href="?ulbid={$ulbid}&emp_id={$emp_id}&dept_id={$dept_id}&app_type_id={$app_type_id}&status={$status}&page={$smarty.section.pages.index+1}">{$smarty.section.pages.index+1}</a>
            </li>
        {/section}
        <!-- Last Page Button -->
        {if $end_page < $pagination.total_pages}
            {if $end_page < $pagination.total_pages - 1}
                <li class="disabled"><a href="#">...</a></li>
            {/if}
            <li><a href="?ulbid={$ulbid}&emp_id={$emp_id}&dept_id={$dept_id}&app_type_id={$app_type_id}&status={$status}&page={$pagination.total_pages}">{$pagination.total_pages}</a></li>
        {/if}

        <!-- Next Button -->
        {if $pagination.current_page < $pagination.total_pages}
            <li><a href="?ulbid={$ulbid}&emp_id={$emp_id}&dept_id={$dept_id}&app_type_id={$app_type_id}&status={$status}&page={$pagination.current_page + 1}">Next &raquo;</a></li>
        {/if}
    </ul>
</nav>
