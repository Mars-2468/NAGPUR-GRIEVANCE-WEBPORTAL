				...
				<div id="area">
					<table id='example' cellpadding="3" cellspacing="0" border="1" class="display table-bordered table-striped table-condensed cf" width="100%">
						<caption style="caption-side:top; text-align:center;font-size:16px;">
							<b>
								<CENTER><strong>VIEW SHOW CAUSE NOTICE LIST DETAILS</strong></CENTER>
							</b>
						</caption>
						<thead>
							<tr style="background-color:#2c3e50; color:#FFF;">
								...
								<th style='text-align:center !important'>SHOW CAUSE COUNT(S) </th>
							</tr>
						</thead>
						<tbody>
						...
						{if $emp_id ne ''}
							<tr>
								...
								<td align='center1' class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}">{if $data[$emp_id].count neq ''}<a href="corp_all_showcause_notice_list.php?ulbid={$ulbid}&app_type_id={$app_type_id}&emp_id={$emp_id}&status=show&dept_id={$emp_dept_list[$emp_id]}&f_date={$fdate}&t_date={$tdate}" style="text-decoration: none;">{$emp_list[$emp_id]}</a>{else}<b>{$emp_list[$emp_id]}</b>{/if}</td>
								<td align='center1'><b>{$dept_list[$emp_dept_list[$emp_id]]}</b></td>
								<td align='center' class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}">
									{if $data[$emp_id].count neq ''}
										<a href="corp_all_showcause_notice_list.php?app_type_id={$app_type_id}&emp_id={$emp_id}&status=show&dept_id={$emp_dept_list[$emp_id]}&f_date={$fdate}&t_date={$tdate}" target="" style="text-decoration: none;">
											{$data[$emp_id].count}
										</a>
									{else}
										<b>-</b>
						...
						<tfoot>
							...
							{if $user_type eq 'U'}
								<td align='center' class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}">{if $tot.received neq ''}<a href="corp_showcauseTotalNotices.php?ulbid={$ulbid}&app_type_id={$app_type_id}&status=show_all&f_date={$fdate}&t_date={$tdate}" style="text-decoration: none;"><b>{$tot.received}</b></a>{else}<b>0</b>{/if}</td>
							{else}
