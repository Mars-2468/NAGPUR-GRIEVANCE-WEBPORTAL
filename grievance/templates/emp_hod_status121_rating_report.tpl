...
<div class="boxed">
	<div class="title-bar blue">
		<h4>Details of Employee Rating Report</h4>
					...
					<table id='example' cellpadding="3" cellspacing="0" border="1" class="display table-bordered table-striped table-condensed cf" width="100%">
						...
						<thead>
							<tr style="background-color:#2c3e50;">
								...
								<th  class="text-center" align='left'  style='vertical-align:middle' ><font color='white'>Total Employees</font></th>
							</tr>							
						</thead>
						<tbody>
							{foreach from=$data key=rating_no item=row}
							<tr>
								<td class="text-center" >{counter}</td>
								<td class="text-center" >
								<div id="eform-id-{$rating_no}" class="d-flex justify-content-start align-items-center" name="eform_name{$rating_no}">
											<div>({$row.rating_no})</div>
											<div class="erating">
												<span>	
													<input type="radio" class="erating-input" id="erating-input-{$rating_no}-10" value="10"  {if $data[$rating_no]['rating_no']==10} checked {/if} onclick="return false;">
													<label for="erating-input-{$rating_no}-10" class="erating-star" title="10"></label>			
													<input type="radio" class="erating-input" id="erating-input-{$rating_no}-9" value="9"  {if $data[$rating_no]['rating_no']==9} checked {/if} onclick="return false;">
													<label for="erating-input-{$rating_no}-9" class="erating-star" title="9"></label>			
													<input type="radio" class="erating-input" id="erating-input-{$rating_no}-8" value="8"  {if $data[$rating_no]['rating_no']==8} checked {/if} onclick="return false;">
													<label for="erating-input-{$rating_no}-8" class="erating-star" title="8"></label>			
													<input type="radio" class="erating-input" id="erating-input-{$rating_no}-7" value="7"  {if $data[$rating_no]['rating_no']==7} checked {/if} onclick="return false;">
													<label for="erating-input-{$rating_no}-7" class="erating-star" title="7"></label>			
													<input type="radio" class="erating-input" id="erating-input-{$rating_no}-6" value="6"  {if $data[$rating_no]['rating_no']==6} checked {/if} onclick="return false;">
													<label for="erating-input-{$rating_no}-6" class="erating-star" title="6"></label>			
													
													<input type="radio" class="erating-input" id="erating-input-{$rating_no}-5" value="5"  {if $data[$rating_no]['rating_no']==5} checked {/if} onclick="return false;">
													<label for="erating-input-{$rating_no}-5" class="erating-star" title="5"></label>			
													<input type="radio" class="erating-input" id="erating-input-{$rating_no}-4" value="4"  {if $data[$rating_no]['rating_no']==4} checked {/if} onclick="return false;">
													<label for="erating-input-{$rating_no}-4" class="erating-star" title="4"></label>
													<input type="radio" class="erating-input" id="erating-input-{$rating_no}-3" value="3"  {if $data[$rating_no]['rating_no']==3} checked {/if} onclick="return false;">
													<label for="erating-input-{$rating_no}-3" class="erating-star" title="3"></label>
													<input type="radio" class="erating-input" id="erating-input-{$rating_no}-2" value="2"  {if $data[$rating_no]['rating_no']==2} checked {/if} onclick="return false;" >
													<label for="erating-input-{$rating_no}-2" class="erating-star" title="2" readonly></label>
													<input type="radio" class="erating-input" id="erating-input-{$rating_no}-1" value="1"  {if $data[$rating_no]['rating_no']==1} checked {/if} onclick="return false;" >
													<label for="erating-input-{$rating_no}-1" class="erating-star" title="1" readonly></label>
												</span>	
											</div>
										</div>
								
								</td>	
								<td><a href="emp_hod_status121_rating_list_report.php?rating_no={$rating_no}&department_id={$department_id}&f_date={$fdate}&t_date={$tdate}">{$row.empcnt}</a></td>									
							</tr>
						
							{/foreach}
						</tbody>
						<tfoot>
							<td align='center' colspan="2"><strong>Total Grievances</strong></td>
							<td align='left'><strong>{$total_employees}</strong></td>									
						</tfoot>
					</table>
					</div></div>
				</div>
