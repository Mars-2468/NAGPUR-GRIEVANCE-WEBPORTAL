				...
				<table class="display table-bordered table-striped table-condensed cf" id="data-table">
					...
					<tbody>
						...
						<tr align="center">
							...
							<td valign="middle">{if $data[$ulbid].count neq ''}<a href="cat_origin.php?ulbid={$ulbid}&app_type_id={$app_type_id}&originid={$origin_id}" target="_blank">{$data[$ulbid].count} </a>{else}-{/if}</td>

							<!--15-06-2024 <td><a href="cat_origin.php?ulbid={$ulbid}&originid={$origin_id}" target="_blank">{$data[$ulbid].count}</a></td> -->
						</tr>
						{/foreach}
					</tbody>
					<tfoot>
