...
<div id="div_print" class="cap_font" style="border:#999999 0px solid;">
	...
	<div class="tab-content">
		...
		<div class="tab-pane active table-responsive">
			...
			<table width="97%" border="1" cellspacing="0" cellpadding="0" class="display table-bordered table-striped table-condensed cf" id="data-table">
				...
				<tr>
					...
					<!--15-06-2024 <td align="center" valign="middle"><b>{$data2.pendingforapproval}</b></td>
					<td align="center" valign="middle"><b>{$data2.pending}</b></td>
					<td align="center" valign="middle"><b>{$data2.completed}</b></td>
					<td align="center" valign="middle"><b>{$data2.total}</b></td>-->

					<!--24-06-2024 <td align="center" class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}" valign="middle"><strong>{if $data2.pendingforapproval eq ''} 0 {else}{$data2.pendingforapproval}{/if}</strong></td>
					<td align="center" class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}" valign="middle"><strong>{if $data2.pending eq ''} 0 {else}{$data2.pending}{/if}</strong></td>
					<td align="center" class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}" valign="middle"><strong>{if $data2.completed eq ''} 0 {else}{$data2.completed}{/if}</strong></td>
					<td align="center" valign="middle"><strong>{if $data2.total eq ''} 0 {else}{$data2.total}{/if}</strong></td> -->
					
					<td align="center" class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}" valign="middle"><strong>{if $data2.pendingforapproval eq ''} 0 {else}<a href="Totalorigin.php?ulbid={$ulbid}&originid={$origin_id}&status=100" target="_blank">{$data2.pendingforapproval}</a>{/if}</strong></td>
					<td align="center" class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}" valign="middle"><strong>{if $data2.pending eq ''} 0 {else}<a href="Totalorigin.php?ulbid={$ulbid}&originid={$origin_id}&status=200" target="_blank">{$data2.pending} </a>{/if}</strong></td>
					<td align="center" class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}" valign="middle"><strong>{if $data2.completed eq ''} 0 {else}<a href="Totalorigin.php?ulbid={$ulbid}&originid={$origin_id}&status=300" target="_blank">{$data2.completed} </a>{/if}</strong></td>
					<td align="center" class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}" valign="middle"><strong>{if $data2.total eq ''} 0 {else}<a href="Totalorigin.php?ulbid={$ulbid}&originid={$origin_id}&status=400" target="_blank">{$data2.total} </a>{/if}</strong></td>
				</tr>
		...
		<tr>
		    ...
		    <td><a href="receive_print.php?gid={$grievance_id}&aptid={$row.app_type_id}" target="_blank">Receipt</a></td>
		    <td>
