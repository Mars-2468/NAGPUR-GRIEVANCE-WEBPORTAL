...
<div>
  ...
  <div class="boxed">
    ...
    <div class="inner no-radius">
      ...
      <!--18-06-2024 <div style="text-align:right;"><a href="reports.php" class="btn btn-warning" onclick="history.go(-1);"><i class="fa fa-backward"></i> Back</a></div>-->
        ...
        <div id="area">
          <table id='example' cellpadding="3" cellspacing="0" border="1" class="display table-bordered table-striped table-condensed cf" width="100%">
			...
			{foreach from=$emp_details_list key=emp_id item=row}
			  ...
			  <td><a class="text-primary" href="employeewise_grievances.php?ulbid={$ulbid}&emp_id={$emp_id}&dept_id={$emp_dept_list[$emp_id]}&app_type_id={$app_type_id}&f_date={$fdate}&t_date={$tdate}&status=100">{$row['emp_name']}</a></td>
			  <td>{$row['dept_name']}-{$emp_dept_list[$emp_id]}</td>
			  <td>{$row['zone_ids']}</td>
			  <td>{$row['street_ids']}</td>
			  <td><a class="text-primary" href="employeewise_grievances.php?ulbid={$ulbid}&emp_id={$emp_id}&dept_id={$emp_dept_list[$emp_id]}&app_type_id={$app_type_id}&f_date={$fdate}&t_date={$tdate}&status=100">{$emp_details_list[$emp_id]['received']}</a></td>
