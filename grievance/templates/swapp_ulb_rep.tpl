...
<div class="boxed">
    ...
    <div class="title-bar blue"><h4>Swachhta statistics for ULBs</h4></div>
 ...
 <tr>
    ...
    <td align="left" valign="top">
		...
		<th align="center" valign="middle">ULBName</th>
		...
		<tr>
			<td>{counter}</td>
			<td><a href="swapp_dashboard.php?ulbid={$ulbid}" target="_blank">{$ulb_list[$ulbid]}</a></td>
			<td>{$received_ulb[$ulbid].count}</td>
			<td>{$received[$ulbid].count}</td>
			<td>{$pending[$ulbid].count}</td>
			<td>{$resolved[$ulbid].count}</td>
			<td>{$rejected[$ulbid].count}</td>
