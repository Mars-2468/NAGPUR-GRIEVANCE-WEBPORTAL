...
<div class="row">
	<div class="boxed">
		<div id="area">
		<div class="title-bar1 white d-flex bg-info justify-content-between align-items-center p-2">
			<h4>USER LOG DETAILS</h4>	
			<a href="all_users_list.php" class="btn btn-warning" >Back</a>	
		</div>
			...
			<table id="data-table" class="table table-striped table-bordered table-hover table-full-width"  width="100%">
					...
					<tbody>
						...
						<tr>
							...
							<td style="">{$row.login_time}</td>
							<td style="">{$row.logout_time|default:'-'}</td>
							
							{if $row.user_log_status eq 'Loggedin'}
								<td style="text-align: center;background-color:#00FF00;">IN</td> 
							{else} 
								<td style="text-align: center;background-color:#FF0000;color:#FFF;">OUT</td> 
							{/if}							
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js"></script> 
 ...
 <script>
    ...
    let worksheet = workbook.addWorksheet("User log details");

    // Define header row with custom styles
    let headerRow = worksheet.addRow(["SR.No", "USER-ID", "LOGIN-TIME", "LOGOUT-TIE","LOG-STATUS"]);
...
<div align='center'>
  ...
  <button type='button' class="btn btn-primary" id="download-pdf" onclick="exportTableToPDF('data-table', 'Users log details')" value="Export To PDF"></i> Export To PDF</button> 
