			...
			if (mysqli_query($conn, $sql)) {
				...
				if (mysqli_num_rows($rs1) > 0) {
					...
					if (mysqli_query($conn, $sql2)) {
						...
						//04-07-2024 $sms = "Dear " . substr($emp_name_list[$row1['emp_id']], 0, 28) . ", A Grievance from " . mysqli_real_escape_string($conn, strip_tags(substr($_REQUEST['person_name'], 0, 28))) . ", Mobile No. " . mysqli_real_escape_string($conn, strip_tags($_REQUEST['mobile'])) . ", " . substr($complaintType, 0, 30) . " with Ref No " . $grievance_id_sel . " is allotted to you on " . date('d-m-Y H:i:s', strtotime($_POST['alloted_date'])) . " https://nmcnagpur.gov.in/grievance/ Regards- CitizenServiceMonitoringCell ,NMCGOV";
