...
<div class="" style="margin-top: -45px;">
	...
	<div class="boxed">
		<div class="title-bar blue">
			<h4>Designation Complaints Report</h4>
		</div>
		<div class="inner no-radius">
			...
			<form method="POST" class="form-horizontal">
				...
				{if $user_type eq 'E'}
				<div style="text-align:right;"><a href="desgnwise_abstract.php?app_type_id={$app_type_id}&dept_id={$dept_id}&status={$status}&f_date={$fdate}&t_date={$tdate}" class="btn btn-warning"><i class="fa fa-backward"></i> Back</a></div>
				{else}
				<div style="text-align:right;"><a href="desgnwise_abstract.php?app_type_id={$app_type_id}&dept_id={$dept_id}&status={$status}&f_date={$fdate}&t_date={$tdate}" class="btn btn-warning" onclick="history.go(-1);"><i class="fa fa-backward"></i> Back</a></div>
				{/if} -->

				{if $user_type eq 'E'}
				<div class="button-container">
					<input name="search" type="submit" class="btn btn-success" value="SEARCH">
					<a href="desgnwise_abstract.php?app_type_id={$app_type_id}&dept_id={$dept_id}&status={$status}&f_date={$fdate}&t_date={$tdate}" class="btn btn-warning" onclick="history.go(-1);">
					...
					<table width="100%" border="1" cellspacing="0" cellpadding="0" class="display table-bordered table-striped table-condensed cf" id="data-table">
						<caption style="caption-side:top; text-align:center;font-size:16px;">
							<b>
								...
								{elseif $status eq '9' || $status eq '500' || $status eq '501' && $user_type eq 'U' }
								<CENTER><strong>VIEW {if $app_type_id eq '1'} ADMIN COMPLETED BEYOND SLA COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>
								{elseif $status eq '9' || $status eq '500' || $status eq '501' && $user_type eq 'E' }
						...
						<thead>
							...
							<!--18-06-2024 <tr style="background-color:#2c3e50; color:#FFF; text-align:center;">
								...
								<!--Not Open <th align='center' valign='middle' colspan='2'>ACTIONS</th> -->
						...
						<tbody>
							...
							<tr>
								...
								<!--18-06-2024 <td align='center' class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}"><a href="view_comp_det_admin.php?grievance_id={$grievance_id}">{$grievance_id}</a></td> -->
