...
<div class="" style="margin-top: -45px;">
	...
	<div class="boxed">
		<div class="title-bar blue">
			{if $status eq 'all'}
			<h4>NO. OF COMPLAINTS REGISTERED REPORT</h4>
			{elseif $status eq 'L1'}
			<h4>LEVEL 1 COMPLAINTS REPORT</h4>
			{elseif $status eq 'L2'}
			<h4>LEVEL 2 COMPLAINTS REPORT</h4>
			{elseif $status eq 'L3'}
			<h4>LEVEL 3 COMPLAINTS REPORT</h4>
			{elseif $status eq 'L4'}
			<h4>LEVEL 4 COMPLAINTS REPORT</h4>
			{else}
		...
		<div class="inner no-radius">
			...
			<form method="POST" class="form-horizontal">
				...
				{if $status eq 'all' || $status eq 'L1' || $status eq 'L2' || $status eq 'L3' || $status eq 'L4'}
				<div style="text-align:right;"><a href="corp_grievance_sla_anlysis_report.php" class="btn btn-warning" onclick="history.go(-1);"><i class="fa fa-backward"></i> Back</a></div>
				{else}
				<div style="text-align:right;"><a href="corp_dept_pendancy_report.php" class="btn btn-warning" onclick="history.go(-1);"><i class="fa fa-backward"></i> Back</a></div>
				{/if} -->

				{if $status eq 'all' || $status eq 'L1' || $status eq 'L2' || $status eq 'L3' || $status eq 'L4'}
				<div class="button-container">
					<input name="search" type="submit" class="btn btn-success" value="SEARCH">
					<a href="corp_grievance_sla_anlysis_report.php" class="btn btn-warning" onclick="history.go(-1);">
				...
				<div class="button-container">
					<input name="search" type="submit" class="btn btn-success" value="SEARCH">
					<a href="corp_dept_pendancy_report.php" class="btn btn-warning" onclick="history.go(-1);">
						<i class="fa fa-backward"></i> Back
					</a>
				</div>
				{/if}
				...
				<div id="area">
					<table width="100%" border="1" cellspacing="0" cellpadding="0" class="display table-bordered table-striped table-condensed cf" id="data-table">
						<caption style="caption-side:top; text-align:center;font-size:16px;">
							<b>
								<!-- <CENTER><strong>VIEW COMPLAINTS SLA ANALYSIS REPORT DETAILS</strong></CENTER> -->
								...
								{elseif $status eq '107' || $status eq '800' && $user_type eq 'P' }
								<CENTER><strong>VIEW {if $app_type_id eq '1'} ADMIN TOTAL PENDING COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>
								{elseif $status eq '107' || $status eq '800' && $user_type eq 'E' }
								<CENTER><strong>VIEW {if $app_type_id eq '1'} EMPLOYEE TOTAL PENDING COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>

								{elseif $status eq '108' || $status eq '900' && $user_type eq 'P' }
								<CENTER><strong>VIEW {if $app_type_id eq '1'} ADMIN TOTAL RESOLVED COMPLAINT {else} SERVICE {/if} DETAILS</strong></CENTER>
								{elseif $status eq '108' || $status eq '900' && $user_type eq 'E' }
								...
								<CENTER><strong>NO. OF COMPLAINTS REGISTERED DETAILS</strong></CENTER>

								{elseif $status eq 'L1'}
								<CENTER><strong>LEVEL 1 COMPLAINTS DETAILS</strong></CENTER>
								{elseif $status eq 'L2'}
								<CENTER><strong>LEVEL 2 COMPLAINTS DETAILS</strong></CENTER>
								{elseif $status eq 'L3'}
								<CENTER><strong>LEVEL 3 COMPLAINTS DETAILS</strong></CENTER>
								{elseif $status eq 'L4'}
								<CENTER><strong>LEVEL 4 COMPLAINTS DETAILS</strong></CENTER>
						...
						<thead>
							...
							<!--30-05-2024 <tr style="background-color:#2c3e50; color:#FFF;text-align:center;">
						...
						<tbody>
							...
							<tr>
								...
								<td align='center' class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}">
									...
									<form action="corp_manage_comp_sel.php" method="post">
...
<nav>
    <ul class="pagination">
        ...
        {if $pagination.current_page > 1}
            <li><a href="?app_type_id={$app_type_id}&dept_id={$dept_id}&status={$status}&page={$pagination.current_page - 1}">&laquo; Prev</a></li>
        {/if}

        <!-- First Page Button -->
        {if $start_page > 1}
            <li><a href="?app_type_id={$app_type_id}&dept_id={$dept_id}&status={$status}&page=1">1</a></li>
            {if $start_page > 2}
        ...
        {section name=pages start=$start_page loop=$end_page}
            <li class="{if $pagination.current_page == $smarty.section.pages.index+1}active{/if}">
                <a href="?app_type_id={$app_type_id}&dept_id={$dept_id}&status={$status}&page={$smarty.section.pages.index+1}">{$smarty.section.pages.index+1}</a>
            </li>
        {/section}
        <!-- Last Page Button -->
        {if $end_page < $pagination.total_pages}
            {if $end_page < $pagination.total_pages - 1}
                <li class="disabled"><a href="#">...</a></li>
            {/if}
            <li><a href="?app_type_id={$app_type_id}&dept_id={$dept_id}&status={$status}&page={$pagination.total_pages}">{$pagination.total_pages}</a></li>
        {/if}

        <!-- Next Button -->
        {if $pagination.current_page < $pagination.total_pages}
            <li><a href="?app_type_id={$app_type_id}&dept_id={$dept_id}&status={$status}&page={$pagination.current_page + 1}">Next &raquo;</a></li>
        {/if}
    </ul>
</nav>
