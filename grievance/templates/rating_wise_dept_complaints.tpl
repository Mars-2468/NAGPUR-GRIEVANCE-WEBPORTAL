	...
	<form method="POST" action="" class="form-horizontal">
		<div class="boxed">
			...
			<div class="inner no-radius" style="border-bottom-left-radius: 0px !important;border-bottom-right-radius: 0px !important;">
				<div class="col-md-3" style="margin-right:15px;" hidden>
...
<div class="boxed">
	<div class="title-bar blue d-flex justify-content-between align-items-center">
		...
		<p class="m-0"><a href="complaints_rating.php" class="btn btn-warning"><i class="fa fa-backward"></i> Back</a></p>
	</div>
								...
								{foreach from=$data key=grievance_id item=row}
									{if $row.dept_id!=0}
										...
										<tr>
											...
											<!-- <td>Rating</td> -->
											<td class="text-center">{if !empty($row.previous) } <img src="{$row.previous}" alt='Image' style='width:100px;height:100px;'> {else} {'NA'} {/if}</td>
											<td class="text-center">{if !empty($row.after1) } <img src="{$row.after1}" alt='Image' style='width:100px;height:100px;'>  {else} {'NA'} {/if}</td>
...
<nav>
    <ul class="pagination">
        ...
        {if $pagination.current_page > 1}
            <li><a href="?app_type_id=1&rating_num={$rating_num}&cs_id={$cs_id}&department_id={$department_id}&page={$pagination.current_page - 1}">&laquo; Prev</a></li>
        {/if}

        <!-- First Page Button -->
        {if $start_page > 1}
            <li><a href="?app_type_id=1&rating_num={$rating_num}&cs_id={$cs_id}&department_id={$department_id}&page=1">1</a></li>
            {if $start_page > 2}
        ...
        {section name=pages start=$start_page loop=$end_page}
            <li class="{if $pagination.current_page == $smarty.section.pages.index+1}active{/if}">
                <a href="?app_type_id=1&rating_num={$rating_num}&cs_id={$cs_id}&department_id={$department_id}&page={$smarty.section.pages.index+1}">{$smarty.section.pages.index+1}</a>
            </li>
        {/section}
        ...
        {if $end_page < $pagination.total_pages}
            {if $end_page < $pagination.total_pages - 1}
                <li class="disabled"><a href="#">...</a></li>
            {/if}
            <li><a href="?app_type_id=1&rating_num={$rating_num}&cs_id={$cs_id}&department_id={$department_id}&page={$pagination.total_pages}">{$pagination.total_pages}</a></li>
        {/if}

        <!-- Next Button -->
        {if $pagination.current_page < $pagination.total_pages}
            <li><a href="?app_type_id=1&rating_num={$rating_num}&cs_id={$cs_id}&department_id={$department_id}&page={$pagination.current_page + 1}">Next &raquo;</a></li>
        {/if}
    </ul>
</nav>
