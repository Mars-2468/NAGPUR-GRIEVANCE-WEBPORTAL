...
<nav>
    <ul class="pagination">
        ...
        {if $pagination.current_page > 1}
            <li><a href="?app_type_id={$app_type_id}&ulbid={$ulbid}&dept_id={$dept_id}&status={$status}&page={$pagination.current_page - 1}{$filter_query}">&laquo; Prev</a></li>
        {/if}

        <!-- First Page Button -->
        {if $start_page > 1}
            <li><a href="?app_type_id={$app_type_id}&ulbid={$ulbid}&dept_id={$dept_id}&status={$status}&page=1{$filter_query}">1</a></li>
            {if $start_page > 2}
        ...
        {section name=pages start=$start_page loop=$end_page}
            <li class="{if $pagination.current_page == $smarty.section.pages.index+1}active{/if}">
                <a href="?app_type_id={$app_type_id}&ulbid={$ulbid}&dept_id={$dept_id}&status={$status}&page={$smarty.section.pages.index+1}{$filter_query}">{$smarty.section.pages.index+1}</a>
            </li>
        {/section}
        <!-- Last Page Button -->
        {if $end_page < $pagination.total_pages}
            {if $end_page < $pagination.total_pages - 1}
                <li class="disabled"><a href="#">...</a></li>
            {/if}
            <li><a href="?app_type_id={$app_type_id}&ulbid={$ulbid}&dept_id={$dept_id}&status={$status}&page={$pagination.total_pages}{$filter_query}">{$pagination.total_pages}</a></li>
        {/if}

        <!-- Next Button -->
        {if $pagination.current_page < $pagination.total_pages}
            <li><a href="?app_type_id={$app_type_id}&ulbid={$ulbid}&dept_id={$dept_id}&status={$status}&page={$pagination.current_page + 1}{$filter_query}">Next &raquo;</a></li>
        {/if}
    </ul>
</nav>
