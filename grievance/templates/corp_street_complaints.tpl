					...
					<tbody style="background-color:#FFF;">
						...
						<tr>
							...
							<td align='center' class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}">
								<a href="view_comp_det_admin.php?grievance_id={$grievance_id}" target="_blank">
...
<nav>
    <ul class="pagination">
        ...
        {if $pagination.current_page > 1}
            <li><a href="?app_type_id={$app_type_id}&ward_id={$ward_id}&street_id={$street_id}&status={$status}&f_date={$fdate}&t_date={$tdate}&page={$pagination.current_page - 1}">&laquo; Prev</a></li>
        {/if}

        <!-- First Page Button -->
        {if $start_page > 1}
            <li><a href="?app_type_id={$app_type_id}&ward_id={$ward_id}&street_id={$street_id}&status={$status}&f_date={$fdate}&t_date={$tdate}&page=1">1</a></li>
            {if $start_page > 2}
        ...
        {section name=pages start=$start_page loop=$end_page}
            <li class="{if $pagination.current_page == $smarty.section.pages.index+1}active{/if}">
                <a href="?app_type_id={$app_type_id}&ward_id={$ward_id}&street_id={$street_id}&status={$status}&f_date={$fdate}&t_date={$tdate}&page={$smarty.section.pages.index+1}">{$smarty.section.pages.index+1}</a>
            </li>
        {/section}
        <!-- Last Page Button -->
        {if $end_page < $pagination.total_pages}
            {if $end_page < $pagination.total_pages - 1}
                <li class="disabled"><a href="#">...</a></li>
            {/if}
            <li><a href="?app_type_id={$app_type_id}&ward_id={$ward_id}&street_id={$street_id}&status={$status}&f_date={$fdate}&t_date={$tdate}&page={$pagination.total_pages}">{$pagination.total_pages}</a></li>
        {/if}

        <!-- Next Button -->
        {if $pagination.current_page < $pagination.total_pages}
            <li><a href="?app_type_id={$app_type_id}&ward_id={$ward_id}&street_id={$street_id}&status={$status}&f_date={$fdate}&t_date={$tdate}&page={$pagination.current_page + 1}">Next &raquo;</a></li>
        {/if}
    </ul>
</nav>
