...
{/literal}
 ...
 <a href="http://www.municipalservices.in/service_emp_map_report.php" target="_blank" class="btn-link" style="color:#FFF;"><span class="fa fa-clipboard"></span> Click Here  To Employee Service Mapping Report</a></button>
 </center>
 	<br>
			
			<form   method="post" action="employee_map_street_wise.php" name="manage_wards"  id="manage_wards" class="form-horizontal" onSubmit="return validateForm()">
    ...
    <div >
		...
		<br>
		<form   method="post" action="employee_map_street_wise.php" name="manage_wards"  id="manage_wards" onSubmit="return validateForm()">
		...
		<thead>
			...
			{foreach from=$ward_list[$cs_id] item=row key=ward_id}
				...
				<td>
				 ...
				 <strong>Ward: &nbsp;&nbsp;</strong>{$ward_list[$cs_id][$ward_id]} 
				<br>
				
				{foreach from=$street_list[$cs_id][$ward_id] item=row2 key=street_id}
				
				<input type="checkbox" name="{'street_id'|cat:$i}" value="{$cs_id}-{$ward_id}-{$street_id}"> {$street_list[$cs_id][$ward_id][$street_id]} 
				<br>
