...
{/literal}
	...
	<div class="row">
			...
			<div class="boxed">
				<!-- Title Bart Start -->
				<div class="d-flex justify-content-between align-items-center success" style="background-color:#2c3e50 !important;color:#FFF !important;padding:5px;">
					<div><h4>USER DETAILS</h4></div>
					<div><a class="btn btn-primary float-right" href="all_users_list.php">Back</a></div>
				
				</div>
				<!-- Title Bart End -->
				<div class="inner no-radius">
							ID : {$data.user_id}<br>
							EMP ID : {$data.emp_id}<br>
						...
						<table id="data-table" class="table table-striped table-bordered table-hover table-full-width"  width="100%">
							<thead>
								<tr>
									...
									<th style="text-align: center1;">DEPT NAME</th>
									<th style="text-align: center1;">DESIGNATION</th>						
								</tr>
							</thead>
							<tbody>								
								{foreach from=$desg_list2 item=row key=emp_id}
									<tr>
										<td style="text-align: center;">{counter}</td>
										<td style="">{$row.dept_id}</td>
										<td style="">{$row.desg_id}</td>							
									</tr>						
								{/foreach}						
							</tbody>
						</table>
			...
			{if $user_services neq ''}	
			
				<div class="boxed">
					<!-- Title Bart Start -->
					<div class="title-bar success">
						<h4>USER SERVICE DETAILS</h4>
						<!--
					...
					<table id="data-table-services" class="table table-striped table-bordered table-hover table-full-width"  width="100%">
						<thead>
							<tr>
								<th style="text-align: center; width: 80px;">SR.No</th>
								<!-- <th style="text-align: center;">DEPT NAME</th> -->
								<th style="text-align: center1;">ACTIVE SERVICES</th>					
							</tr>
						</thead>
						<tbody>	
						{assign var="slno" value=1}
							{foreach from=$user_services key=service_key item=service_name}
								<tr>
		...
		{if $user_logs_list neq ''}		
			<div class="boxed">	<!-- Title Bart Start -->
				<div class="title-bar success">
					<h4>USER LOG DETAILS</h4>
				
				</div>						
				{foreach from=$user_logs_list key=user item=logs}
				<table id="data-table-services" class="table table-striped table-bordered table-hover table-full-width"  width="100%">
						...
						{foreach from=$logs item=row}
							<tr>
								...
								<td>{$row.logout_time|default:'-'}</td>
								<td>{$row.user_log_status|default:'-'}</td>
								
									{if $row.user_log_status == 'Loggedin'}
										<td style="background-color:#00ff00 !important;"><i class="fa fa-sign-in" style="font-size:18px;"></i></td>
									{else}
										<td style="background-color:#ff0000 !important;color:#FFF !important;"><i class="fa fa-sign-out" style="font-size:18px;"></i></td>
									{/if}
