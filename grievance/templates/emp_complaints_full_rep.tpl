		...
		<div class="inner no-radius">
			...
			<form method="POST" class="form-horizontal">
				...
				<div class="button-container">
					...
					<a href="complaints_full_rep.php?ulbid={$ulbid}&app_type_id={$app_type_id}&cs_id=1&status={$status}&f_date=&t_date=" class="btn btn-warning" onclick="history.go(-1);">
						<i class="fa fa-backward"></i> Back
					</a>
				</div>
			</form>

			<!--17-06-2024 <div class="col-md-4" align="right"><input name="search" type="submit" class="btn btn-success" value="SEARCH"> </div>

			</form>-->
			<!--Not Open <div style="text-align:right;"><a href="complaint_wise_rep.php?active=tr-clmn" class="btn btn-warning" onclick="history.go(-1);"><i class="fa fa-backward"></i> Back</a></div> -->
			<!-- <div style="text-align:right;"><a href="complaints_full_rep.php?ulbid={$ulbid}&app_type_id={$app_type_id}&cs_id=1&status={$status}&f_date=&t_date=" class="btn btn-warning" onclick="history.go(-1);"><i class="fa fa-backward"></i> Back</a></div> -->
		</div>
				...
				<div id="area" class="table-responsive">
					<table width="100%" border="1" cellspacing="0" cellpadding="0" class="display table-bordered table-striped table-condensed cf" id="data-table">
						<caption style="caption-side:top; text-align:center;font-size:16px;">
							<b>
								{if $status eq '0' || $status eq '1' && $user_type eq 'U' }
								...
								{elseif $status eq '108' && $user_type eq 'U' }
...
<nav>
    <ul class="pagination">
        ...
        {if $pagination.current_page > 1}
            <li><a href="?ulbid={$ulbid}&app_type_id={$app_type_id}&cs_id={$cs_id}&status={$status}&f_date={$fdate}&t_date={$tdate}&page={$pagination.current_page - 1}">&laquo; Prev</a></li>
        {/if}

        <!-- First Page Button -->
        {if $start_page > 1}
            <li><a href="?ulbid={$ulbid}&app_type_id={$app_type_id}&cs_id={$cs_id}&status={$status}&f_date={$fdate}&t_date={$tdate}&page=1">1</a></li>
            {if $start_page > 2}
        ...
        {section name=pages start=$start_page loop=$end_page}
            <li class="{if $pagination.current_page == $smarty.section.pages.index+1}active{/if}">
                <a href="?ulbid={$ulbid}&app_type_id={$app_type_id}&cs_id={$cs_id}&status={$status}&f_date={$fdate}&t_date={$tdate}&page={$smarty.section.pages.index+1}">{$smarty.section.pages.index+1}</a>
            </li>
        {/section}
        <!-- Last Page Button -->
        {if $end_page < $pagination.total_pages}
            {if $end_page < $pagination.total_pages - 1}
                <li class="disabled"><a href="#">...</a></li>
            {/if}
            <li><a href="?ulbid={$ulbid}&app_type_id={$app_type_id}&cs_id={$cs_id}&status={$status}&f_date={$fdate}&t_date={$tdate}&page={$pagination.total_pages}">{$pagination.total_pages}</a></li>
        {/if}

        <!-- Next Button -->
        {if $pagination.current_page < $pagination.total_pages}
            <li><a href="?ulbid={$ulbid}&app_type_id={$app_type_id}&cs_id={$cs_id}&status={$status}&f_date={$fdate}&t_date={$tdate}&page={$pagination.current_page + 1}">Next &raquo;</a></li>
        {/if}
    </ul>
</nav>
