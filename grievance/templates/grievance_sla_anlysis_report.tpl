						...
						<tbody>
							{foreach from=$cs_list item=row key=cs_id}
								<tr>
									...
									<td>{$row.desc}</td>
									<td align='center' class="{if $active_class eq 'tr-clmn'}activ_column{/if}"><a href="grievance_sla_anlysis_inner_report.php?cid={$cs_id}&status=all">{$data['complaints'][$cs_id]['count']}</a></td>
