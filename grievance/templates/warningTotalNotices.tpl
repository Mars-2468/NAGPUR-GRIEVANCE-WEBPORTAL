	...
	<div class="boxed">
		...
		<div class="inner no-radius">
			<form method="POST" class="form-horizontal">
				...
				<input type="hidden" name="warning_no" value="{$warning_no}">
				...
				<div class="col-md-4">
					<div class="form-group">
						<label class="control-label col-sm-6" for="usr">Warning Ref Num:</label>
						<div class="col-sm-6">
							<input type="text" class="phone-group form-control demoInputBox" name="warning_no" value="{$warning_no}" placeholder="Enter Warning Ref No." autocomplete="off">
						</div>
					</div>
				</div>
				...
				<div class="button-container">
					...
					<a href="warning_notice.php?active=tr-clmn" class="btn btn-warning" onclick="history.go(-1);">
				...
				<div id="area" class="table-responsive">
					<table width="100%" border="1" cellspacing="0" cellpadding="0" class="display table-bordered table-striped table-condensed cf" id="example">
						<caption style="caption-side:top; text-align:center;font-size:16px;">
							<b>
								{if $status eq 'war_all' && $user_type eq 'U' }
								<CENTER><strong>VIEW {if $app_type_id eq '1'} WARNING NOTICE LIST {else} SERVICE {/if} DETAILS</strong></CENTER>
								{elseif $status eq 'war_all'&& $user_type eq 'E' }
								<CENTER><strong>VIEW {if $app_type_id eq '1'} WARNING NOTICE LIST {else} SERVICE {/if} DETAILS</strong></CENTER>
								{else}
						...
						<thead>
							<tr style="background-color:#2c3e50; color:#FFF;">
								...
								<th align="center" valign="middle" style="width: 150px; text-align: center; vertical-align: middle;">WARNING REF NUM</th>
								...
								<th align="center" valign="middle" style="width: 200px; text-align: center; vertical-align: middle;">WARNING DATE</th>
							</tr>
						</thead>
						<tbody>
							...
							<tr>
								...
								<td align='center'>{$row.grievance_id}</td>

								<td align='center'>{$row.emp_name}</td>

								<td align='center'>{$row.dept_desc}</td>
								...
								<td align='center'>{$row.warning_date|date_format:"%d-%m-%Y %H:%M:%S"}</td>
							</tr>
							{/foreach}
						</tbody>
					</table>
...
<nav>
    <ul class="pagination">
        ...
        {if $pagination.current_page > 1}
            <li><a href="?ulbid=250&app_type_id=1&status={$status}&page={$pagination.current_page - 1}{$filter_query}">&laquo; Prev</a></li>
        {/if}

        <!-- First Page Button -->
        {if $start_page > 1}
            <li><a href="?ulbid=250&app_type_id=1&status={$status}&page=1{$filter_query}">1</a></li>
            {if $start_page > 2}
        ...
        {section name=pages start=$start_page loop=$end_page}
            <li class="{if $pagination.current_page == $smarty.section.pages.index+1}active{/if}">
                <a href="?ulbid=250&app_type_id=1&status={$status}&page={$smarty.section.pages.index+1}{$filter_query}">{$smarty.section.pages.index+1}</a>
            </li>
        {/section}
        <!-- Last Page Button -->
        {if $end_page < $pagination.total_pages}
            {if $end_page < $pagination.total_pages - 1}
                <li class="disabled"><a href="#">...</a></li>
            {/if}
            <li><a href="?ulbid=250&app_type_id=1&status={$status}&page={$pagination.total_pages}{$filter_query}">{$pagination.total_pages}</a></li>
        {/if}

        <!-- Next Button -->
        {if $pagination.current_page < $pagination.total_pages}
            <li><a href="?ulbid=250&app_type_id=1&status={$status}&page={$pagination.current_page + 1}{$filter_query}">Next &raquo;</a></li>
        {/if}
    </ul>
</nav>
