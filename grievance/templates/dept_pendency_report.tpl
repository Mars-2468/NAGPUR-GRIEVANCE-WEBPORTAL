        ...
        <div id="area">
            <table id='example' cellpadding="3" cellspacing="0" border="1" class="display table-bordered table-striped table-condensed cf" width="100%">
                ...
                <tbody>
                   {foreach from=$dept_list key=dept_id item=row}
                    ...
                    <tr>
                        ...
                        <td align='center' class="{if $active_class eq 'pwsla-clmn'}activ_column{/if}">{if $data[$dept_id].pending neq ''}<a href="inner_pendancy_report.php?app_type_id=1&dept_id={$dept_id}&status=107&f_date={$fdate}&t_date={$tdate}&sla_status=2" target="">{$data[$dept_id].pending}</a>{else}-{/if}</a></td>
                        <td align='center' class="{if $active_class eq 'rwsla-clmn'}activ_column{/if}">{if $data[$dept_id].completed neq ''}<a href="inner_pendancy_report.php?app_type_id=1&dept_id={$dept_id}&status=108&f_date={$fdate}&t_date={$tdate}" target="">{$data[$dept_id].completed}</a>{else}-{/if}</a></td>
					</tr>
                    {/foreach}
                ...
                <tfoot>
                    ...
                    <td align='center'>{if $tot['received'] gt 0}<a href="inner_pendancy_report.php?app_type_id=1&dept_id={$dept_id}&status=100&f_date={$fdate}&t_date={$tdate}">{$tot['received']}</a>{else}{$tot['received']}{/if}</td>
					<td align='center'>{if $tot['pending'] gt 0}<a href="inner_pendancy_report.php?app_type_id=1&dept_id={$dept_id}&status=800&f_date={$fdate}&t_date={$tdate}">{$tot['pending']}</a>{else}{$tot['pending']}{/if}</td>
                    <td align='center'>{if $tot['completed'] gt 0}<a href="inner_pendancy_report.php?app_type_id=1&dept_id={$dept_id}&status=900&f_date={$fdate}&t_date={$tdate}">{$tot['completed']}</a>{else}{$tot['completed']}{/if}</td>
                </tfoot>
            </table>
        </div>
