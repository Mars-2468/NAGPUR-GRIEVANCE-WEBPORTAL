...
<div class="" style="margin-top: -45px;">
	...
	<div class="boxed">
		<div class="title-bar blue" style="display: flex; justify-content: space-between; align-items: center;">
			<h4 style="margin: 0;">SHOW CAUSE NOTICE LIST</h4>
		</div>
			...
			<form method="POST" class="form-horizontal">
				...
				<div class="col-md-4">
					<div class="form-group">
						...
						<div class="col-sm-6">
							<input type="text" class="phone-group form-control demoInputBox" name="showcause_no" value="{$showcause_no}" placeholder="Enter Show Cause Ref" autocomplete="off">
						</div>
					</div>
				</div>
				...
				<div id="area" class="table-responsive">
					<table width="100%" border="1" cellspacing="0" cellpadding="0" class="display table-bordered table-striped table-condensed cf" id="example">
						<caption style="caption-side:top; text-align:center;font-size:16px;">
							<b>
								{if $status eq 'show_all' && $user_type eq 'U' }
								<CENTER><strong>VIEW {if $app_type_id eq '1'} SHOW CAUSE NOTICE LIST {else} SERVICE {/if} DETAILS</strong></CENTER>
								{elseif $status eq 'show_all'&& $user_type eq 'E' }
								<CENTER><strong>VIEW {if $app_type_id eq '1'} SHOW CAUSE NOTICE LIST {else} SERVICE {/if} DETAILS</strong></CENTER>
								{else}
						...
						<tbody>
							...
							<tr>
								...
								<td align='center'>{$row.showcase_date|date_format:"%d-%m-%Y %H:%M:%S"}</td>

								<td align='center'>
									<a href="showcause_notice_print.php?gid={$grievance_id}&showcause_id={$row.showcause_id}&emp_id={$row.emp_id}" target="" style="color: red;font-size: 20px;font-weight:bolder;"><b><i class="fa fa-eye" aria-hidden="true"></i></b></a>
								</td>
							</tr>
							{/foreach}
						</tbody>
