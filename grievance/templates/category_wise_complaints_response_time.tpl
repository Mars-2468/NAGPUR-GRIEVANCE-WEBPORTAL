...
<CENTER><strong>CATEGORY WISE COMPLAINTS AVG RESPONSE TIME</strong></CENTER>
    ...
    <div style="width:100%; overflow:scroll;">
	     <table class="table table-striped table-bordered table-hover table-full-width" id="data-table">
										...
										<tr>
										    <td>{counter}</td>
										    <td><a href="category-basis-ulb-wise-complaints-response-time.php?cat_id={$cs_id}" target="_blank">{$cat_list[$cs_id]}</a></td>
										    <td>{$response_time[$cs_id]['complaints'].count}</td>
										   <td>{$response_time[$cs_id]['complaints'].avg_res_time}</td>
