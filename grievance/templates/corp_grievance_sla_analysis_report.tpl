...
<div class="row ">
	<div class="">
		<div class="boxed">
			<!-- Title Bart Start -->
			<div class="title-bar blue1 d-flex align-items-center justify-content-between">
			...
			<div class="inner no-radius table-responsive">
				<table class="table table-striped table-bordered table-hover table-full-width" id="data-table">
					<caption style="caption-side:top; text-align:center;font-size:16px;">
						<b>
							<CENTER><strong>VIEW COMPLAINT SLA ANALYSIS REPORT DETAILS</strong></CENTER>
						</b>
					</caption>
					<thead>
						...
						<tbody>
							{foreach from=$cs_list item=row key=cs_id}
								<tr>
									...
									<td>{$row.desc}</td>
									<td align='center' class="{if $active_class eq 'tr-clmn'}activ_column{/if}"><a href="corp_grievance_sla_anlysis_inner_report.php?cid={$cs_id}&status=all">{$data['complaints'][$cs_id]['count']}</a></td>
									<td align='center'>{$data['level'][$cs_id]['L1']['count']}</td>
									<td align='center'>{$data['level'][$cs_id]['L2']['count']}</td>
									<td align='center'>{$data['level'][$cs_id]['L3']['count']}</td>
									<td align='center'>{$data['level'][$cs_id]['L4']['count']}</td>
				...
				<!--<form  method="post" action="corp_comp_cutoffdate_map.php" name="manage_wards"  class="form-horizontal" onSubmit="return validateForm()">
