					...
					<table id='example' cellpadding="3" cellspacing="0" border="1" class="display table-bordered table-striped table-condensed cf" width="100%">
						...
						<tbody>
							...
							{if $emp_id ne ''}
								...
								<td align='center' class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}">{if $data[$emp_id].count neq ''}<a href="all_warning_notice_list.php?ulbid={$ulbid}&app_type_id={$app_type_id}&emp_id={$emp_id}&status=war&dept_id={$emp_dept_list[$emp_id]}&f_date={$fdate}&t_date={$tdate}" style="text-decoration: none;">{$emp_list[$emp_id]}</a>{else}<b>{$emp_list[$emp_id]}</b>{/if}</td>
								<td align='center'><b>{$dept_list[$emp_dept_list[$emp_id]]}</b></td>
								<td align='center' class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}">{if $data[$emp_id].count neq ''}<a href="all_warning_notice_list.php?app_type_id={$app_type_id}&emp_id={$emp_id}&status=war&dept_id={$emp_dept_list[$emp_id]}&f_date={$fdate}&t_date={$tdate}" target="" style="text-decoration: none;">{$data[$emp_id].count}</a>{else}<b>-</b>{/if}</td>
						...
						<tfoot>
							...
							{if $user_type eq 'U'}
							<td align='center' class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}">{if $tot.received neq ''}<a href="warningTotalNotices.php?ulbid={$ulbid}&app_type_id={$app_type_id}&status=war_all&f_date={$fdate}&t_date={$tdate}" style="text-decoration: none;"><b>{$tot.received}</b></a>{else}<b>0</b>{/if}</td>
							{else}
