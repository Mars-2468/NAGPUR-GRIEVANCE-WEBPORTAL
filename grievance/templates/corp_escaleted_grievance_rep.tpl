						...
						<tbody>
							{foreach from=$max_comp_details item=row key=cs_id}
								...
								<tr>
									...
									<td align='center' class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}"><a href="corp_tot_escalations_received.php?cat3_id={$row.cat3_id}&ward_id=-1&status_id={$gstatus}&f_date={$fdate}&t_date={$tdate}">{$tot[$row.cat3_id]['total']}</a></td>
									{foreach from=$ward_list item=row2 key=ward_id}
									<td align='center' class="{if $active_class eq 'tr-clm3qclmn'}activ_column{/if}"><a href="corp_tot_escalations_received.php?cat3_id={$row.cat3_id}&ward_id={$ward_id}&status_id={$gstatus}&fdate={$f_date}&t_date={$tdate}">{$comp_details[$row.cat3_id][$ward_id]['count']}</a></td>
									{/foreach}
								</tr>
