...
//echo "<pre>";print_r($_SESSION['csrf_token']);echo "</pre>";die();
	...
	if (isset($_FILES)) {
		...
		$field_visit_form_docs_id = $_POST['field_visit_form_docs_id'];
		...
		$sql = "select * from field_visit_form_docs where ulbid='" . $_SESSION['ulbid'] . "' and id=".$field_visit_form_docs_id."";
		
		$rs = mysqli_query($conn, $sql);
		
		$row =mysqli_fetch_assoc($rs);
		
		$field_visit_form_id=$row['field_visit_form_id'];
		
		$result=uploadSingleFile($docfile);
		$msg=$result['msg'];
		
		if (!empty($result['path'])) {
			$sql = "update field_visit_form_docs set doc_title='".$doctitle."',doc_file='".$result['path']."',mime_type='".$result['mime_type']."',created_by='".$created_by."',updated_at='".$updated_at."' where id=".$field_visit_form_docs_id." ";
