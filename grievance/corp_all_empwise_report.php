...
if (isset($_SESSION['uid'])) {
	...
	//$emp_name = $_REQUEST['emp_name'];
		...
		if (isset($_POST['search'])) {
			...
			if(!validateField($_POST['emp_name'],'text') || !validateField($_POST['app_type_id'],'dnumber') || !validateField($_POST['dept_id'],'dnumber')){
				$errors[] = "Invalid employee name/app type id/department id!";
			}
	...
	g.ulbid='" . $_SESSION['ulbid'] . "' and gt.disposal_status IN(2,3,5,6,8,9,10,11,12,13) and g.grievance_status_id IN(2,3,5,6,8,9,10,11,12,13) and g.cat3_id !='0' ";

	if($app_type_id!=''){
		$sql .= "  and g.app_type_id='" . $app_type_id . "' ";
	}
	...
	$sql .= " group by emp_id, gt.disposal_status ";
	...
	if($app_type_id!=''){
		$sql1 .= "  and  g.app_type_id='" . $app_type_id . "' ";
	}
	...
	if($ward_id!=''){
		$sql1 .= " and g.ward_id=" . $ward_id . " ";
	}

	if($f_date!= '' && $t_date!='' && ($f_date < $t_date)){
		$sql1 .= " and date(date_regd) between '" . $f_date . "' and '" . $t_date . "' ";
	} 
	...
	$sql21 = "select COUNT(DISTINCT g.grievance_id) as count1,emp_id,disposal_status,g.date_regd,g.sla_status from grievances g, ".$_SESSION['grievances_trns']." gt where 
			g.grievance_id=gt.grievance_id and g.ulbid='" . $_SESSION['ulbid'] . "' and 
			gt.disposal_status!='5' and g.sla_status='1' and
	...
	$sql21 = "SELECT count(DISTINCT g.grievance_id) as count1,emp_id,disposal_status FROM grievances g,".$_SESSION['grievances_trns']." gt where g.grievance_id=gt.grievance_id and ulbid='" . $_SESSION['ulbid'] . "' and 
			 grievance_status_id IN('3','9','8') and grievance_status_id IN('3','9','8') and sla_status=1 and cat3_id !='0' and gt.disposal_status!='5' ";

	if($app_type_id!=''){
		$sql21 .= " and g.app_type_id='" . $app_type_id . "' ";
	}

	if($dept_id!=''){
		$sql21 .= " and gt.dept_id='" . $dept_id . "' ";
	}

	if($ward_id!=''){
		$sql21 .= " and g.ward_id=" . $ward_id . " ";
	}
	
	if($f_date!= '' && $t_date!='' && ($f_date < $t_date)){
		$sql21 .= " and date(date_regd) between '" . $f_date . "' and '" . $t_date . "' ";
	} 
	
		$sql21 .= " group by gt.emp_id  ";
	...
	$sql2 = "SELECT count(DISTINCT gt.grievance_id) as count2,emp_id  FROM grievances g,".$_SESSION['grievances_trns']." gt where g.grievance_id=gt.grievance_id and ulbid='" . $_SESSION['ulbid'] . "' and 
    				 grievance_status_id IN('3','9','8') and gt.disposal_status IN('3','9','8') and sla_status=2 and cat3_id !='0' and gt.disposal_status!='5' ";

	if($dept_id!=''){
		$sql2 .= " and gt.dept_id='" . $dept_id . "' ";
	}

	if($ward_id!=''){
		$sql2 .= " and g.ward_id=" . $ward_id . " ";
	}
	
	if($f_date!= '' && $t_date!='' && ($f_date < $t_date)){
		$sql2 .= " and date(date_regd) between '" . $f_date . "' and '" . $t_date . "' ";
	} 
	...
	if($dept_id!=''){
		$sql31 .= " and gt.dept_id='" . $dept_id . "' ";
	}

	if($ward_id!=''){
		$sql31 .= " and g.ward_id=" . $ward_id . " ";
	}
	
	if($f_date!= '' && $t_date!='' && ($f_date < $t_date)){
		$sql31 .= " and date(date_regd) between '" . $f_date . "' and '" . $t_date . "' ";
	} 
	...
	$sql3 = "select COUNT(DISTINCT g.grievance_id) as count2,emp_id,disposal_status,g.date_regd,g.sla_status from grievances g, ".$_SESSION['grievances_trns']." gt where 
				 g.grievance_id=gt.grievance_id and g.ulbid='" . $_SESSION['ulbid'] . "' and 
				 gt.disposal_status!='5' and g.cat3_id !='0'  and gt.disposal_status IN ('2') and grievance_status_id IN('2') and g.sla_status='2' ";
	...
	if($app_type_id!=''){
		$sql3 .= "  and g.app_type_id='" . $app_type_id . "' ";
	}

	if($dept_id!=''){
		$sql3 .= "  and gt.dept_id='" . $dept_id . "' ";
	}

	if($ward_id!=''){
		$sql3 .= "  and g.ward_id=" . $ward_id . " ";
	}

	if($f_date!= '' && $t_date!='' && ($f_date < $t_date)){
		$sql3 .= " and date(date_regd) between '" . $f_date . "' and '" . $t_date . "' ";
	} 
...
// echo "<pre>";print_r($f_date);echo "</pre>"; die(); 
	...
	$tpl->display('corp_all_empwise_report.tpl');
} else {
