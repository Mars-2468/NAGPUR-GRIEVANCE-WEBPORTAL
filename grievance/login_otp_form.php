    ...
    <div class="container">
		...
		<?php
            ...
            if (isset($_SESSION['flash_error_message'])) {
                $flash_error_message = $_SESSION['flash_error_message'];
                unset($_SESSION['flash_error_message']); ?>

                <div class="alert1 bg-danger" style="padding:15px;margin-bottom:10px;">
                    <span class="closebtn p-3" onclick="this.parentElement.style.display='none';">&times;</span>
                    <?php echo $flash_error_message; ?>
				</div>
            <?php    } ?>
            ...
            <div class="panel-body mb_bg">
                ...
                <?php if ($_SESSION['otpstaus'] == 0) { ?>
                    ...
                    <form action="process_mobile.php" class="form-horizontal" method="post">
        ...
        <?php /* if ($_SESSION['otpstaus'] == 1 && $_SESSION['mobilesubmit'] == 1) {
			...
			if ($_SESSION['otpstaus'] == 1 && $_SESSION['mobilesubmit'] == 1) {
            $_SESSION['otpstaus_CHECK'] = 5;
        ?>
            ...
            <form action="process_otp.php" method="post">
                <div class="row dcenter">
                    ...
                    <div class="col-md-6 text-center" style="padding-top: 24px; margin-bottom:275px">
						...
						<a href="process_otp_terminate.php"  class="btn btn-danger" onclick="terminateOTP()">Terminate OTP</a>
					</div>

                </div>
            </form>
            <form action="process_mobile.php" id="resend_otp" method="post">
...
$(function () {
    ...
    if (otp_status == 1 && mob_status == 1 && verify_otp_status == 0) {
        initiateTimer();
    }

    // Stop timer when OTP form is submitted
    $(document).on('submit', 'form[action="process_otp.php"]', function () {
        stopTimer();
    });
});
...
function checkSecond(sec) {
    sec = parseInt(sec);
    if (sec < 10 && sec >= 0) { sec = "0" + sec; } // add zero
    if (sec < 0) { sec = "59"; }
    return sec;
}
