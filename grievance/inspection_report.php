...
//echo "<pre>";print_r($_SESSION['csrf_token']);echo "</pre>";die();
	...
	if (isset($_POST['search']) && $department_id!='') {
		$sql .= " and dept_id=" . $department_id . " ";
	}
	
	if (isset($_POST['search']) && ($f_date!='' && $t_date!='')) {
		$sql .= " and date_format(created_at,'%Y-%m-%d') between  '" . $f_date . "' and '" . $t_date . "' ";
	}
...
//echo "<pre>";print_r($desg_list);echo "</pre>";die();
	...
	$tpl->display('inspection_report.tpl');
} else {
