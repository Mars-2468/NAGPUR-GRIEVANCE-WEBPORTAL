	...
	{	    
		...
		if($f_date!= '' && $t_date!='' && ($f_date < $t_date)){
			...
			if(!empty($department_id)){
				$sql ="SELECT count(DISTINCT (g.grievance_id)) as count, g.cat3_id,cm.description,cm.cat_id,c.sub_cat_id,g.ward_id FROM `grievances` g, cs_mst c, category_mst cm , " . $_SESSION['grievances_trns'] . " gt where g.grievance_id=gt.grievance_id and g.cat3_id=c.cs_id and c.cat_id=cm.cat_id and `is_escalated` = 1 and c.cat_id=". $department_id . " and g.grievance_status_id IN(" . $grievance_status . ") and g.date_regd between '" . $f_date . "' and '" . $t_date . "' ";
			}else{
				$sql ="SELECT count(DISTINCT (g.grievance_id)) as count, g.cat3_id,cm.description,cm.cat_id,c.sub_cat_id,g.ward_id FROM `grievances` g, cs_mst c, category_mst cm , " . $_SESSION['grievances_trns'] . " gt where g.grievance_id=gt.grievance_id and g.cat3_id=c.cs_id and c.cat_id=cm.cat_id and `is_escalated` = 1 and g.grievance_status_id IN(" . $grievance_status . ") and g.date_regd between '" . $f_date . "' and '" . $t_date . "'  ";
			}
			
		}else{
			
			if(!empty($department_id)){
				$sql ="SELECT count(DISTINCT (g.grievance_id)) as count, g.cat3_id,cm.description,cm.cat_id,c.sub_cat_id,g.ward_id FROM `grievances` g, cs_mst c, category_mst cm , " . $_SESSION['grievances_trns'] . " gt where g.grievance_id=gt.grievance_id and g.cat3_id=c.cs_id and c.cat_id=cm.cat_id and `is_escalated` = 1 and c.cat_id=" . $department_id . " and g.grievance_status_id IN(" . $grievance_status . ")  ";
			}else{
				$sql ="SELECT count(DISTINCT (g.grievance_id)) as count, g.cat3_id,cm.description,cm.cat_id,c.sub_cat_id,g.ward_id FROM `grievances` g, cs_mst c, category_mst cm , " . $_SESSION['grievances_trns'] . " gt where g.grievance_id=gt.grievance_id and g.cat3_id=c.cs_id and c.cat_id=cm.cat_id  and `is_escalated` = 1 and g.grievance_status_id IN(" . $grievance_status . ") ";
			}
			
		}
		
		$sql .= " and g.ward_id = {$ward_id} ";
		$sql .= " group by g.cat3_id , g.ward_id order by count DESC ";
...
//echo "<pre>";print_r($sql);echo "</pre>";die();	
		...
		$tpl->display('corp_escaleted_grievance_rep.tpl');
	}
	else
	{
