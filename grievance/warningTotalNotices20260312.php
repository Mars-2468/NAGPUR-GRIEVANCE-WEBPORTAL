...
if (isset($_SESSION['uid'])) {
	...
	if ($status == 'war_all') {
		$query="SELECT COUNT(DISTINCT sc.warning_id) as num, sc.datetime AS warning_date, e.emp_id,e.emp_name, g.ward_id, g.street_id, g.date_regd,sc.grievance_id,g.cat3_id FROM show_case_response_logs sc,emp_mst e,grievances g WHERE sc.emp_id = e.emp_id and sc.grievance_id = g.grievance_id and g.ulbid='" . $_SESSION['ulbid'] . "' and g.app_type_id='" . $_REQUEST['app_type_id'] . "'";
	}
	...
	if($_REQUEST['f_date']!= '' && $_REQUEST['t_date']!='')
	{
		$fdate = date('Y-m-d', strtotime($_REQUEST['f_date']));
		$tdate = date('Y-m-d', strtotime($_REQUEST['t_date']));
		$query .= " and date(sc.datetime) between '" . $fdate . "' and '" . $tdate . "'";
	}
...
//echo "<pre>";print_r($sql);echo "</pre>";die();
	...
	$tpl->display('warningTotalNotices.tpl');
} else {
