...
function clean_input(string $field_name, ?string $input): string {
    $input = trim($input ?? '');
    ...
    // Remove suspicious patterns (like this=null)
...
function validate_mobile(string $mobile): string {
    // Remove non-digit characters
    $mobile = preg_replace('/\D/', '', $mobile);

    if (strlen($mobile) != 10) {
        $_SESSION['error_message'] = "Error: Mobile number must be 10 digits!";
        header("Location: smartnmcInsert.php");
        exit;
    }
...
$mobile      = validate_mobile(clean_input('Mobile', $_POST['mobile'] ?? ''));
$email       = validate_email(clean_input('Email', $_POST['email'] ?? ''));
