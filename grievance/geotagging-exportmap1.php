...
ini_set('display_errors',0);
	               ...
	               //	session_start();
            		...
            		//$conn=getconnection();
                    $sql21 ="select * from ulbmst where ulbid='".$_SESSION['ulb']."'";
		  ...
		  if($_POST['type']==9){
		     ...
		     if($_POST['type']==1 and $_POST['subtype'] == 4){  
                 ...
                 if(mysqli_num_rows($rsHhlData14) == 0){ ?>
                   <script>
                     ...
                     location.replace("http://municipalservices.in/geotagging-reportmap1.php")
                   </script> 
                 <?php exit();
                }    
		     ...
		     if($hoading==3){
    		     ...
    		     $sqlIEC12="SELECT tb1.*,tb2.ward_desc FROM `IECHoardings` tb1 join ward_mst tb2 on tb1.Wardno=tb2.ward_id where tb1.ulbid='".$_POST['ulb']."' $Wardno $datesbw  ORDER BY tb1.Id DESC";
               	 $rsIEC12=mysqli_query($conn,$sqlIEC12);
    		...
    		if($drains==8){
    		     ...
    		     $sqlIEC12="SELECT tb1.*,tb2.ward_desc FROM `Drain_SewersUGD` tb1 join ward_mst tb2 on tb1.Wardno=tb2.ward_id where tb1.ulbid='".$_POST['ulb']."' and tb1.Typeid='".$drains."' $Wardno $datesbw  ORDER BY tb1.Id DESC";
               	 $rsIEC12=mysqli_query($conn,$sqlIEC12);
                  if(mysqli_num_rows($rsIEC12) == 0){ ?>
                       <script>
                         alert('No Records found');
		   ...
		   if($ugd==9){
    		     ...
    		     $sqlIEC12="SELECT tb1.*,tb2.ward_desc FROM `Drain_SewersUGD` tb1 join ward_mst tb2 on tb1.Wardno=tb2.ward_id where tb1.ulbid='".$_POST['ulb']."' and tb1.Typeid='".$ugd."' $Wardno $datesbw  ORDER BY tb1.Id DESC";
               	 $rsIEC12=mysqli_query($conn,$sqlIEC12);
                  if(mysqli_num_rows($rsIEC12) == 0){ ?>
                       <script>
                         alert('No Records found');
		    ...
		    <div style="padding:10px; background-color:#a20e0e; margin:0 auto;font-size: 25px; text-align:center; font-family:'Open Sans'; font-size:14px; color:#FFF;">
                <div class="heading3"><strong style="font-size: 25px;" >ULB :: <?php echo $_SESSION['uid']; ?>  </strong></div>
		    ...
		    if($_POST['type']==1 and $_POST['subtype'] == 4){  
		     ...
		     if($drains==8){
    		     ...
    		     $sqlIEC12="SELECT tb1.*,tb2.ward_desc FROM `Drain_SewersUGD` tb1 join ward_mst tb2 on tb1.Wardno=tb2.ward_id where tb1.ulbid='".$_POST['ulb']."' and tb1.Typeid='".$drains."' $Wardno $datesbw  ORDER BY tb1.Id DESC";
                	...
                	while($rowsIEC12 = mysqli_fetch_assoc($rsIEC12))
    	     	{
    		      
    		          echo '["'.$rowsIEC12['AreaFrom'].'TO'.$rowsIEC12['AreaTo'].'", '.$rowsIEC12['latitude'].', '.$rowsIEC12['longitude'].'],';
    		    }
		     }
		     if($ugd==9){
		            
    		     $sqlIEC112="SELECT tb1.*,tb2.ward_desc FROM `Drain_SewersUGD` tb1 join ward_mst tb2 on tb1.Wardno=tb2.ward_id where tb1.ulbid='".$_POST['ulb']."' and tb1.Typeid='".$ugd."' $Wardno $datesbw  ORDER BY tb1.Id DESC";
               	 $rsIEC112=mysqli_query($conn,$sqlIEC112);
              
                	while($rowsIEC112 = mysqli_fetch_assoc($rsIEC112))
    	     	{
    		      
    		          echo '["'.$rowsIEC112['AreaFrom'].'TO'.$rowsIEC112['AreaTo'].'", '.$rowsIEC112['latitude'].', '.$rowsIEC112['longitude'].'],';
    		    }
		     }
    		    ?>
    ];
         ...
         <?php
		    ...
		    if($_POST['type']==1 and $_POST['subtype'] == 4){  
		     ...
		     if($drains==8){
    		     ...
    		     $sqlIEC11="SELECT tb1.*,tb2.ward_desc FROM `Drain_SewersUGD` tb1 join ward_mst tb2 on tb1.Wardno=tb2.ward_id where tb1.ulbid='".$_POST['ulb']."' and tb1.Typeid='".$drains."'  $Wardno $datesbw  ORDER BY tb1.Id DESC";
               	 $rsIEC11=mysqli_query($conn,$sqlIEC11);
              
                	while($rowsIEC11 = mysqli_fetch_assoc($rsIEC11))
    	     	{
    		       ?>
                  ["<div class='info_content'>" +
                 "<h3><?php echo $rowsIEC11['AreaFrom'].' to '.$rowsIEC11['AreaTo']; ?></h3>" +
                 "<p>Ward no:<?php echo $rowsIEC11['ward_desc']; ?></p>" + "</div>"],
                  <?php
    		    }
		     }
		     ...
		     if($ugd==9){
    		     ...
    		     $sqlIEC111="SELECT tb1.*,tb2.ward_desc FROM `Drain_SewersUGD` tb1 join ward_mst tb2 on tb1.Wardno=tb2.ward_id where tb1.ulbid='".$_POST['ulb']."' and tb1.Typeid='".$ugd."'  $Wardno $datesbw  ORDER BY tb1.Id DESC";
               	 $rsIEC111=mysqli_query($conn,$sqlIEC111);
              
                	while($rowsIEC111 = mysqli_fetch_assoc($rsIEC111))
    	     	{
    		       ?>
                  ["<div class='info_content'>" +
                 "<h3><?php echo $rowsIEC111['AreaFrom'].' to '.$rowsIEC111['AreaTo']; ?></h3>" +
                 "<p>Ward no:<?php echo $rowsIEC111['ward_desc']; ?></p>" + "</div>"],
                  <?php
    		    }
		     }
    		    ?>
