...
if (isset($_SESSION['uid'])) {
	...
	$sql = "select app_type_id,person_name,email,hno,address,ward_id,street_id,mobile,cat3_id,comp_desc,grievance_origin_id,grievance_status_id,date_regd,file_no,endorsement,mcat3_id,file_url from grievances where grievance_id='" . $_POST['grievance_id'] . "' and ulbid='" . $_SESSION['ulbid'] . "'";
	...
	try {
		...
		if (isset($_POST['save'])) {
			...
			$disposed_date = date('Y-m-d H:i:s', strtotime($_REQUEST['disposed_date']));
			$disposal_remarks = $_REQUEST['disposal_remarks'];
			$rca = $_REQUEST['rca'];
			$ca = $_REQUEST['ca'];
			...
			if ($disposal_status != 5) {
				$sql = "update grievances_transactions set disposed_date='" . $disposed_date . "',disposal_status='" . $disposal_status . "',disposal_remarks='" . $disposal_remarks . "',updated_by='" . $_SESSION['uid'] . "',origin_id='3',rca='" . $rca . "',ca='" . $ca . "' where grievance_id='" . $grievance_id . "' and disposal_status='2'";
				...
				$transaction_id = $trnsrow['transaction_id'];


				$sql = "update grievances_transactions set disposed_date='" . $disposed_date . "',disposal_status='" . $disposal_status . "',disposal_remarks='" . $disposal_remarks . "',updated_by='" . $_SESSION['uid'] . "',origin_id='3',rca='" . $rca . "',ca='" . $ca . "' where grievance_id='" . $grievance_id . "' and transaction_id='" . $transaction_id . "'";


				// Check if the record already exists before inserting
				$sql_check = "SELECT COUNT(*) FROM grievances_transactions WHERE grievance_id = ? AND transaction_id = ?";
				$query_check = $conn->prepare($sql_check);
				$query_check->bind_param("ii", $grievance_id, $transaction_id);
				$query_check->execute();
				...
				if ($count == 0) {
					...
					$query->bind_param("iiisiii", $grievance_id, $transaction_id, $emp_id, $emp_dept, $disposed_date, $disposal_status, $_SESSION['uid']);
					...
					if ($query->execute()) {
						...
						$tpl->assign('msg', 'Failed To Insert Record: ' . $query->error);
					}
					$query->close();
				} else {
					// Handle case where record already exists
					$tpl->assign('msg', 'Record Already Exists..!');
				}
				...
				if ($disposal_status == 5) {
					...
					$sms = "Dear " . substr($data1['person_name'], 0, 29) . " , Your Grievance regarding " . substr($cs_list[$data1['cat3_id']], 0, 30) . " with Ref No " . $grievance_id . " is resolved on " . $disposed_date . "Regards - Citizen Service Monitoring Cell ,NMCGOV";
					$mobile = $data1['mobile'];
					$templateId = "1707167653168579049";
					$message = str_replace(' ', '%20', $sms);
					$url = "http://smsatm.net/v3/api.php?username=nmcgov&apikey=fe4ba54cf34bec238813&senderid=NMCGov&templateid=" . $templateId . "&mobile=" . $mobile . "&message=" . $message;
					//require_once('aurangabad_sms_config.php');
					$post = curl_init();
	...
	if (isset($_POST['grievance_id'])) {
		...
		$sql = "select transaction_id,emp_id,alloted_date,disposed_date,disposal_status,disposal_remarks,app_type_id,rca,

			 ca from grievances g, grievances_transactions gt where  g.grievance_id=gt.grievance_id and 

			 gt.grievance_id='" . $_POST['grievance_id'] . "' and disposal_status IN('2','11','6') order by transaction_id";
