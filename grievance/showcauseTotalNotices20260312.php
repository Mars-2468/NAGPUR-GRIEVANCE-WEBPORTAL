...
if (isset($_SESSION['uid'])) {
	...
	if ($status == 'war') {
		$query="SELECT COUNT(DISTINCT sc.warning_id) as num, sc.datetime AS showcase_date, e.emp_id,e.emp_name, g.ward_id, g.street_id, g.date_regd,sc.grievance_id,g.cat3_id FROM show_case_response_logs sc,emp_mst e,grievances g WHERE sc.emp_id = e.emp_id and sc.grievance_id = g.grievance_id and g.ulbid='" . $_SESSION['ulbid'] . "' and g.app_type_id='" . $_REQUEST['app_type_id'] . "'";
	}
	...
	if($_REQUEST['f_date']!= '' && $_REQUEST['t_date']!='')
	{
		$fdate = date('Y-m-d', strtotime($_REQUEST['f_date']));
		$tdate = date('Y-m-d', strtotime($_REQUEST['t_date']));
		$query .= " and date_format(sc.datetime,'%Y-%m-%d') between '" . $fdate . "' and '" . $tdate . "' and date_format(sc.date_regd, '%Y-%m-%d') >= '2024-09-01'  ";
	}else{
		$query .= "  and date_format(sc.date_regd, '%Y-%m-%d') >= '2024-09-01' ";				
	}

	if ($selectedYear) { 
		$query .=" and date_format(sc.date_regd, '%Y') = '" . $selectedYear . "'  ";
	}
