<?php
   session_start();
	...
	if(isset($_SESSION['uid']))
	{
		...
		if(isset($_POST['save']))
		{
			$maxsize = 5000000;
			 ...
			 $sql ="update comm_tab set address='".sanitize_input($_POST['address'])."',
                    address_marathi='".sanitize_input($_POST['address_marathi'])."',
                    link='".trim(mysqli_real_escape_string($conn,strip_tags($_POST['link'])))."',
                    comm_name='".sanitize_input($_POST['comm_name'])."',
                    comm_name_marathi='".sanitize_input($_POST['comm_name_marathi'])."',
			...
			if(mysqli_query($conn,$sql))
			{
				...
				$tpl->assign('msg','Details Updated Successfully ');
			}
			else
			{
	...
	function sanitize_input($cust_data) {

		//PHP
		// Remove unnecessary spaces
		$cust_data = trim($cust_data,"-+=\'\"");

		// Strip tags to prevent HTML and PHP code injection
		$cust_data = strip_tags($cust_data);

		// Convert special characters to HTML entities (e.g., < to &lt;)
		$cust_data = htmlspecialchars($cust_data);	

		$cust_data = preg_replace('/[^a-zA-Z0-9_@()-]/', '', $cust_data);	
