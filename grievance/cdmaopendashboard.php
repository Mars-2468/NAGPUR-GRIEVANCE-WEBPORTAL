<?php
    ...
    if($https =='on')
    {
        echo 'hkh';
        exit;
        header('location:http://municipalservices.in/cdmaopendashboard.php');
    }*/
	...
	require_once('get_services.php');
		...
		if($_SESSION['user_type']=='E')
		{
		    ...
		    $sql->bind_param("i",htmlspecialchars(strip_tags($_SESSION['emp_id'])));
		    $sql->execute();
        	$rs2 = $sql->get_result();
        	$nr2 = $rs2->num_rows;
	    ...
	    $sql->bind_param("ssii",htmlspecialchars(strip_tags($_SESSION['ulbid'])),htmlspecialchars(strip_tags($_SESSION['ulbid'])),$flag,$cs_type_id);
	    $sql->execute();
    	$rs = $sql->get_result();
		while($row = $rs->fetch_assoc())
		{
	    ...
	    $sql->bind_param("ssi",htmlspecialchars(strip_tags($_SESSION['ulbid'])),htmlspecialchars(strip_tags($_SESSION['ulbid'])),$flag);
	    $sql->execute();
    	$rs = $sql->get_result();
		while($row = $rs->fetch_assoc())
		{
		...
		$tpl->display('cdmaopendashboard.tpl');
