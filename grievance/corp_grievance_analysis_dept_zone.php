...
if (isset($_SESSION['uid'])) {
		...
		if (isset($_POST['search'])) {
			...
			if (!empty($_POST['f_date']) && !empty($_POST['t_date'])) {
				...
				if ($f_date_obj && $t_date_obj) {
					$fdate = $f_date_obj->format('Y-m-d');
					$tdate = $t_date_obj->format('Y-m-d');
	...
	$sql = "select COUNT(DISTINCT gt.grievance_id) as count,gt.dept_id as emp_dept,disposal_status,ward_id from grievances g, " . $_SESSION['grievances_trns'] . " gt where 
	...
	$sql .= " group by gt.dept_id,ward_id";
	//echo $sql;     

	if ($rs = mysqli_query($conn, $sql)) {
		while ($row = mysqli_fetch_assoc($rs)) {
			$data[$row['emp_dept']][$row['ward_id']]['count'] = $row['count'];
			$tot['received'] += $row['count'];
		}
	} else
		printf("Errormessage: %s\n", mysqli_error($conn));

	$sql = "SELECT count(DISTINCT g.grievance_id) as count1,dept_id as emp_dept,ward_id FROM grievances g," . $_SESSION['grievances_trns'] . " gt where g.grievance_id=gt.grievance_id and ulbid='" . $_SESSION['ulbid'] . "' and 
	...
	$sql .= " group by gt.dept_id,g.ward_id,gt.disposal_status";

	//echo $sql;

	if ($rs = mysqli_query($conn, $sql)) {
		while ($row = mysqli_fetch_assoc($rs)) {
			$data_list[$row['emp_dept']][$row['ward_id']]['completed'] += $row['count1'];
	...
	$sql = "select COUNT(DISTINCT g.grievance_id) as count,dept_id as emp_dept,ward_id from grievances g, " . $_SESSION['grievances_trns'] . " gt where 
	...
	$sql .= " group by gt.dept_id,g.ward_id";

	$rs = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($rs)) {
		$data_list[$row['emp_dept']][$row['ward_id']]['reopened'] += $row['count'];
		$tot['reopened'] += $row['count'];
	}

	/// unresolvable complaints

	$sql = "select COUNT(g.grievance_id) as count1,dept_id as emp_dept,g.ward_id from grievances g, " . $_SESSION['grievances_trns'] . " gt where 
	...
	$rs = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($rs)) {
		$data_list[$row['emp_dept']][$row['ward_id']]['unresolved'] += $row['count1'];
	...
	$rs = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($rs)) {
		$data_list[$row['emp_dept']][$row['ward_id']]['fin'] += $row['count1'];
		$tot['fin'] += $row['count1'];
	}
	...
	$sql = "select COUNT(DISTINCT gt.grievance_id) as count2,dept_id as emp_dept,g.ward_id from grievances g, " . $_SESSION['grievances_trns'] . " gt where 
	g.grievance_id=gt.grievance_id  and cat3_id !='0' and g.app_type_id='1' 
	...
	if ($rs = mysqli_query($conn, $sql)) {
		while ($row = mysqli_fetch_assoc($rs)) {
			$data_list[$row['emp_dept']][$row['ward_id']]['rejected'] += $row['count2'];
	...
	$sql = "SELECT count(DISTINCT g.grievance_id) as count3,dept_id as emp_dept,g.ward_id FROM grievances g," . $_SESSION['grievances_trns'] . " gt where g.grievance_id=gt.grievance_id and ulbid='" . $_SESSION['ulbid'] . "' and 
	...
	$rs = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($rs)) {
		$data_list[$row['emp_dept']][$row['ward_id']]['completed_be_sla'] += $row['count3'];
		$tot['completed_be_sla'] += $row['count3'];
	}

	$sql6 = "SELECT count(g.grievance_id) as count2,dept_id as emp_dept,g.ward_id FROM grievances g," . $_SESSION['grievances_trns'] . " gt where g.grievance_id=gt.grievance_id and ulbid='" . $_SESSION['ulbid'] . "' and 
	...
	if($ward_ide!= ''){
		$sql6 .= " and g.ward_id=" . $ward_id . " ";
	}
	$sql6 .= " group by gt.dept_id,g.ward_id";

	$rs6 = mysqli_query($conn, $sql6);
	//echo $sql6;
	while ($row = mysqli_fetch_assoc($rs6)) {
		$data_list[$row['emp_dept']][$row['ward_id']]['pending'] += $row['count2'];
		$tot['pending'] += $row['count2'];
	}

	$sql11 = "SELECT count(g.grievance_id) as count2,dept_id as emp_dept,g.ward_id FROM grievances g," . $_SESSION['grievances_trns'] . " gt where g.grievance_id=gt.grievance_id and ulbid='" . $_SESSION['ulbid'] . "' and 
	...
	if($ward_ide!= ''){
		$sql11 .= " and g.ward_id=" . $ward_id . " ";
	}
	$sql11 .= " group by gt.dept_id,g.ward_id";

	//echo $sql11;
	$rs611 = mysqli_query($conn, $sql11);
	while ($row = mysqli_fetch_assoc($rs611)) {
		$data_list[$row['emp_dept']][$row['ward_id']]['pending_be'] += $row['count2'];
		$tot['pending_be'] += $row['count2'];
	}

	$sql = "select COUNT(DISTINCT gt.grievance_id) as count,dept_id as emp_dept,g.grievance_status_id,g.ward_id from grievances g, 
	" . $_SESSION['grievances_trns'] . " gt where g.grievance_id=gt.grievance_id  and cat3_id !='0' and g.app_type_id='1' and
	...
	$rs = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($rs)) {
		$data_list[$row['emp_dept']][$row['ward_id']][$row['grievance_status_id']]['reopend_underProgress'] += $row['count'];
	...
	$sql = "select COUNT(DISTINCT gt.grievance_id) as count,dept_id as emp_dept,g.grievance_status_id,g.ward_id from grievances g, 
	" . $_SESSION['grievances_trns'] . " gt where g.grievance_id=gt.grievance_id  and cat3_id !='0' and g.app_type_id='1' and g.grievance_status_id IN ('12') and gt.disposal_status IN('12') and ulbid='" . $_SESSION['ulbid'] . "' and gt.disposal_status NOT IN('5','9','13')";
	...
	$rs = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($rs)) {
		$data_list[$row['emp_dept']][$row['ward_id']]['reopend_completed'] += $row['count'];
	...
	$tpl->display('corp_grievance_analysis_dept_zone.tpl');
