   ...
   require "config.php";
	...
	if(isset($_SESSION['uid']))
	{
	    ...
	    if(isset($_POST['search']))
	    {
    		 ...
    		 {
    		     $sql="select g.grievance_id,person_name,email,hno,address,g.ward_id,ward_desc,street_id,mobile,comp_subject,
    		  comp_desc,grievance_origin_id,grievance_status_id,date_regd,cat3_id,file_no,app_type_id,emp_id,dept_id 
    		  from grievances g left join grievances_transactions gt on g.grievance_id=gt.grievance_id left join ward_mst w on g.ward_id=w.ward_id where  person_name like '%".$_POST['applicant_name']."%' and mobile like '%".$_POST['mobile']."%' and app_type_id='1'";
    		 }
    		 else
    		 { 
    		    $sql="select g.grievance_id,person_name,email,hno,address,g.ward_id,ward_desc,street_id,mobile,comp_subject,
    		  comp_desc,grievance_origin_id,grievance_status_id,date_regd,cat3_id,file_no,app_type_id,emp_id,dept_id 
    		  from grievances g left join grievances_transactions gt on g.grievance_id=gt.grievance_id left join ward_mst w on g.ward_id=w.ward_id where g.grievance_id like '".$_POST['ref_no']."'
    ";
    		 }
        ...
        $sql =$conn->prepare("select emp_id,emp_name,emp_mobile from emp_mst");
        $sql->execute();
        $rs = $sql->get_result();
        while($row = $rs->fetch_assoc())
        {
	    ...
	    {
	        $tpl->assign('msg',$_SESSION['message']);
		...
		$tpl->display('search_grievances.tpl');
	}
	else
	{
