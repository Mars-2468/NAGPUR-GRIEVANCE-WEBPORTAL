<?php
	...
	if(isset($_SESSION['uid']))
	{
		...
		$sql2 ="SELECT emp_dept,emp.emp_id FROM `emp_map` emp, emp_mst e WHERE emp.emp_id=e.emp_id and `emp_id2` LIKE '".$_SESSION['emp_id']."' group by emp.emp_id";
		$sql3 ="SELECT emp_dept,emp.emp_id2 FROM `emp_map` emp, emp_mst e WHERE emp.emp_id2=e.emp_id and `emp_id3` LIKE '".$_SESSION['emp_id']."' group by emp.emp_id2";
		$sql4 ="SELECT emp_dept,emp.emp_id3 FROM `emp_map` emp, emp_mst e WHERE emp.emp_id3=e.emp_id and `emp_id4` LIKE '".$_SESSION['emp_id']."' group by emp.emp_id3";
		 ...
		 $sql ="select COUNT(DISTINCT gt.grievance_id) as count,dept_id as emp_dept,disposal_status,sla_status from grievances g, grievances_transactions gt,emp_mst e where 
		  ...
		  if(isset($_POST['search']))
          {
	            ...
	            $t_date = date('Y-m-d',strtotime($_POST['t_date']));
			       ...
			       $sql.=" and e.emp_dept in ('$ids') and gt.emp_id in('$empids') group by gt.dept_id,gt.disposal_status,g.sla_status";
			  ...
			  while($row = mysqli_fetch_assoc($rs))
			    {
					...
					$data[$row['emp_dept']][$row['disposal_status']][$row['sla_status']]['count']+=$row['count'];
	       ...
	       $sql="select dept_id,dept_desc from dept_mst where ulbid='".$_SESSION['ulbid']."' and dept_id in ('$ids') ";
	...
	$tpl->assign('user_type',$_SESSION['user_type']);
		...
		$tpl->display('level_dashboard.tpl');
