...
if (isset($_SESSION['uid'])) {
	...
	/*01-04-24 $tpl->assign('app_type_id', $app_type_id);
	...
	if ($_REQUEST['status'] == 1 || $_REQUEST['status'] == 109) {
		$sql = "select g.*,gt.emp_id from grievances g, grievances_transactions gt where g.grievance_id=gt.grievance_id  and date_regd >= DATE_SUB(CURDATE(), INTERVAL 1 WEEK) 
  		AND date_regd <= CURDATE()";

		/*01-04-24 $sqlExcel = "select g.grievance_id as ReferenceNo,person_name as ApplicantName, mobile as Mobile,address as Address,comp_subject as Description,comp_desc,date_regd as ComplaintDate from grievances g where date_regd >= DATE_SUB(CURDATE(), INTERVAL 1 WEEK) 
		AND date_regd <= CURDATE()";*/
	...
	} else if ($_REQUEST['status'] == 2 || $_REQUEST['status'] == 110) {
		$sql = "select g.*,gt.emp_id from grievances g, grievances_transactions gt where g.grievance_id=gt.grievance_id  and date_regd <= DATE_SUB(CURDATE(), INTERVAL 30 DAY)";

		/*01-04-24 $sqlExcel = "select g.grievance_id as ReferenceNo,person_name as ApplicantName, mobile as Mobile,address as Address,comp_subject as Description,comp_desc,date_regd as ComplaintDate from grievances g where date_regd <= DATE_SUB(CURDATE(), INTERVAL 30 DAY) ";*/
	...
	if ($_REQUEST['status'] == 1 || $_REQUEST['status'] == 109) {
		...
		/*01-04-24 if (isset($_POST['search'])) 
		{
			if ($_POST['reference_no'] != '') 
			{
	...
	/*21-03-24 $targetpage = "grievance_month_inner_report.php"; 	//your file name  (the name of this file)
	...
	/*01-04-24 $tpl->assign('fdate', $_POST['f_date']);
	...
	$tpl->display('grievance_month_inner_report.tpl');
} else {
