...
require "config.php";
	...
	if(isset($_SESSION['uid']))
	{
		...
		$ulbid=isset($_REQUEST['ulbid'])?$_REQUEST['ulbid']:'';
		$page=isset($_REQUEST['page'])?$_REQUEST['page']:'';
		$app_type_id=isset($_REQUEST['app_type_id'])?$_REQUEST['app_type_id']:'';
		$ward_id=isset($_REQUEST['ward_id'])?$_REQUEST['ward_id']:'';
...
if ($query->execute()) {
    ...
    while ($row = $rs->fetch_assoc()) {
        ...
        $emp_list[$row['emp_id']] = [
            ...
            'emp_dept'   => isset($dept_list[$row['emp_dept']]) ? $dept_list[$row['emp_dept']] : '',
            'emp_desg'   => isset($desg_list[$row['emp_desg']]) ? $desg_list[$row['emp_desg']] : ''
        ];
    }
...
$query->close();		
 ...
 //echo "<pre>";print_r($emp_list);echo "</pre>";die();		
		...
		$tpl->display('corp_grievance_view_comp_det_admin.tpl');
	}
	else
	{
