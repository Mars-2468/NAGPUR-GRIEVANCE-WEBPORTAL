...
// exit;
	...
	// get the form data start
	
	
    // Sanitize inputs
    $person_name = trim(htmlspecialchars($_POST['person_name']));
    $mobile      = trim($_POST['mobile']);
    $email       = trim($_POST['email']);
    $hno         = trim(htmlspecialchars($_POST['hno']));
    ...
    if (!preg_match("/^[a-zA-Z ]{3,50}$/", $person_name)) {
        $errors['person_name'] = "Invalid name (letters and spaces only).";
    }
    ...
    if (strlen($hno) > 50) {
        $errors['hno'] = "House number / address too large.";
    }

    if (strlen($address) > 100) {
        $errors['address'] = "Address too large.";
    }

    if (strcasecmp($captcha, $_SESSION['captcha']) != 0) {
        $errors['captcha'] = "Incorrect CAPTCHA entered.";
    }

  // echo "<pre>";print_r($errors);echo "</pre>";die();
	
	
	// get the form data end
	
	if (count($errors) ) {
        $_SESSION[ 'register_form' ] = $errors;
        header( "Location: web_register_form.php?id=250" );
		exit;
    }
    ...
    //   $sql = "INSERT INTO `users_test`( `user_id`,  `user_name`, `user_mobile`, `user_email`, `user_type`,  `ulbid`, `login_status`,  `created_by`, `created_at`, `otp`, `emailOtp`, `otpdatetime`, `otpexpiretime`, `otp_status`,`house_no`,`address`) VALUES ('" . $_POST[ 'mobile' ] . "','" . $_POST[ 'person_name' ] . "','" . $_POST[ 'mobile' ] . "','" . $_POST[ 'email' ] . "','D','" . $ulb_id . "','1','1','" . date('Y-m-d H:i:s', strtotime($stamp)) . "','" . $otp . "','" . $otp  . "','" . date('Y-m-d H:i:s', strtotime($stamp)) . "','" . $otpExpiryDate . "','1','" . $_POST[ 'hno' ] . "','" . $_POST[ 'address' ] . "')";  
//mysqli_query( $conn, $sql );
     $sql = "select * from users_test where user_id='" . $_POST[ 'mobile' ]. "' "; 
    ...
    if (mysqli_num_rows($rs)>0 ) {
      //  echo "gg"; exit;
					...
					/* 	$message =str_replace ( ' ', '%20', $sms);
						...
						$sql = "insert into user_otp_logs(
										...
										otp_status
											...
											'" . $otp . "',
											'" . date('Y-m-d H:i:s') . "',
											'" . date('Y-m-d H:i:s', strtotime('+5 minutes', strtotime(($stamp)))) . "',
											'0'
											)";
						mysqli_query($conn, $sql);
		...
		$_SESSION['flash_message'] = "Already existing user enter your OTP to complaint"; 
        ...
        $indexpage = 'complaint_form.php';
        //header( "location:$indexpage" );
		$_SESSION['flash_message'] = "Register Success";
		$_SESSION['user_mobile'] = $_POST[ 'mobile' ] ;
        echo "<script>window.location='$indexpage';</script>";
    } 
    // else{
	...
	// 	if($_POST['reg_mob']!="") {
    //     $indexpage = 'web_register_form.php?id=250&mobile='.$_POST['reg_mob'];
	// 	}else{
	// 	$indexpage = 'web_register_form.php?id=250';
