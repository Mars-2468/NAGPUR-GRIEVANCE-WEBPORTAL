...
require "config.php";
	...
	if(isset($_SESSION['uid']))
	{
		...
		$current_category_name=$row['cs_desc'];
		 ...
		 $sql="SELECT count(grievance_id) as count,ulbid FROM `grievances` WHERE   (grievance_status_id =? or grievance_status_id =? or grievance_status_id =? or grievance_status_id =? or grievance_status_id =? or grievance_status_id =? or grievance_status_id =? ) and cat3_id !=? and app_type_id=? group by ulbid";
		    ...
		    $grievance_status_id10='10';
		    $grievance_status_id11='12';
		    $cat3_id='0';
		    $app_type_id='1';
		    
		      $query=$conn->prepare($sql);
		      $query->bind_param("iiiiiiiii",htmlspecialchars(strip_tags($grievance_status_id1)),htmlspecialchars(strip_tags($grievance_status_id2)),htmlspecialchars(strip_tags($grievance_status_id3)),htmlspecialchars(strip_tags($grievance_status_id4)),htmlspecialchars(strip_tags($grievance_status_id6)),htmlspecialchars(strip_tags($grievance_status_id10)),htmlspecialchars(strip_tags($grievance_status_id11)),htmlspecialchars(strip_tags($cat3_id)),htmlspecialchars(strip_tags($app_type_id)));
    			if(!$query->execute())
    	        {
    	            echo "Query not executed 1";
    	        }
		...
		while($row = $rs->fetch_assoc())
		{
	     ...
	     $total_days_services='';
		    ...
		    $query=$conn->prepare($sql3);
		      $query->bind_param("siiiiiiiiii",htmlspecialchars(strip_tags($ulbid)),htmlspecialchars(strip_tags($response_time1)),htmlspecialchars(strip_tags($grievance_status_id1)),htmlspecialchars(strip_tags($grievance_status_id2)),htmlspecialchars(strip_tags($grievance_status_id3)),htmlspecialchars(strip_tags($grievance_status_id4)),htmlspecialchars(strip_tags($grievance_status_id6)),htmlspecialchars(strip_tags($grievance_status_id10)),htmlspecialchars(strip_tags($grievance_status_id11)),htmlspecialchars(strip_tags($cat3_id)),htmlspecialchars(strip_tags($app_type_id)));
    			if(!$query->execute())
    	        {
	...
	$query->close();
		...
		$tpl->display('ulb_complaints_responsetime.tpl');
	}
	else
	{
