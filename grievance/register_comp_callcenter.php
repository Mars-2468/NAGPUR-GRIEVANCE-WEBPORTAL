...
//	$sql="select emp_id,emp_name,emp_dept,emp_desg,emp_mobile from emp_mst where ulbid='".mysqli_real_escape_string($conn,strip_tags($_SESSION['ulbid']))."'";
	...
	$sql=$conn->prepare("select emp_id,emp_name,emp_dept,emp_desg,emp_mobile from emp_mst where ulbid=?");
	$sql->bind_param("s",$_POST['selulbid']);
	...
	$sql=$conn->prepare("select emp_id,emp_name,emp_dept,emp_desg,emp_mobile from emp_mst_od where ulbid=?");
	$sql->bind_param("s",$_POST['selulbid']);
	 ...
	 $sql ="select cs_id,comp_desc from category3_mst where ulbid='".mysqli_real_escape_string($conn,strip_tags($_SESSION['ulbid']))."'";
		...
		$sql ="select grievance_id from grievances where 
			...
			else
			{
			 ...
			 $sql="insert into grievances(app_type_id,person_name,email,hno,address,ward_id,street_id,mobile,altmobile,comp_subject,comp_desc,
			 ...
			 endorsement,mcat3_id,isVillageInUlb,distid,mandalid,villageid) 
			 values(
			 ...
			 '".mysqli_real_escape_string($conn,htmlspecialchars($_POST['altmobile']))."',
			 ...
			 '1','".mysqli_real_escape_string($conn,htmlspecialchars($_POST['selulbid']))."',
			...
			if(mysqli_query($conn,$sql))
			{
			  ...
			  //Start of ULB response time report
			       ...
			       $ulbid='';$cat3_id='';$grievance_status_id='';$date_regd='';$disposed_date='';
	                	    	...
	                	    	$sql="SELECT grievance_id,ulbid,cat3_id,grievance_status_id,app_type_id FROM `grievances` g where grievance_id=".mysqli_real_escape_string($conn,strip_tags($grievance_id));
	                	    	//	echo $sql;exit;
	                	    	$rs=mysqli_query($conn,$sql);
		...
		if(is_uploaded_file($_FILES['f1']['tmp_name']))
			...
			$sql ="select u.address as api_address,ulb_type_desc,api_ulbname,lat,lng,ulbname from ulb_type ut,ulbmst u where ut.ulb_type_id=u.ulb_type and u.ulbid='".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."'";
					$rs = mysqli_query($conn,$sql);
		    ...
		    if($_POST['emp_id'] == '22222222222' || $_POST['emp_id'] == '')
		    {
        			...
        			if($_POST['app_type_id']=='1' && $_POST['dept_id']=='3' && $_POST['selulbid']=='052')
        			{
        			     $sql1="SELECT emp_id1 as emp_id  FROM water_tanker_emp_map WHERE  water_tank_id ='".mysqli_real_escape_string($conn,strip_tags($_POST['tanker_id']))."'  and ulbid='".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."'";
        			}
        			else
        			{
        		         $sql1="SELECT cs_id,emp_id,dept_id  FROM emp_map WHERE  cs_id ='".mysqli_real_escape_string($conn,strip_tags($_POST['cs_id']))."' and ward_id='".mysqli_real_escape_string($conn,strip_tags($_POST['ward_id']))."' and ulbid='".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."' and flag='1' and cs_type_id='".mysqli_real_escape_string($conn,strip_tags($_POST['app_type_id']))."' and street_id='".mysqli_real_escape_string($conn,strip_tags($_POST['street_id']))."'";
        			}
		    ...
		    else
		    {
		        ...
		        {
		        	$sql1="SELECT emp_id,emp_dept as dept_id,emp_desg  FROM emp_mst WHERE  emp_id='".mysqli_real_escape_string($conn,strip_tags($_POST['emp_id']))."'";

		        }
		        else
		        {
		             $sql1="SELECT emp_id,emp_dept as dept_id,emp_desg  FROM emp_mst_od WHERE  emp_id='".mysqli_real_escape_string($conn,strip_tags($_POST['emp_id']))."'";
		  ...
		  file_put_contents('querytxt.txt',$sql1);
			 ...
			 if($nr > 0)
			 {
			   ...
			   if($_POST['emp_id'] == '22222222222' || $_POST['emp_id'] == '')
				...
				if(mysqli_query($conn,$sql2))
				{
				    ...
				    $sql ="select under_progress_sla from dashboard_count where app_type_id='".mysqli_real_escape_string($conn,strip_tags($_POST['app_type_id']))."' and ulbid='".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."'";
				    $rs = mysqli_query($conn,$sql);
				    $rows = mysqli_fetch_assoc($rs);
				    $under_progress_sla=$rows['under_progress_sla']+1;
				    
				    $sql ="update dashboard_count set under_progress_sla='".$under_progress_sla."' where app_type_id='".mysqli_real_escape_string($conn,strip_tags($_POST['app_type_id']))."' and ulbid='".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."'";
				    mysqli_query($conn,$sql);
        			...
        			$sql3 = "select (".$field_name.") as under_sla from complaints_cat_count where ulbid = '".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."' and status = '5' " ;
        			$rs3 = mysqli_query($conn,$sql3);
        			$rows3 = mysqli_fetch_assoc($rs3);
        			$under_sla = $rows3['under_sla']+1;
        			
        			$qry1 = "update complaints_cat_count set ".$field_name." = ".$under_sla." where ulbid = '".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."' and status = '5' " ;
        		...
        		if($old_complaint_check_id != 1){			 
        		    $sms="Dear ".$emp_name_list[$row1['emp_id']].", A ".$aaa." from ".mysqli_real_escape_string($conn,strip_tags($_POST['person_name'])).",".mysqli_real_escape_string($conn,strip_tags($_POST['hno'])).", ward ".$ward_list[$_POST['ward_id']].",".mysqli_real_escape_string($conn,strip_tags($_POST['address'])).",Mobile No.".mysqli_real_escape_string($conn,strip_tags($_POST['mobile'])).", regarding ".$subject." was alloted to you on ".date('d-m-Y',strtotime($today))." Regards - Grievance Monitoring Cell , ".$ulb_info['ulbname']." ".$ulb_info['ulb_type_desc'];
...
//$sms1="Drst ".$_POST['person_name']." ,Your Service registered ref no ".$grievance_id.", regarding ".$_POST['comp_subject'].",".$_POST['comp_desc'].",";
		        ...
		        if($old_complaint_check_id != 1){
		            $sms1 = "Dear ".$_POST['person_name']." , Your ".$aaa." regarding ".$subject." registered ref no ".$grievance_id." Status is under progress , Regards - Citizen Service Monitoring Cell , ".$ulb_info['ulbname']." ";			  
					
					$sms1.=" Your ".$aaa." Will be completed on  ".date('d-m-Y',strtotime($_POST['cut_off_time'])).",";
						//$sms1.=" Your ".$aaa." Will be completed on  ".date('d-m-Y',strtotime($_POST['cut_off_time'])).",";
					...
					$sms1.=" Responsible officer  ".$emp_name_list[$row1['emp_id']].", MOBILE ".$mobile." -CMNR ".$ulb_info['ulbname']." ".$ulb_info['ulb_type_desc'];
		 	 ...
		 	 $sql ="select pending_for_approval from dashboard_count where app_type_id='".mysqli_real_escape_string($conn,strip_tags($_POST['app_type_id']))."' and ulbid='".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."'";
				    $rs = mysqli_query($conn,$sql);
				    $rows = mysqli_fetch_assoc($rs);
				    $pending_for_approval=$rows['pending_for_approval']+1;
				    
				   $sql ="update dashboard_count set pending_for_approval='".$pending_for_approval."' where app_type_id='".mysqli_real_escape_string($conn,strip_tags($_POST['app_type_id']))."' and ulbid='".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."'";
				    mysqli_query($conn,$sql);
			...
			$sql21 = "select (".$field_name.") as pending_apprvl from complaints_cat_count where ulbid = '".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."' and status = '2' " ;
			$rs21 = mysqli_query($conn,$sql21);
			$rows2 = mysqli_fetch_assoc($rs21);
			$pending_apprvl = $rows2['pending_apprvl']+1;
			
			$qry = "update complaints_cat_count set ".$field_name." = ".$pending_apprvl." where ulbid = '".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."' and status = '2' " ;
			$res = mysqli_query($conn,$qry);	    
				...
				$sms="Dear ".$_POST['person_name'].", Thank you for using online Grievance Redressal system. Your ".$aaa." has been successfully registered with reference number : ".$grievance_id.". regarding ".$subject.",".$_POST['comp_desc'].", Regards - ".$ulb_info['ulbname']." ".$ulb_info['ulb_type_desc'];

				$sms1="Dear ".$admin_name.", A ".$aaa." was registered by ".mysqli_real_escape_string($conn,strip_tags($_POST['person_name'])).", ".mysqli_real_escape_string($conn,strip_tags($_POST['hno'])).", ward ".$ward_list[$_POST['ward_id']].",".mysqli_real_escape_string($conn,strip_tags($_POST['address'])).",Mobile No.".mysqli_real_escape_string($conn,strip_tags($_POST['mobile'])).", regarding ".$subject.", through ".$grievance_origin_list[$_POST['grievance_origin_id']]." with reference number : ".$grievance_id.". Regards - ".$ulb_info['ulbname']." ".$ulb_info['ulb_type_desc'];	
			 ...
			 $sql ="select received from dashboard_count where app_type_id='".mysqli_real_escape_string($conn,strip_tags($_POST['app_type_id']))."' and ulbid='".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."'";
				    $rs = mysqli_query($conn,$sql);
				    $rows = mysqli_fetch_assoc($rs);
				    $received=$rows['received']+1;
				    
				    $sql ="update dashboard_count set received='".$received."' where app_type_id='".mysqli_real_escape_string($conn,strip_tags($_POST['app_type_id']))."' and ulbid='".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."'";
				    mysqli_query($conn,$sql);
			...
			$sql21 = "select (".$field_name.") as received from complaints_cat_count where ulbid = '".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."' and status = '1' " ;
			$rs21 = mysqli_query($conn,$sql21);
			$rows2 = mysqli_fetch_assoc($rs21);
			$received = $rows2['received']+1;
			
			$qry = "update complaints_cat_count set ".$field_name." = ".$received." where ulbid = '".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."' and status = '1' " ;
			$res = mysqli_query($conn,$qry);
		...
		$sql="select cat_id,description  from category_mst where ulbid='".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."' order by description ";
		if($rs=mysqli_query($conn,$sql))
		{
			while($row = mysqli_fetch_assoc($rs))
			...
			$sql ="SELECT * FROM `ulb_online_application_map` where ulbid='".mysqli_real_escape_string($conn,strip_tags($_POST['selulbid']))."'";
		$rs =mysqli_query($conn,$sql);
		while($row = mysqli_fetch_assoc($rs))
		{
	...
	//	print_r($online_applications);
	  ...
	  $sql ="select COUNT(id) as user_count from login_details where type='1' and ulbid like '%".$_POST['selulbid']."%'"; 
	      $rs = mysqli_query($conn,$sql);
	      $row = mysqli_fetch_assoc($rs);
