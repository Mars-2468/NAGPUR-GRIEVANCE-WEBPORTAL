...
if (isset($_SESSION['uid'])) {
	...
	$prev_aptid=$_POST['prev_aptid'];
	$prev_sla=$_POST['prev_sla'];
	$prev_status=$_POST['prev_status'];
	$prev_user_type=$_POST['prev_user_type'];
	$prev_f_date=$_POST['prev_f_date'];
	$prev_t_date=$_POST['prev_t_date'];
	
	$tpl->assign('prev_aptid', $prev_aptid);
	$tpl->assign('prev_status', $prev_status);
	$tpl->assign('prev_sla', $prev_sla);
	$tpl->assign('prev_user_type', $prev_user_type);
	$tpl->assign('prev_f_date', $prev_f_date);
	$tpl->assign('prev_t_date', $prev_t_date);
...
//echo "<pre>";print_r($_POST);echo "</pre>";die();
	...
	try {
		...
		if (isset($_POST['save'])) {
			...
			$sel_date = $_POST['disposed_date'] . " " . date('H:i:s');
			...
			//old code change date 29-04-24 $disposal_remarks = $_POST['disposal_remarks'];
			//$dis_remark = str_replace("'", "", $_POST['disposal_remarks']);
			$dis_remark = trim(strip_tags($_POST['disposal_remarks']));
			
		/* 	$disposal_remarks = sanitize_input($dis_remark);//$dis_remark;
			$rca =  sanitize_input($dis_remark);//$_POST['rca'];
			$ca =  sanitize_input($dis_remark);//$_POST['ca'];  */
			
			//echo "<pre>";print_r($_POST);echo "</pre>";die();
			
			$disposal_remarks = $dis_remark;//$dis_remark;
			$rca =  $dis_remark;//$_POST['rca'];
			$ca =  $dis_remark;//$_POST['ca'];
			...
			echo "<script>alert('Updated Successfully..!'); window.location='tot_received.php?aptid={$prev_aptid}&status={$prev_status}&sla={$prev_sla}&user_type={$prev_user_type}';</script>";
		}
	} catch (Exception $e) {
		return $e->getMessage();
	}
	...
	if (isset($_POST['grievance_id'])) {
		...
		//22-05-24 $sql = "select transaction_id,emp_id,alloted_date,disposed_date,disposal_status,disposal_remarks,app_type_id,rca,ca from grievances g, grievances_transactions gt where  g.grievance_id=gt.grievance_id and gt.grievance_id='" . $_POST['grievance_id'] . "' and disposal_status IN('2','11','6') order by transaction_id";
...
//echo "<pre>";print_r($data2);echo "</pre>";die();
	...
	$tpl->display('manage_transfered_comp_sel.tpl');
} else {
