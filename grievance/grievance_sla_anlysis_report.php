...
if (isset($_SESSION['uid'])) {
	...
	$sql = "SELECT count(`grievance_id`) as count,cat3_id FROM `grievances` group by `cat3_id`";
	$rs = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($rs)) {
		$data['complaints'][$row['cat3_id']]['count'] = $row['count'];
	}

	$sql = "SELECT count(`grievance_id`) as count,cat3_id ,grievance_at_emp_level FROM `grievances` group by `cat3_id`,grievance_at_emp_level";
	$rs = mysqli_query($conn, $sql);
	while ($row = mysqli_fetch_assoc($rs)) {
		$data['level'][$row['cat3_id']][$row['grievance_at_emp_level']]['count'] = $row['count'];
	}
