...
//echo "<pre>";print_r($_SESSION);echo "</pre>";die();
	...
	if ($_REQUEST['status'] == 'war') {
		if ($user_type == 'U') {
			$query = "SELECT COUNT(DISTINCT sc.warning_id) as num,e.emp_name,d.dept_desc,sc.emp_id from show_case_response_logs sc,emp_mst e,dept_mst d,grievances g where e.emp_id=sc.emp_id and e.emp_dept = d.dept_id and sc.grievance_id = g.grievance_id and g.ulbid='" . $_SESSION['ulbid'] . "' and sc.emp_id = '" . $_REQUEST['emp_id'] . "' ";
		} else {
			$query = "SELECT COUNT(DISTINCT sc.warning_id) as num,e.emp_name,d.dept_desc,sc.emp_id from show_case_response_logs sc,emp_mst e,dept_mst d,grievances g where e.emp_id=sc.emp_id and e.emp_dept = d.dept_id and sc.grievance_id = g.grievance_id and g.ulbid='" . $_SESSION['ulbid'] . "' and sc.emp_id = '" . $_SESSION['emp_id'] . "' ";
		}
	}

 	if($_REQUEST['f_date']!= '' && $_REQUEST['t_date']!='')
	{
		$query .= " and date(sc.datetime) between '" . $f_date . "' and '" . $t_date . "'  and date_format(sc.date_regd, '%Y-%m-%d') >= '2024-09-01'  ";
	}else{
		$query .= "   and date_format(sc.date_regd, '%Y-%m-%d') >= '2024-09-01' ";				
	} 
	
	if (isset($_POST['search'])) {
		if ($_POST['warning_no'] != '') {
			$query .= " and sc.warning_id='" . $_POST['warning_no'] . "' ";
		}
		if ($_POST['f_date'] != '' && $_POST['t_date'] != '') {
			$f_date = date('Y-m-d', strtotime($_POST['f_date']));
			$t_date = date('Y-m-d', strtotime($_POST['t_date']));
