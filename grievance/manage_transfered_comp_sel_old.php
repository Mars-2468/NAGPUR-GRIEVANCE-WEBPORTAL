					...
					try {
						if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_dir . '/' . $newfilename)) {
							...
							$file_url = $target_dir . '/' . $newfilename;
								...
								$sql = "update grievances_transactions set file_url='" . $file_url . "' where grievance_id='" . $_POST['grievance_id'] . "' ";
							
								$uploadStatus=mysqli_query($conn, $sql);
						}
					} catch (Exception $ex) {
						die($ex->getMessage);
					}
	...
	if (isset($_POST['grievance_id'])) {
		...
		if ($rs = mysqli_query($conn, $sql)) {
			...
			while ($row = mysqli_fetch_assoc($rs)) {
				...
				$data2[$row['transaction_id']]['emp_id'] = $emp_list[$row['emp_id']]['emp_id'];
				...
				$dept_id = $data2[$row['transaction_id']]['emp_dept'];
...
//echo "<pre>";print_r($data2);echo "</pre>";die();	
		...
		$sql = "select distinct cat_id,description from category_mst  where ulbid='" . $_SESSION['ulbid'] . "'";

		if ($rs = mysqli_query($conn, $sql)) {

			while ($row = mysqli_fetch_assoc($rs))
				$dept_list_all[$row['cat_id']] = $row['description'];
		} else

			printf("Errormessage: %s\n", mysqli_error($conn));	
		
		$sql = "select cs_id,cat_id from complaint_ulbmap where ulbid='" . $_SESSION['ulbid'] . "' and cs_id = '" . $data1['cat3_id'] . "'";

		if ($rs = mysqli_query($conn, $sql)) {

			while ($row = mysqli_fetch_assoc($rs)){
				$cs_mst_list[$row['cs_id']] = $row['cat_id'];
				$dept_list[$cs_mst_list[$row['cs_id']]] = $dept_list_all[$cs_mst_list[$row['cs_id']]];
			}
...
//13,11,12
	...
	//echo"<pre>"; print_r($cs_data[$data1['cat3_id']]);echo"</pre>";die();
	//echo"<pre>"; print_r($data1['cat3_id']);echo"</pre>";die();
	//echo"<pre>"; print_r($cat_list);echo"</pre>";die();
	//echo"<pre>"; print_r($cs_list['Electrical Department '][1]);echo"</pre>";die();

	$sql = "SELECT emp_id,user_id FROM  `users` ";
		$rs = mysqli_query($conn, $sql);
		while ($row = mysqli_fetch_assoc($rs)) {
			$user_id_list[$row['emp_id']] = $row['user_id'];
		}


//echo"<pre>"; print_r($data2);echo"</pre>";die();

	$tpl->assign('user_id_list', $user_id_list);
	...
	$tpl->assign('dept_list_all', $dept_list_all);
