...
$base_url = "https://tinyurl.com/2bfw2y96";
echo "<br>";
...
echo $sql = "SELECT * FROM `grievances` WHERE `app_type_id` LIKE '1' AND `grievance_status_id` LIKE '2' and sla_status = 2 and is_test_done = '0' and grievance_id='14182'";
	...
	if ($row['grievance_at_emp_level'] == 'L1') {
		...
		if ($disposed_date < $todayDate) {
			...
			'1'
				...
				if (mysqli_query($conn, $sql)) {
					// send sms to L1 employee
					$sms = "Escalation Notice: Citizen grievance ". $row['grievance_id'] ." regarding " . substr($cs_list[$row['cat3_id']], 0, 29) . " moved to " . substr($emp2['emp_name'], 0, 29) . " due to unsatisfactory response. Please take immediate corrective action. Regards: NMCGOV";
					$mobile = $emp1['emp_mobile'];
					//$mobile = '8484972066';
					// $templateId = "1707166305992205791";
					//01-08-2024 Dhanraj $templateId = "1707167653156033634";
					$templateId = "1707172181803415109";
		...
		echo $sql = "SELECT * FROM `grievances_transactions` WHERE `grievance_id` = '" . $grievances_id . "' and disposal_status=2 and emp_id='" . $emp1['emp_id2'] . "'";
		$rs2 = mysqli_query($conn, $sql);
		$preveious_employee_allotteddate = mysqli_fetch_assoc($rs2);
		// get allotted date 
		...
		if ($disposed_date < $todayDate) {
			...
			if (mysqli_query($conn, $sql)) {
				...
				if (mysqli_query($conn, $sql)) {
					...
					// send Warning sms to L2 employee
					$sms = "Escalation Notice: Citizen grievance ". $row['grievance_id'] ." regarding " . substr($cs_list[$row['cat3_id']], 0, 29) . " moved to " . substr($emp2['emp_name'], 0, 29) . " due to unsatisfactory response. Please take immediate corrective action. Regards: NMCGOV";
					$mobile = $emp1['emp_mobile'];
					//$mobile = '7020442916';
					// $templateId = "1707166305992205791";
					...
					$templateId = "1707172181803415109";
					$message = str_replace(' ', '%20', $sms);
					?><br><? echo $url = "http://smsatm.net/v3/api.php?username=nmcgov&apikey=fe4ba54cf34bec238813&senderid=NMCGov&templateid=" . $templateId . "&mobile=" . $mobile . "&message=" . $message;
					//require_once('aurangabad_sms_config.php');
					$post = curl_init();
					curl_setopt($post, CURLOPT_SSL_VERIFYPEER, false);
					curl_setopt($post, CURLOPT_URL, $url);
		...
		if ($disposed_date < $todayDate) {
			...
			'1'
				...
				if (mysqli_query($conn, $sql)) {
					/*********sms strarts**************/
					// send Warning sms to L3 employee
					$sms = "Escalation Notice: Citizen grievance ". $row['grievance_id'] ." regarding " . substr($cs_list[$row['cat3_id']], 0, 29) . " moved to " . substr($emp2['emp_name'], 0, 29) . " due to unsatisfactory response. Please take immediate corrective action. Regards: NMCGOV";
					$mobile = $emp1['emp_mobile'];
					//$mobile = '8484972066';
					// $templateId = "1707166305992205791";
					...
					$alloted_date = date('d-m-Y H:i:s', strtotime($preveious_employee_allotteddate['alloted_date']));
					//18-07-2024 $sms = "Respected Sir, Complaint from " . substr($row['person_name'], 0, 29) . ", Mobile No " . $row['mobile'] . ", regarding " . substr($cs_list[$row['cat3_id']], 0, 29) . " with Ref No : " . $row['grievance_id'] . " was allotted to " . substr($emp1['emp_name'], 0, 29) . " on " . date('d-m-Y H:i:s', strtotime($preveious_employee_allotteddate['alloted_date'])) . " is not resolved https://www.nmcnagpur.gov.in//grievance/ Regards- Citizen Service Monitoring Cell, NMCGOV";
					
					/*$sms = "Respected Sir, Complaint from " . substr($row['person_name'], 0, 29) . ", Mobile No " . $row['mobile'] . ", regarding " . substr($cs_list[$row['cat3_id']], 0, 29) . " with Ref No : " . $row['grievance_id'] . " was allotted to " . substr($emp1['emp_name'], 0, 29) . " on " . $alloted_date . " is not resolved  " . $base_url . "  Regards- Citizen Service Monitoring Cell, NMCGOV";*/
					$sms = "Dear " . substr($emp2['emp_name'], 0, 29) . ", Complaint from " . substr($row['person_name'], 0, 29) . ", Mobile No " . $row['mobile'] . " , regarding " . substr($cs_list[$row['cat3_id']], 0, 29) . " with Ref No : " . $row['grievance_id'] . " was allotted to " . substr($emp1['emp_name'], 0, 29) . " on " . $alloted_date . " is not resolved " . $base_url . " Regards- Citizen Service Monitoring Cell, NMCGOV";
					$mobile = $emp2['emp_mobile'];
					//$mobile = '7020442916';
