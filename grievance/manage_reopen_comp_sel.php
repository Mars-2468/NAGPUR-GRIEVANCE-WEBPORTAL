...
//echo "<pre>";print_r($sql);echo "</pre>";die();
	...
	while ($row = mysqli_fetch_assoc($rs)) {
		...
		$cat3_list[$row['cs_id']] = $row['comp_desc'];
			...
			$sql = "select * from grievance_status_mst where grievance_status_id IN(10,11,12,13)";
		...
		if (isset($_POST['save'])) {
			...
			if (!empty($_FILES["fileToUpload"]["name"])) {
				...
				if ($uploadOk == 0) {
					...
					$newfilename = $temp[0] . '_' . $_REQUEST['grievance_id'] . '_' . date('Y') . date('m') . date('d') . date('h') . date('i') . date('s') . '.' . end($temp);
...
//echo "<pre>";print_r();echo"</pre>";die();
			...
			if (in_array($disposal_status,[10])){
				...
				//$curstatus = $rws['grievance_status_id'];
				
				// $previous_status = $rws['grievance_status_id'];

				
				if ($disposal_status ==10) {
					$curstatus = 11;
				}
...
//echo"<pre>";print_r($grievance_status_list);echo"</pre>";die();
	...
	if (isset($_REQUEST['grievance_id'])) {
		...
		$sql = "select app_type_id,mcat3_id,cat3_id,app_type_id,person_name,email,hno,address,ward_id,street_id,mobile,comp_subject,comp_desc,grievance_origin_id,grievance_status_id,date_regd,grievance_at_emp_level,endorsement from grievances where grievance_id='" . $_REQUEST['grievance_id'] . "' and ulbid='" . $_SESSION['ulbid'] . "'";
...
//echo"<pre>";print_r($data1['grievance_status_id']);echo"</pre>";die();
		//22-05-24 $sql = "select transaction_id,emp_id,alloted_date,disposed_date,disposal_status,disposal_remarks,app_type_id,rca,ca from grievances g, ".$_SESSION['grievances_trns']." gt where  g.grievance_id=gt.grievance_id and gt.grievance_id='" . $_POST['grievance_id'] . "' and disposal_status IN('2','11','6') order by transaction_id";

		

		$sql = "select transaction_id,emp_id,DATE_FORMAT(alloted_date, '%Y-%m-%d') AS alloted_date,gt.disposed_date,gt.disposal_status,gt.disposal_remarks,app_type_id,rca,ca,gt.dept_id,g.grievance_at_emp_level,g.comp_desc,g.comp_subject from grievances g, ".$_SESSION['grievances_trns']." gt where  g.grievance_id=gt.grievance_id and gt.grievance_id='" . $_REQUEST['grievance_id'] . "' and disposal_status IN(".$data1['grievance_status_id'].") order by transaction_id DESC LIMIT 1"; // modified on 19-07-2024  added DESC LIMIT 1
		...
		$tpl->assign('grievance_id', $_REQUEST['grievance_id']);
		...
		if ($app_type_id == '1') {
			...
			$sql = "select grievance_status_id from grievances where grievance_id='" . $_REQUEST['grievance_id'] . "'";
			...
			if ($status == '11' && in_array($data1['grievance_at_emp_level'],['L1','L2','L3','L4'])) {
				
//13,11,12
				$sql_status = "select grievance_status_id,grievance_status_desc from grievance_status_mst where grievance_status_id IN('10','12')";
			
			}else if ($status == '13' && in_array($data1['grievance_at_emp_level'],['L2','L3','L4'])) {				
//13,11,12
				$sql_status = "select grievance_status_id,grievance_status_desc from grievance_status_mst where grievance_status_id IN('10','12','11')";
			...
			$sql_status = "select grievance_status_id,grievance_status_desc from grievance_status_mst where grievance_status_id IN(10,11,12,13)";
		...
		if ($rs = mysqli_query($conn, $sql_status)) {
			...
			while ($row = mysqli_fetch_assoc($rs))
				$present_grievance_status_list[$row['grievance_status_id']] = $row['grievance_status_desc'];
		} else
	...
	if (isset($_POST['escalate_page'])) {
		...
		$sql = "select * from grievances_transactions where grievance_id ='" . $_REQUEST['grievance_id'] . "' and emp_id='" . $_SESSION['emp_id'] . "' order by transaction_id DESC limit 1";
...
//echo "<pre>";print_r($present_grievance_status_list);echo "</pre>";die();
	...
	$tpl->assign('present_grievance_status_list', $present_grievance_status_list);
	...
	$tpl->display('manage_reopen_comp_sel.tpl');
} else {
