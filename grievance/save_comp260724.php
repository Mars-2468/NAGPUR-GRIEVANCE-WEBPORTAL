...
if (isset($_POST['save'])) {
	...
	if (!($ext == "jpg" || $ext == "png" || $ext == "jpeg")) {
		$_SESSION['f1'] = "Allowed  Images formats Only";
		header("Location: web_complaint_form.php?id={$ulb_id}");
	} else {
		unset($_SESSION['f1']);
	}
	...
	if (mysqli_query($conn, $sql)) {
		...
		if (mysqli_num_rows($rs1) > 0) {
			...
			if (mysqli_query($conn, $sql2)) {
				...
				$sms = "Dear " . substr($emp_name_list[$row1['emp_id']], 0, 29) . ", A Complaint from " . substr($_POST['person_name'], 0, 5) . "," . substr($_POST['hno'], 0, 29) . "," . substr($_POST['address'], 0, 29) . ",Mobile No." . substr($_POST['mobile'], 0, 29) . ", regarding " . substr($_POST['comp_subject'], 0, 29) . " was alloted to you on " . $today . " Regards - Grievance Monitoring Cell , " . $ulb_info['ulbname'];
				$mobile = $emp_mobile_list[$row1['emp_id']];



				$sms1 = "Dear " . substr($_POST['person_name'], 0, 29) . " Your Complaint registered ref no " . substr($grievance_id, 0, 30) . ", Responsible officer, please contact " . substr($emp_name_list[$row1['emp_id']], 0, 29) . ", MOBILE " . $mobile . " -CMNR " . $ulb_info['ulbname'] . " " . $ulb_info['ulb_type_desc'];
				...
				try {
					$sms1 = "Dear " . $_POST['person_name'] . ", Your Grievance regarding " . substr($complaintType, 0, 30) . " with Ref No " . substr($grievance_id, 0, 30) . " was allotted to " . $emp_name_list[$row1['emp_id']] . " on " . date('d-m-Y H:i:s') . " is in process. Regards - Citizen Service Monitoring Cell , NMCGOV";
				...
				$sms = "Dear " . substr($emp_name_list[$row1['emp_id']], 0, 28) . ", A Grievance from " . mysqli_real_escape_string($conn, strip_tags(substr($_POST['person_name'], 0, 28))) . ", Mobile No. " . mysqli_real_escape_string($conn, strip_tags($_POST['mobile'])) . ", " . substr($complaintType, 0, 30) . " with Ref No " . $grievance_id . " is allotted to you on " . date('d-m-Y H:i:s') . " https://nmcnagpur.gov.in/grievance/ Regards- CitizenServiceMonitoringCell ,NMCGOV";

				$mobile = $emp_mobile_list[$row1['emp_id']];
				// $templateId = "1707166305897852132";
			...
			$sms = "Dear " . substr($_POST['person_name'], 0, 28) . ", Mobile No." . $_POST['mobile'] . " regarding " . substr($_POST['comp_desc'], 0, 30) . " with RefNo " . substr($grievance_id, 0, 30) . " Is Submitted Successfully on " . date('d-m-Y H:i:s') . " Regards - CFC ,  AMCORP";
