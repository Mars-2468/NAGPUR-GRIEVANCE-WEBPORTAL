<?php
	...
	{	    
		...
		while($row = $rs3->fetch_assoc())
		{
		    $status_list[$row['grievance_status_id']]['status_desc']=$row['grievance_status_desc'];
		}
		...
		$sql ="SELECT count(grievance_id) as count, grievance_status_id,cat3_id FROM `grievances` where grievance_status_id in(3,8,9) ";
		
		$sql .= " and ward_id=" . $ward_id . " ";
		$sql .= " group by grievance_status_id,cat3_id ";
		
		$rs = mysqli_query($conn,$sql);
		$data['total']=0;
		
		while($row = mysqli_fetch_assoc($rs)){
			$data[$row['cat3_id']][$row['grievance_status_id']]['count'] = $row['count'];
		...
		$tpl->display('corp_grievance_resolved_percent.tpl');
	}
	else
	{
