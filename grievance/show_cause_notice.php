...
if (isset($_SESSION['uid'])) {
	...
	if ($user_type1 == 'U') {	
		...
		$sql = "SELECT COUNT(DISTINCT sl.warning_id) as count,e.emp_name,d.dept_desc,sl.emp_id FROM `show_case_response_logs` as sl LEFT JOIN emp_mst as e ON e.emp_id=sl.emp_id LEFT JOIN dept_mst as d ON e.emp_dept = d.dept_id where sl.row_number%5=0 ";
		if ($selectedYear) { 
		...
		$sql = "SELECT COUNT(DISTINCT sl.warning_id) as count,e.emp_name,d.dept_desc,sl.emp_id FROM `show_case_response_logs` as sl LEFT JOIN emp_mst as e ON e.emp_id=sl.emp_id LEFT JOIN dept_mst as d ON e.emp_dept = d.dept_id where sl.emp_id = '" . $_SESSION['emp_id'] . "' and sl.row_number%5=0 ";
		...
		if ($_REQUEST['f_date'] != '' && $_REQUEST['t_date'] != '') {
			$sql .= " and date(sc.datetime) between '" . $fdate . "' and '" . $tdate . "'  and date_format(sl.date_regd, '%Y-%m-%d') >= '2024-09-01' ";
		}else{
			$sql .= "  and date_format(sl.date_regd, '%Y-%m-%d') >= '2024-09-01' ";	
...
//echo "<pre>";print_r($data);echo "</pre>";die();
	...
	$sql = "select sc.showcause_id,sc.emp_id,sc.showcase_count,sc.datetime,e.emp_id,e.emp_name,e.emp_dept from show_case_emp_count sc,emp_mst e where sc.emp_id=e.emp_id";
	...
	$tpl->display('show_cause_notice.tpl');
} else {
