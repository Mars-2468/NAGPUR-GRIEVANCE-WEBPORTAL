...
/* 	if (isset($_POST['save']) && ($_POST['token'] === $_SESSION['token'])) {
	...
	$sql = "select user_id,emp_id,user_name,user_mobile,user_type,has_access,is_active from  users where ulbid='" . mysqli_real_escape_string($conn, strip_tags($_SESSION['ulbid']))."' ";
	...
	$is_active_count = count(array_filter($data, fn($item) => $item['is_active'] == 1));
...
//echo "<pre>";print_r($desg_list);echo "</pre>";die();
	...
	$tpl->assign('is_active_count', $is_active_count);
	...
	$tpl->display('all_users_list.tpl');
} else {
