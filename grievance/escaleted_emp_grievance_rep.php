<?php
	...
	if(isset($_SESSION['uid']))
	{
		...
		$sql ="SELECT count(DISTINCT (gt.grievance_id)) as count , cat3_id,description,cm.cat_id,c.sub_cat_id,ward_id FROM `grievances` g, cs_mst c, category_mst cm , grievances_transactions gt where g.grievance_id=gt.grievance_id and  g.cat3_id=c.cs_id and c.cat_id=cm.cat_id and  `is_escalated` = 1  and emp_id in ('".$_SESSION['emp_id']."') group by cat3_id , ward_id order by count DESC";
	...
	while($row = $rs->fetch_assoc())
	{
		...
		$tpl->display('escaleted_emp_grievance_rep.tpl');
	}
	else
	{
