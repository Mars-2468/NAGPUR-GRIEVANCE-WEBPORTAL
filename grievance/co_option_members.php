...
require_once('csrf.class.php');
	...
	if(isset($_SESSION['uid']))
	{
		...
		$sql= "insert into council_mst(cid,name,designation,mobile,party,img_url,ulbid) values(?,?,?,?,?,?,?)";
		$query=$conn->prepare($sql);
		$query->bind_param("issssss",$cid,$name,$designation,$mobile,$party,$img_url,htmlspecialchars(strip_tags($_SESSION['ulbid'])));
		...
		$sql="select * from council_mst where ulbid=? and cid='3'";
		$query=$conn->prepare($sql);
		$query->bind_param("s",htmlspecialchars(strip_tags($_SESSION['ulbid'])));
		$query->execute();
		$rs=$query->get_result();
		...
		$tpl->display('co_option_members.tpl');
