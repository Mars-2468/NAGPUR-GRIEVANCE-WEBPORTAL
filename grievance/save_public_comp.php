...
//echo"<pre>";print_r(isset($_POST['save']));echo"</pre>";die();
...
if (isset($_POST['save'])) {
	...
	//ulbid
	$ulb_id = 250;
	$loc_of_gvp_id = $_POST['loc_of_gvp_id'];
	...
	if ($person_name == "") {
		$_SESSION['person_name'] = "Name is required";
		header("Location: public_complaint_form.php?loc_of_gvp_id={$loc_of_gvp_id}");
		exit;
	} elseif (!preg_match("/^[a-zA-Z0-9\s]+$/", $person_name)) {
		$_SESSION['person_name'] = "Only letters, numbers, and spaces are allowed";
		header("Location: public_complaint_form.php?loc_of_gvp_id={$loc_of_gvp_id}");
		exit;
	} else {
	...
	if ($address == "") {
		...
		//header("Location: complaint_form.php?loc_of_gvp_id={$loc_of_gvp_id}");
	} elseif (!preg_match("/^[\p{L}0-9\s,.\-–—\/#()\n\r]+$/u", $address)) {
	...
	$stmt->bind_param('i', $_POST['cs_id']);
	$stmt->execute();
	$rs = $stmt->get_result();
	$complaint_rec_row = $rs->fetch_assoc();
	
	//echo "<pre>";print_r($complaint_rec_row);echo "</pre>";die();
	...
	$comp = strip_tags($_POST['comp_desc']);

	
	if ($comp != "") {

		if (!preg_match("/^[a-zA-Z0-9\s]+$/", $comp)) {
			$_SESSION['comp_desc'] = "Only letters, numbers, and spaces are allowed";
			header("Location: public_complaint_form.php?loc_of_gvp_id={$loc_of_gvp_id}");
			exit;
		}

		if (strlen($comp) > 1500) {
			$_SESSION['comp_desc'] = "Description too long";
			header("Location: public_complaint_form.php?loc_of_gvp_id={$loc_of_gvp_id}");
			exit;
		}

	} else {
		unset($_SESSION['comp_desc']);
	}
	
	
	$sub = null;//sanitize_input($_POST['comp_subject']);
	
	$address = mysqli_real_escape_string($conn, strip_tags($_POST['address']));
	$auto_location = !empty($auto_location)?$auto_location:'';
	
	$latitude=$_POST['latitude']??$_POST['latitude2'];
	$longitude=$_POST['longitude']??$_POST['longitude2'];
	...
	$sql = "insert into grievances(
			...
			null,
			'" . $address . "',
			'" . strip_tags($_POST['zone_id']) . "',
			" . strip_tags($_POST['ward_id']) . ",
			...
			'1',
			now(),
			'" . $complaint_rec_row['cs_id'] . "',
			'" . $complaint_rec_row['cat_id'] . "',
			'" . $complaint_rec_row['sub_cat_id'] . "',
			null,
			'" . $_POST['ulbid'] . "',
			'" . $cutoffdate2 . "',
			null,
	...
	if (mysqli_query($conn, $sql)) {
		...
		if (is_uploaded_file($_FILES['f1']['tmp_name'])) {
			...
			if (move_uploaded_file($_FILES['f1']['tmp_name'], $photo_url)) {
				$photo_url = base_url() . "/grievance/photos/" . $newfile;
			} else {
				$photo_url = "#";
			}
		} else {
		...
		if ($_POST['cat_id'] == '3' && $_POST['ulbid'] == '052') {
			...
			$sql1 = "SELECT cs_id,emp_id,dept_id  FROM emp_map WHERE  cs_id ='" . $_POST['cs_id'] . "' and ward_id='" . $_POST['zone_id'] . "' and ulbid='" . $_POST['ulbid'] . "' and cs_type_id='1' and flag='1' and street_id='" . $_POST['ward_id'] . "'";
		}
		...
		$indexpage = "public_complaint_form.php?id=" . $_POST['ulbid'] . "&status=" . $status . "&ref_id=" . $grievance_id;
