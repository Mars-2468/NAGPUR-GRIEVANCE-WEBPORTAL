<?php
	...
	if(isset($_SESSION['uid']))
	{
		...
		else
		{
			...
			//$sql ="SELECT count(DISTINCT g.grievance_id) as count , cat3_id,`description`,cm.cat_id,c.sub_cat_id,gt.dept_id,ward_id FROM grievances g, grievances_transactions gt, cs_mst c, category_mst cm where g.grievance_id=gt.grievance_id AND g.cat3_id=c.cs_id and c.cat_id=cm.cat_id and `grievance_status_id` = 2 AND gt.dept_id IN( $deptlist ) " ;
			//$sql ="SELECT count(g.grievance_id) as count , cat3_id,`description`,cm.cat_id,c.sub_cat_id,gt.dept_id,gt.emp_id,ward_id FROM grievances g, grievances_transactions gt, cs_mst c, category_mst cm where g.grievance_id=gt.grievance_id AND g.cat3_id=c.cs_id and c.cat_id=cm.cat_id and `grievance_status_id` = 2 AND gt.dept_id IN( $deptlist ) AND gt.emp_id IN( $emplist ) " ;
		}		
		...
		$tpl->display('grievance_pending_rep.tpl');
	}
	else
	{
