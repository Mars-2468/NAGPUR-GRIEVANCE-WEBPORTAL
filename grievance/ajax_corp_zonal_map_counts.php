...
if ($_SESSION['zone_id'] != 'all') {

    $where = 'where g.ward_id="' . $_SESSION['zone_id'] . '"';
    $where1 = 'where g.ward_id="' . $_SESSION['zone_id'] . '"';
} else {
    $where1 = '';
    $where = '';
}
...
<table class="table table-bordered ">
    <thead>
        <tr class="boxed title-bar text-white" style="background:<?php echo  $data['count']['color'] ?>">
            <th colspan="" style="text-align:center;color:#FFF;">ZONE</th>
            <th style="text-align:center;color:#FFF;"><?php echo  $data['count']['zone'] ?> </th>
        </tr>
    </thead>
    <tr>
