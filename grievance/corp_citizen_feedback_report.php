<?php
	...
	c.cs_id = g.cat3_id and rating_no = ? and g.ulbid = ? and g.ward_id=? group by g.cat3_id");
	$rating_no=5;
	$ulbid=htmlspecialchars(strip_tags($_SESSION['ulbid']));
	$sql->bind_param("isi",$rating_no,$ulbid,$ward_id);
	$sql->execute();
	$rs=$sql->get_result();
	$totals[5]=0;
	$totals[4]=0;
	...
	while($row= $rs->fetch_assoc())
	{
		...
		$sql ="select count(DISTINCT(r.grievance_id)) as count,cat3_id from rating_mst r, grievances g where r.grievance_id=g.grievance_id and g.ward_id=".$ward_id." group by cat3_id";
		...
		$tpl->display('corp_citizen_feedback_report.tpl');
	}
	else
	{
