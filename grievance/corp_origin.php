...
if (isset($_SESSION['uid'])) {
	...
	$cs_id=$_REQUEST['cat3_id']??'';
	
	/* if (!empty($cs_id)) {
		$cs_id = intval($cs_id);

		$sql = "SELECT cs_id, cat_id, cs_desc AS comp_desc 
				FROM cs_mst 
				WHERE cs_id = $cs_id";
 ...
 */
	if ($_REQUEST['originid'] !== '') {
		...
		$sql = "select * from grievances where app_type_id = '1' and ulbid = '" . $_REQUEST['ulbid'] . "' and cat3_id = '" . $cs_id . "' and ward_id = " . $ward_id . " ";
...
/************************* pagination end  **************************/
	...
	$tpl->assign('cat3_id', $cs_id);
	$tpl->assign('grievance_status_id', $_REQUEST['grievance_status_id']);
