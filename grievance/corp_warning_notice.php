...
if (isset($_SESSION['uid'])) {
	...
	if ($_REQUEST['f_date'] != '' && $_REQUEST['t_date'] != '') {
		...
		$sql = "SELECT COUNT(DISTINCT sc.warning_id) as count,sc.emp_id FROM `show_case_response_logs` as sc LEFT JOIN emp_mst as e ON e.emp_id=sc.emp_id LEFT JOIN emp_map as em ON em.emp_id=sc.emp_id RIGHT JOIN dept_mst as d ON e.emp_dept = d.dept_id where em.ward_id = ".$ward_id." ";
		...
		if ($_REQUEST['f_date'] != '' && $_REQUEST['t_date'] != '') {
			...
			$sql .= "  and sc.date_regd >= date('2024-09-01') ";				
...
//echo "<pre>";print_r($emp_list[3]);echo "</pre>";die();
	...
	$tpl->display('corp_warning_notice.tpl');
} else {
