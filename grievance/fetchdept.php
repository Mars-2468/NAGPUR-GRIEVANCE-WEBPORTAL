...
if(!empty($_POST["emp_id"])) {
 $keyword=strip_tags($_POST["emp_id"]);
//$customer_name=$_POST["customer_name"];


        $sql1 ="SELECT emp_dept FROM emp_mst WHERE emp_id=?";
...
$rs1=$query->get_result();
	...
	//$dropdown="<td>".$_POST["ward_desc"]."</td><td><select id='dept_type' name='dept_type' class='form-control myinput_size' onchange='editemp(".$_POST["emp_id"].",".$_POST["ward_id"].",".json_encode($_POST["ward_desc"]).",".$_POST["cs_id"].",".this.")>
 ...
 $dropdown='<td>'.strip_tags($_POST["ward_desc"]).'</td><td>'.strip_tags($_POST["street_desc"]).'</td><td><select id="dept_type" name="dept_type" class="form-control myinput_size" 
onchange="editemp('.strip_tags($_POST["emp_id"]).','.strip_tags($_POST["street_id"]).','.$street_desc.','.strip_tags($_POST["ward_id"]).','.$ward_desc.','.strip_tags($_POST["cs_id"]).','.this.', '.$_REQUEST['cs_type_id'].')">
 	<option value="0">---select---</option>';
...
while($row1=$rs1->fetch_assoc())
			...
			if($row1['dept_id'] == $deptt_id1 )
			{
		$dropdown.="<option value='".$row1['dept_id']."' selected = '".$deptt_id1 ."'>".$row1['dept_desc']."</option>";
		}
		else
		{
...
while($row1=$rs1->fetch_assoc())
			{
			if($row1['emp_id'] == $_POST['emp_id'] )
			{
			$dropdown.="<option value='".$row1['emp_id']."' selected = '".$_POST['emp_id']."'>".$row1['emp_name']."(".$desg_list[$row1['emp_desg']].")"."-".$row1['emp_mobile']."</option>";
			}
			else
			{
...
while($row1=$rs1->fetch_assoc())
		...
		if($row1['emp_id'] == $_POST['emp_id2'] )
			{
			$dropdown.="<option value='".$row1['emp_id']."' selected = '".$_POST['emp_id2']."'>".$row1['emp_name']."(".$desg_list[$row['emp_desg']].")"."-".$row1['emp_mobile']."</option>";
			}
			else
			{
		$dropdown.="<option value='".$row1['emp_id']."'>".$row1['emp_name']."(".$desg_list[$row1['emp_desg']].")"."-".$row1['emp_mobile']."</option>";
...
while($row1=$rs1->fetch_assoc())
		...
		if($row1['emp_id'] == $_POST['emp_id3'] )
			{
			$dropdown.="<option value='".$row1['emp_id']."' selected = '".$_POST['emp_id2']."'>".$row1['emp_name']."(".$desg_list[$row1['emp_desg']].")"."-".$row1['emp_mobile']."</option>";
			}
			else
			{
		$dropdown.="<option value='".$row1['emp_id']."'>".$row1['emp_name']."(".$desg_list[$row1['emp_desg']].")"."-".$row1['emp_mobile']."</option>";
...
$rs1=$query->get_result();
	...
	while($row1=$rs1->fetch_assoc())
			...
			{
		$dropdown.="<option value='".$row1['emp_id']."'>".$row1['emp_name']."-".$row1['emp_mobile']."</option>";
		}
		  }
