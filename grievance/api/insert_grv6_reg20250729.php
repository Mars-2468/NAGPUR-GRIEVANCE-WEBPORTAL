	...
	if($rs=mysqli_query($conn,$sql))
	{
		...
		if($_FILES["image"]["name"] !='')
	    {
           ...
           if(!($ext =="jpg" || $ext =="png" || $ext =="PNG" || $ext =="JPG"))
           {
			...
			{
		        $target_file="https://" . $_SERVER['HTTP_HOST'] . '/csms/grievance/photos/'.$dat.'/'.$file;
			...
			if($swatchta_app_status_yn=='1')
		      ...
		      //$targetDate = date ( 'Y-m-d' , $date );
				...
				/*** sending sms to Employee ****/
				    ...
				    $sms ="Dear ".substr($emp_name_list[$row1['emp_id']], 0, 28)." i, A Grievance from ".mysqli_real_escape_string($conn,strip_tags(substr($data['person_name'], 0, 28))).",Mobile No.".mysqli_real_escape_string($conn,strip_tags($data['mobile']))." ,".substr($complaintType, 0, 28)."  with Ref No ".$grievance_id_sel."  was alloted to you on ".date('d-m-Y H:i:s')." Regards - CFC,  AMCORP";
					$mobile=$emp_mobile_list[$row1['emp_id']];
				...
				$sms1="Dear ".substr($data['person_name'], 0, 28).", Your Grievance  regarding ".substr($complaintType, 0, 28)." with Ref No ".$grievance_id_sel." was alloted to ".substr($emp_name_list[$row1['emp_id']], 0, 28)." on ".date('d-m-Y H:i:s')." Will be completed on ".$targetDate." Regards - CFC,  AMCORP";
					//echo "<br>";
