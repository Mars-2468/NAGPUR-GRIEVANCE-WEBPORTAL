...
// 	  $sql="select edition_date,img_url,description,description_marathi,ac.edition_no,ae.edition_no as edition_desc from add_content ac,add_edition ae where ac.edition_no=ae.id and ac.ulbid='".$_REQUEST['ulbid']."' and ac.edition_no='".$_REQUEST['edition_no']."'";
 $sql ="SELECT *  FROM users_test ";
	   if($_REQUEST['u_id'] != ''){
	       $sql .= "where user_id='".$_REQUEST['u_id']."'";
	   }
	if($rs=mysqli_query($conn,$sql))
	{
		if(mysqli_num_rows($rs)>0)
		{
			...
			{
			 //   if($langId==1){
				    $data=array('id' => $row['id'],  $row['id'], 'user_name' => (!empty($row['user_name']) || !is_null($row['user_name'])) ? $row['user_name'] : '' 
	                                , 'user_mobile' =>  (!empty($row['user_id']) || !is_null($row['user_id'])) ? $row['user_id'] : '' 
	                                , 'user_email' => (!empty($row['user_email']) || !is_null($row['user_email'])) ? $row['user_email'] : '' 
	                                , 'age' => (!empty($row['age']) || !is_null($row['age'])) ? $row['age'] : '' 
	                                , 'address' => (!empty($row['address']) || !is_null($row['address'])) ? $row['address'] : ''
	                                );
			 //   }else{
