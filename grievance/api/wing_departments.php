...
require_once('../connection.php');
	...
	$sql="select * from wing_department_mst order by priority asc";
	    ...
	    if(mysqli_num_rows($rs) > 0)
		{
        	...
        	$response['status_msg'] = 'success';
        	 $response['data'] = array();    
        		...
        		{
        		    if($lang_id==1){
        			$data=array('id'=>$row['id'], 'title'=>$row['title'], 'icon_url'=>$row['icon_url']);
        		    }else{
        		     	$data=array('id'=>$row['id'], 'title'=>$row['marathi_title'], 'icon_url'=>$row['icon_url']);
        		    }
