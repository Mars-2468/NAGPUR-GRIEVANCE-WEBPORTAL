...
//echo $newDate = date("d-m-Y", strtotime($originalDate));
			...
			$sql ="select grievance_status_id,grievance_status_desc from grievance_status_mst where grievance_status_id NOT IN('1','9','11','12')";
			$rs = mysqli_query($conn,$sql);
			while($row = mysqli_fetch_assoc($rs))
			{
	...
	if(mysqli_query($conn,$sql))
	{
		...
		else
		{
		    ...
		    if($_REQUEST['update_status']==3 || $_REQUEST['update_status']==8)
			 ...
			 $sms="Dear ".$person_name.", Your Service  regarding ".$subject." with Ref No : ".$_REQUEST['Complaint_no']." Status is ".$status_list[$_REQUEST['update_status']]." , ".$_REQUEST['remarks']." Regards - Citizen Service Monitoring Cell , ".$ulb_list[$ulbid];
		...
		$sql1="select g.grievance_id,app_type_id,date_regd,disposed_date,DATEDIFF(disposed_date,date_regd) AS target,gt.disposal_status,
				 ...
				 if($row1['target'] <= $row1['target_days'])
				{
				    ...
				    // end
            				    ...
            				    {
            				        $sql ="select reopened,reopened_completed from ulbid='".$ulbid."' and app_type_id='".$app_type_id1."'";
            				        $rs=mysqli_query($conn,$sql);
            				        $row= mysqli_fetch_assoc($rs);
            				        $reopened=$row['reopened']-1;
            				        $reopened_completed=$row['reopened_completed']+1;
            				        $sql ="update dashboard_count set reopened='".$reopened."',reopened_completed='".$reopened_completed."' where ulbid='".$ulbid."' and app_type_id='".$app_type_id1."'";
            				        mysqli_query($conn,$sql);
				    ...
				    // end
					    if($_REQUEST['update_status']==3 || $_REQUEST['update_status']==8 ||  $_REQUEST['update_status']==9)
            				   ...
            				   else if($_REQUEST['update_status']==4 || $_REQUEST['update_status']== 10)
            			        {
                				    ...
                				    $sql ="select under_pro_be_sla from dashboard_count where app_type_id='".$app_type_id1."' and ulbid='".$ulbid."'";
                				    $rs = mysqli_query($conn,$sql);
                				    $rows = mysqli_fetch_assoc($rs);
                				    ...
                				    $sql ="update dashboard_count set under_pro_be_sla='".$under_pro_be_sla."' where app_type_id='".$app_type_id1."' and
                				    ulbid='".$ulbid."'";
                				    mysqli_query($conn,$sql);
