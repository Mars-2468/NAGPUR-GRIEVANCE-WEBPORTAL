	...
	if($_REQUEST['generic_id'] !='' && $_REQUEST['ulbid'] !='' && $_REQUEST['imei_no'] !='' && $_REQUEST['rating_no'] !='0')
	{
	    ...
	    if($_REQUEST['ulbid']==207)
	    {
	      ...
	      if($nr > 0)
    	   ...
    	   $sql="insert into rating_mst(grievance_id,imei_no,rating_no,comment_desc,sub_option_id,resolved_id)values('".$_REQUEST['generic_id']."','".$_REQUEST['imei_no']."','".(integer)$_REQUEST['rating_no']."','".mysqli_real_escape_string($conn,$_REQUEST['comment_desc'])."','".$_REQUEST['sub_option_id']."','".$_REQUEST['sub_option_id']."') ON DUPLICATE KEY UPDATE comment_desc='".mysqli_real_escape_string($conn,$_REQUEST['comment_desc'])."',resolved_id='".$_REQUEST['sub_option_id']."'";
