...
$sql = "select g.grievance_status_id,grievance_id,address,ward_id,street_id,mobile,comp_subject,comp_desc,grievance_status_desc,date_regd,
	...
	cm.cat_id=c.cat_id and g.grievance_status_id=gsm.grievance_status_id and mobile like '" . $_REQUEST['imei'] . "' and g.ulbid='" . $_REQUEST['ulbid'] . "' order by grievance_id desc ";
// 	echo $sql;
	...
	if (mysqli_num_rows($rs) > 0) {
		while ($row = mysqli_fetch_assoc($rs)) {
			...
			if ($row['grievance_status_desc'] == 'Completed') {
				...
				$data[] = array(
					'sno' => $i, 'grievance_id' => $row['grievance_id'], 'address' => $row['address'], 'comp_desc' => $row['comp_desc'],
		...
		/*10-07-2024 $data[0] = array('grievance_id' => '0', 'address' => 'N/A', 'comp_desc' => 'N/A', 'date_regd' => 'N/A', 'grievance_status_desc' => 'N/A', 'file_url' => 'N/A', 'subject' => 'N/A');
		$response['data'] = $data;*/
	}
} else{
		$response['status_code'] = 400;
