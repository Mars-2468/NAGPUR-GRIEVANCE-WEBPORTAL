...
ini_set('display_errors',0);
	...
	$conn=getconnection();
  	...
  	if(isset($_REQUEST['emp_id']))
  	{
  		...
  		$empid= $_REQUEST['emp_id'];
  		 if($user_type == 'U')
  	    {
  	        $sql="SELECT COUNT(grievance_id) as  date_regd FROM `grievances` WHERE `app_type_id` LIKE '2' AND `ulbid` LIKE '".$ulbid."'";
  	    }
  	    else
  	    {
		$sql="SELECT IFNULL(count(g.grievance_id),0) as date_regd FROM grievances_transactions gt,grievances g where g.grievance_id=gt.grievance_id and 
		gt.emp_id='".$empid."' and g.app_type_id = 2 and gt.disposal_status !=5";
  	    }
		...
		//$sql="select IFNULL(count(g.grievance_id),0) as grievance_id,g.app_type_id  from grievances g , grievances_transactions gt where g.grievance_id=gt.grievance_id and g.grievance_status_id NOT IN('3','8')  and gt.emp_id='".$empid."' and TIMESTAMPDIFF(HOUR,date_regd,now()) <= 24 and g.app_type_id = 1";
		 if($user_type == 'U')
  	    {
  	         $sql="SELECT COUNT(grievance_id) as  grievance_id FROM `grievances` WHERE `app_type_id` LIKE '2' AND `ulbid` LIKE '".$ulbid."' and 
		...
		g.grievance_status_id IN('2')  and gt.emp_id='".$empid."' and sla_status='1' and g.app_type_id = 2 and gt.disposal_status !=5 and (is_reopened_yn='0' or is_reopened_yn is NULL)";
	...
	/*	$sql="select IFNULL(count(g.grievance_id),0) as grievance_id,g.app_type_id  from grievances g , grievances_transactions gt where 
		...
		$sql="select IFNULL(count(g.grievance_id),0) as grievance_id,g.app_type_id  from grievances g , grievances_transactions gt where 
		g.grievance_id=gt.grievance_id and g.grievance_status_id IN('2')  and gt.emp_id='".$empid."' and sla_status='2' and 
		g.app_type_id = 2 and gt.disposal_status !=5 and (is_reopened_yn='0' or is_reopened_yn is NULL)";
  	    }
		$rs=mysqli_query($conn,$sql);				
		while($row = mysqli_fetch_assoc($rs))
		{
		...
		if($user_type == 'U')
  	    {
  	        $sql="SELECT COUNT(grievance_id) as  grievance_id FROM `grievances` WHERE `app_type_id` LIKE '2' AND `ulbid` LIKE '".$ulbid."' and 
  	        grievance_status_id IN('3','8','4','9')";
  	    }
  	    else
  	    {
		...
		g.grievance_status_id IN('3','8','4','9') and gt.emp_id='".$empid."' and g.app_type_id = 2 and gt.disposal_status !=5 and (is_reopened_yn='0' or is_reopened_yn is NULL)";
  	    }
		...
		if($user_type == 'U')
  	    {
  	        $sql="SELECT COUNT(grievance_id) as  grievance_id FROM `grievances` WHERE `app_type_id` LIKE '2' AND `ulbid` LIKE '".$ulbid."' and grievance_status_id IN('10')";
  	    }
  	    else
  	    {
		$sql="select count(g.grievance_id) as grievance_id,g.app_type_id from grievances g , grievances_transactions gt where g.grievance_id=gt.grievance_id and 
		g.grievance_status_id IN('10')and gt.emp_id='".$empid."' and g.app_type_id = 2 and gt.disposal_status !=5 and (is_reopened_yn='0' or is_reopened_yn is NULL)";
  	    }
		$rs=mysqli_query($conn,$sql);				
		while($row = mysqli_fetch_assoc($rs))
		{
			$response['rejected']=$row['grievance_id'];
		} 
		
		
		            if($user_type == 'U')
		            {
		               $sql3 ="select IFNULL(count( DISTINCT g.grievance_id),0) as count,grievance_status_id from grievances where grievance_status_id IN('11','12','13') and ulbid='".$ulbid."' and app_type_id='2' group by app_type_id,grievance_status_id";
		            
		                
		            }
		            else 
				        {
				        $sql3 ="select IFNULL(count(gt.grievance_id),0) as count,app_type_id from grievances g,grievances_transactions gt where g.grievance_id=gt.grievance_id and emp_id='".$empid."' and gt.disposal_status IN('9','12') and is_reopened_yn='1'  and g.grievance_status_id IN('13') group by app_type_id";
				        $sql4 ="select IFNULL(count(DISTINCT g.grievance_id),0) as count,disposal_status as grievance_status_id from grievances g,grievances_transactions gt where g.grievance_id=gt.grievance_id and gt.disposal_status IN('11','12') and gt.emp_id='".$empid."' and app_type_id='2' group by grievance_status_id";
				        }
