...
// 	require_once('../check_access_key.php');
// 	echo "hi";
    // return check_access_key($_REQUEST['check_access_key']);
    // die;
    ...
    if(isset($_REQUEST['user_id']))
  	{
		...
		if($nr > 0)
		{
    		...
    		$sms = "One Time Password (OTP) for NMC application logging is " . $otp . " Please use this OTP for logging NMCGov. Pls do not share this with any one, Valid for 5 minutes.&priority=ndnd&stype=normal";
...
// 			$message1="Dear Customer, Your OTP for Login is ".$otp.". Use this OTP to Validate, Thank you. Regards, AMCORP";
		...
		var_dump($user_mobile);die();			
                    $result=sendSMS($user_mobile,$sms); //result from mobile seva server
...
curl_close($curl);
					...
					/*$response['status_code'] = 200;
						$response['message']   = 'OTP sent to your mobile number and Email Id';
						$response['user_id']   = $row['user_id'];
						$response['user_type'] = $row['user_type'];
						$response['otp_status']= 1;
						$response['ulbid']     = $row['ulbid'];
						$response['otp']       = $otp;
					$response['emailotp']  = $otp;*/
    		...
    		$sql = "select * from users_test where login_status='1' and (user_id='".$uid."' or user_email='".$uid."')";
    		    ...
    		    if($nr > 0)
    		    {
        		    ...
        		    $emailotp = rand(1000,9999);
                    $otpexpiredatetime1 = date('Y-m-d H:i:s',strtotime('+5 minutes',strtotime($datetime)));
                    $updateuser="update users_test set otp='".$otp."',otpdatetime='".$datetime."',otpexpiretime='".$otpexpiredatetime1."',otp_status='1',emailOtp='".$emailotp."'  where user_id='".$row["user_id"]."' and ulbid='".$row['ulbid']."'";
                    $updateres = mysqli_query($conn,$updateuser);
			        $message1  = "Dear ".$row["user_name"].",%0aThanks for downloading ".$uName." citizen buddy application,%0aYour OTP for registration is ".$otp;
    				$message   = str_replace ( ' ', '%20', $message1);
    				$user_mobile = $row["user_id"];
    				$url = "https://manage.smssolutions.in/smsapi/index?key=35FD85B7BD7DA4&campaign=0&routeid=6&type=text&contacts=".$user_mobile."&senderid=TSMCPL&msg=".$message;
    				//$url = "https://manage.smssolutions.in/smsapi/index?key=55E872F291A860&campaign=0&routeid=6&type=text&contacts=".$user_mobile."&senderid=VMAXES&msg=".$message;
					...
					$mailbody = 'Dear '.$row["user_name"].',<br> Thanks for downloading '.$uName.' citizen buddy application,<br> Your OTP for registration is <b>'.$emailotp.'</b>';
					...
					else
					{
						...
						'user_email'  =>$row['user_email'],
						'user_name'   =>$row['user_name'],
                        'message'     =>'OTP sent to your mobile number and Email Id',"version"=>"1.1");
					} 
					*/
