...
if (!empty($data["user_id"]) && !empty($data["user_name"])) {
        ...
        if ($errors == 0) {
            try {
				...
				/* $sql = "INSERT INTO users_test(user_id, user_name, user_email, user_type, created_at, ulbid, login_status)
                        VALUES('" . $data["user_id"] . "','" . $data["user_name"] . "','" . $data["user_email"] . "','D','" . date("Y-m-d H:i:s") . "','" . $data['ulbid'] . "','1')";
             */
			 
			$sql = $conn->prepare("
                INSERT INTO users_test 
                    (user_id, user_name, user_email, user_type, created_at, ulbid, login_status, otp_status)
                VALUES 
                    (?, ?, ?, 'D', ?, ?, '2','1')
            ");
    ...
    if (mysqli_query($conn, $sql)) {
		...
		$result = sendSMS($user_mobile, $sms);
