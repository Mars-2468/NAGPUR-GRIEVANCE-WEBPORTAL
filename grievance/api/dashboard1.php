<?php
	...
	$conn=getconnection();
  	...
  	if(isset($_REQUEST['emp_id']))
  	{
		...
		$sql="select IFNULL(count(g.grievance_id),0) as grievance_id,g.app_type_id  from grievances g , grievances_transactions gt where g.grievance_id=gt.grievance_id and g.grievance_status_id NOT IN('3','8','6')  and gt.emp_id='".$empid."' and DATEDIFF(now(),date_regd) <= 1 and g.app_type_id = 1 and gt.disposal_status !=5";
		...
		$sql="select count(g.grievance_id) as grievance_id,g.app_type_id from grievances g , grievances_transactions gt where g.grievance_id=gt.grievance_id and g.grievance_status_id IN('3','8') and gt.emp_id='".$empid."' and g.app_type_id = 1 and gt.disposal_status !=5";
		$rs=mysqli_query($conn,$sql);				
		while($row = mysqli_fetch_assoc($rs))
		{
