...
//echo "<pre>";print_r($_POST);echo "</pre>";die();
	...
	try {
		if (isset($_POST['save'])) {
			...
			$rca =  sanitize_input($dis_remark);//$_REQUEST['rca'];
			$ca =  sanitize_input($dis_remark);//$_REQUEST['ca'];
	...
	if (isset($_POST['grievance_id'])) {
		...
		if ($app_type_id == '1') {
			...
			if(($status == '5' || $status == '10') && in_array($data1['grievance_at_emp_level'],['L1','L2','L3','L4'])){
			//echo "<pre>";print_r($status);echo "</pre>";die('sss');	
				if($status == '5')
				$sql_status = "select grievance_status_id,grievance_status_desc from grievance_status_mst where grievance_status_id IN('2')";
				else
					$sql_status = "select grievance_status_id,grievance_status_desc from grievance_status_mst where grievance_status_id IN('11')";
