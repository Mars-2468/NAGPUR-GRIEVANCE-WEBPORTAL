...
while($row = mysqli_fetch_assoc($rs))
{
	if($row['grievance_at_emp_level'] == 'L1'){
	    ...
	    $todayDate = strtotime(date('Y-m-d H:i:s'));
			...
			if(count($emp1) <=0)
			{
				$sql ="select * from emp_map em, emp_mst_od e where em.emp_id=e.emp_id and ward_id='".$row['ward_id']."' and street_id='".$row['street_id']."' and cs_id='".$row['cat3_id']."' and cs_type_id='1' ";
			    $rs2 = mysqli_query($conn,$sql);
			    $emp1 = mysqli_fetch_assoc($rs2);
			}
			...
			if(mysqli_query($conn,$sql))
			{
				    ...
				    // 	$sms ="Dear ".$emp2['emp_name'].", Complaintfrom ".$row['person_name'].", Mobile No ".$row['mobile'].",regarding".$cs_list[$row['cat3_id']]." with Ref No : ".$row['grievance_id']." was allotted to ".$emp1['emp_name']." on ".date('d-m-Y H:i:s',strtotime($preveious_employee_allotteddate['alloted_date']))." is notresolved https://www.nmcnagpur.gov.in//grievance/Regards - Citizen ServiceMonitoring Cell ,NMCGOV";
                    $sms = "Dear ".$emp2['emp_name'].", Complaintfrom ".$row['person_name'].", Mobile No ".$row['mobile'].",regarding".$cs_list[$row['cat3_id']]." with Ref No : ".$row['grievance_id']." was allotted to ".$emp1['emp_name']." on ".date('d-m-Y H:i:s',strtotime($preveious_employee_allotteddate['alloted_date']))." is notresolved https://www.nmcnagpur.gov.in//grievance/Regards - Citizen ServiceMonitoring Cell ,NMCGOV";
				    ...
				    //echo $url ="http://smsatm.net/v3/api.php?username=ASCDCL&apikey=c01f32640f54e44f7660&senderid=AMCGOV&templateid=".$templateId."&mobile=".$mobile."&message=".$message;
			...
			if(mysqli_query($conn,$sql)){
    			...
    			if(mysqli_query($conn,$sql))
    			{
    				    ...
    				    // 	$sms ="Dear ".$emp2['emp_name'].", Complaintfrom ".$row['person_name'].", Mobile No ".$row['mobile'].",regarding".$cs_list[$row['cat3_id']]." with Ref No : ".$row['grievance_id']." was allotted to ".$emp1['emp_name']." on ".date('d-m-Y H:i:s',strtotime($preveious_employee_allotteddate['alloted_date']))." is notresolved https://www.nmcnagpur.gov.in//grievance/Regards - Citizen ServiceMonitoring Cell ,NMCGOV";
    				    // 	$sms = "Dear ".$emp2['emp_name'].", Complaintfrom ".$row['person_name'].", Mobile No ".$row['mobile'].",regarding".$cs_list[$row['cat3_id']]." with Ref No : ".$row['grievance_id']." was allotted to ".$emp1['emp_name']." on ".date('d-m-Y H:i:s',strtotime($preveious_employee_allotteddate['alloted_date']))." is notresolved https://www.nmcnagpur.gov.in//grievance/Regards - Citizen ServiceMonitoring Cell ,NMCGOV";
    					$sms = "Dear ".$emp2['emp_name']." , Complaintfrom ".$row['person_name']." , Mobile No. ".$row['mobile']." regarding".$cs_list[$row['cat3_id']]." with Ref No : ".$row['grievance_id']." was allotted to ".$emp1['emp_name']." on ".date('d-m-Y H:i:s',strtotime($preveious_employee_allotteddate['alloted_date']))." is not resolved https://www.nmcnagpur.gov.in//grievance/Regards - Citizen Service Monitoring Cell ,NMCGOV";
    					$mobile=$emp2['emp_mobile'];
