	 ...
	 $sql ="select cs_id,comp_desc from category3_mst where ulbid='".mysqli_real_escape_string($conn,strip_tags($_SESSION['ulbid']))."'";
		...
		$sql ="select grievance_id from grievances where 
			...
			else
			{
			 ...
			 $sql="insert into grievances(app_type_id,person_name,email,hno,address,ward_id,otherward,street_id,otherstreet,mobile,altmobile,comp_subject,comp_desc,
			 ...
			 '".mysqli_real_escape_string($conn,htmlspecialchars($_POST['otherwoarddesc']))."',
			 
			 '".mysqli_real_escape_string($conn,htmlspecialchars($_POST['street_id']))."',
			 '".mysqli_real_escape_string($conn,htmlspecialchars($_POST['otherstreetdesc']))."',
		...
		if(is_uploaded_file($_FILES['f1']['tmp_name']))
		    ...
		    if($_POST['emp_id'] == '22222222222' || $_POST['emp_id'] == '')
		    {
        			...
        			if($_POST['app_type_id']=='1' && $_POST['dept_id']=='3' && $_SESSION['ulbid']=='052')
        			{
        			     $sql1="SELECT emp_id1 as emp_id  FROM water_tanker_emp_map WHERE  water_tank_id ='".mysqli_real_escape_string($conn,strip_tags($_POST['tanker_id']))."'  and ulbid='".mysqli_real_escape_string($conn,strip_tags($_SESSION['ulbid']))."'";
        			}
        			else
        			{
...
$sql="select * from ulbmst order by ulbname";
		if($rs=mysqli_query($conn,$sql))
		{
			while($row = mysqli_fetch_assoc($rs))
		...
		$sql="SELECT * FROM `enquiryList`";
		if($rs=mysqli_query($conn,$sql))
		{
			while($row = mysqli_fetch_assoc($rs))
				$reg_list[$row['id']]=$row['listdesc'];
		}
		else
			printf("Errormessage: %s\n", mysqli_error($conn));
