<?php
	...
	//echo "<pre>";print_r($_POST);echo "</pre>";die();
			...
			if(	!validateField($_POST['app_type_id'],'dnumber')['valid'] || 
				!validateField($_POST['person_name'],'text')['valid'] || 
				!validateField($_POST['email'],'email')['valid'] || 
				!validateField($_POST['hno'],'text')['valid'] || 
				!validateField($_POST['address'],'address')['valid'] || 
				!validateField($_POST['ward_id'],'dnumber')['valid'] || 
				!validateField($_POST['street_id'],'dnumber')['valid'] || 
				!validateField($_POST['mobile'],'mobile')['valid'] || 
				!validateField($_POST['comp_subject'],'sptext')['valid'] || 
				!validateField($_POST['comp_desc'],'sptext')['valid'] || 
				!validateField($_POST['grievance_origin_id'],'dnumber')['valid'] ){
					
				$msg  = "Enter Valid complaint person_name/email/hno/address/mobile/subject/description...!";
				$class = "alert alert-danger display-hide";
				set_flash($msg,$class);
				header('Location:corp_register_comp_helpline.php');
				exit;
				}else{
				...
				if($nr > 0)
				{
					...
					{			    
						...
						if(is_uploaded_file($_FILES['f1']['tmp_name']))
						{
							...
							{
								$photo_url=base_url() . "/grievance/".$photo_url;
						...
						if($_POST['app_type_id']==1)
						{
							...
							$class= 'alert alert-warning display-hide';
							$msg='updated Service not assigned to employee , please map employee in emp matp';
						 ...
						 if($nr > 0)
						 {
						   ...
						   if($_POST['emp_id'] == '22222222222' || $_POST['emp_id'] == '')
							...
							if(mysqli_query($conn,$sql2))
							{
								...
								$class= 'alert alert-success display-hide';
								$msg='inserted successfully - ' . $grievance_id;
							...
							if($old_complaint_check_id != 1){			 
							$sms = "Dear ".substr($emp_name_list[$row1['emp_id']], 0, 29).", A Grievance from ".mysqli_real_escape_string($conn,strip_tags(substr($_POST['person_name'], 0, 29))).", Mobile No. ".mysqli_real_escape_string($conn,strip_tags(substr($_POST['mobile'], 0, 29))).", ".substr($complaintType, 0, 30)." with Ref No ".$grievance_id."is allotted to you on ".date('d-m-Y H:i:s')." https://nmcnagpur.gov.in/grievance/ Regards- CitizenServiceMonitoringCell ,NMCGOV";
