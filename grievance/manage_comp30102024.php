...
if (isset($_SESSION['uid'])) {
	...
	if ($_SESSION['user_type'] == 'A') {
		$sql = "select grievance_id,emp_dept from grievances_transactions,emp_mst where grievances_transactions.emp_id=emp_mst.emp_id and 
		...
		$sql2 = "select grievance_id,emp_dept from grievances_transactions,emp_mst_od where grievances_transactions.emp_id=emp_mst.emp_id and 
			 disposal_status ='2' and app_type_id like '%" . mysqli_real_escape_string($conn, preg_replace('/[^A-Za-z0-9]/', ' ', $_POST['app_type_id'])) . "%' and g.grievance_id like '%" . mysqli_real_escape_string($conn, preg_replace('/[^A-Za-z0-9]/', ' ', $_POST['ref_no'])) . "%' and 
			 g.person_name like '%" . trim(mysqli_real_escape_string($conn, preg_replace('/[^A-Za-z0-9]/', ' ', $_POST['applicant_name']))) . "%' and g.mobile like '%" . mysqli_real_escape_string($conn, preg_replace('/[^A-Za-z0-9]/', ' ', $_POST['mobile'])) . "%' and 
			 ward_id like '%" . mysqli_real_escape_string($conn, preg_replace('/[^A-Za-z0-9]/', ' ', $_POST['ward_id'])) . "%' and gt.dept_id like '%" . mysqli_real_escape_string($conn, preg_replace('/[^A-Za-z0-9]/', ' ', $_POST['dept_id'])) . "%' and 
			 g.cat3_id like '%" . mysqli_real_escape_string($conn, preg_replace('/[^A-Za-z0-9]/', ' ', $_POST['cat3_id'])) . "%' order by grievance_id desc";
		...
		if ($_SESSION['mc_yn'] == 1) {
			$sql = "select g.grievance_id,emp_dept,g.app_type_id from grievances_transactions gt,emp_mst e,grievances g where 
			...
			$sql2 = "select g.grievance_id,emp_dept,g.app_type_id from grievances_transactions gt,emp_mst_od e,grievances g where 
			 g.grievance_id=gt.grievance_id and gt.emp_id=e.emp_id and gt.disposal_status IN('2','11','6','13') and g.ulbid='" . mysqli_real_escape_string($conn, $_SESSION['ulbid']) . "' and 
			 app_type_id like '%" . mysqli_real_escape_string($conn, preg_replace('/[^A-Za-z0-9]/', ' ', $_POST['app_type_id'])) . "%' and g.grievance_id like '%" . mysqli_real_escape_string($conn, preg_replace('/[^A-Za-z0-9]/', ' ', $_POST['ref_no'])) . "%' and 
			 g.person_name like '%" . trim(mysqli_real_escape_string($conn, preg_replace('/[^A-Za-z0-9]/', ' ', $_POST['applicant_name']))) . "%' and g.mobile like '%" . mysqli_real_escape_string($conn, preg_replace('/[^A-Za-z0-9]/', ' ', $_POST['mobile'])) . "%' and 
			 ward_id like '%" . mysqli_real_escape_string($conn, preg_replace('/[^A-Za-z0-9]/', ' ', $_POST['ward_id'])) . "%' and gt.dept_id like '%" . mysqli_real_escape_string($conn, preg_replace('/[^A-Za-z0-9]/', ' ', $_POST['dept_id'])) . "%' and g
			...
			$sql = "select g.grievance_id,g.grievance_at_emp_level,emp_dept,g.app_type_id from grievances_transactions gt,emp_mst e,grievances g where 
			...
			$sql2 = "select g.grievance_id,g.grievance_at_emp_level,emp_dept,g.app_type_id from grievances_transactions gt,emp_mst_od e,grievances g where 
		...
		$sql = "select g.grievance_id,e.emp_dept,g.app_type_id from grievances_transactions gt,emp_mst e,grievances g where 
	...
	if ($rs = mysqli_query($conn, $sql)) {
		while ($row = mysqli_fetch_assoc($rs)) {
			$data2[$row['grievance_id']]['emp_dept'] = $row['emp_dept'];
			$data2[$row['grievance_id']]['app_type_id'] = $row['app_type_id'];
			$grievance_level[$row['grievance_id']]['grievance_at_emp_level'] = $row['grievance_at_emp_level'];
		}
	}
	...
	$tpl->assign('grievance_level', $grievance_level);
	if ($_SESSION['mc_yn'] == 1) {
		$sql = 'select grievance_id,person_name,email,hno,address,ward_id,street_id,mobile,comp_subject,comp_desc,grievance_origin_id,
		  grievance_status_id,date_regd,cat3_id,file_no,app_type_id,mcat3_id from grievances 
		  where grievance_status_id IN("2","11","6","13")  and grievance_id in(' . implode(",", array_keys($data2)) . ') ';
	} else {

		$sql = 'select grievance_id,person_name,email,hno,address,ward_id,street_id,mobile,comp_subject,comp_desc,grievance_origin_id,
		  grievance_status_id,date_regd,cat3_id,file_no,app_type_id,mcat3_id from grievances 
		  where grievance_status_id IN("2","11","6")  and grievance_id in(' . implode(",", array_keys($data2)) . ') ';
	}
	//echo $sql;
	if ($rs = mysqli_query($conn, $sql)) {
	...
	if ($_SESSION['user_type'] == 'E') {
		$sql = "select dept_id,dept_desc from dept_mst dm, emp_mst e  where e.emp_dept=dm.dept_id and e.emp_id='" . mysqli_real_escape_string($conn, $_SESSION['emp_id']) . "'";
	}
